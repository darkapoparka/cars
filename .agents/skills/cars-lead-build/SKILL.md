...
Use the existing canonical Cars checkout on synchronized main. `new-client.mjs` now performs checkout and current upstream discovery checks itself, verifies committed releases, refuses drift/duplicates and stages all three copies before canonical installation. Newer unfinished development is reported rather than released automatically. Complete release review/promotion for a finished update within the task; do not claim an older snapshot is the newest release. `template-release.mjs discover` and `status` remain available for standalone release work. Preview creation with `node scripts/new-client.mjs --client <slug> --repository <owner/repo> --preset standard --dry-run` (or import). Continue the authorized build with the same command without dry-run. Never bypass an unexplained release hold.
