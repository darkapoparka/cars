	...
	const reviews: Review[] = [
		{
			id: 'ivan-dimitrov',
			description:
				'Получих ясна информация за автомобила, бърза организация за оглед и коректно съдействие по документите.',
			avatar: '/assets/images/avatar/avatar-1.png',
			name: 'Иван Димитров',
			meta: 'Клиент на Day Night Auto'
		},
		{
			id: 'maria-georgieva',
			description:
				'Финансирането беше обяснено спокойно и разбираемо, а автомобилът отговаряше на описанието и снимките.',
			avatar: '/assets/images/avatar/avatar-3.webp',
			name: 'Мария Георгиева',
			meta: 'Покупка с финансиране'
		},
		{
			id: 'nikolay-petrov',
			description:
				'Огледът мина без изненади, а екипът помогна с проверката, регистрацията и следващите стъпки.',
			avatar: '/assets/images/avatar/avatar-4.png',
			name: 'Николай Петров',
			meta: 'Клиент от София'
		}
	];
							...
							<div class="daynight-home-action-card__content">
								...
								<ul class="daynight-home-action-card__list">
									{#each balancedActionCards ? card.balancedBullets : card.bullets as bullet (bullet)}
										<li><img src="/assets/icons/check.svg" alt="" aria-hidden="true" />{bullet}</li>
									{/each}
								</ul>
								<div class="home-action-button">{#if card.modifier === 'inventory'}<DesktopBrowseLink
...
<style>
	...
	:global(body.daynight-home-page) .daynight-home-action-card__list img {
		flex: 0 0 auto;
		height: 15px;
		width: 15px;
	}
