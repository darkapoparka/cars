<script lang="ts">
	...
	let detailsExpanded = $state(
		browser &&
			['plate', 'vin', 'make', 'model', 'year', 'mileage', 'phone'].some((key) => initialParam(key))
	);
	...
	// `/sell-your-car/request` renders this same component as the confirmation step
	// after the mobile form successfully writes a durable sell lead.
	const isRequestStep = $derived(
		browser ? appPage.url.pathname.endsWith('/request') : initialPath.endsWith('/request')
	);

	type SellEditHref = '/sell-your-car' | `/sell-your-car?${string}`;

	// Back from the confirmation to the form, keeping the entered values in the
	// URL so the fields stay prefilled even after a full page load.
	const editPath = $derived.by((): SellEditHref => {
		if (!browser) {
			return '/sell-your-car';
		}

		const qs = appPage.url.searchParams.toString();
		return qs ? `/sell-your-car?${qs}` : '/sell-your-car';
	});
	...
	const submittedFields = $derived.by(() => {
		...
		return labels
			.map(([key, label]) => ({ label, value: params.get(key)?.trim() ?? '' }))
			.filter((field) => field.value);
	});

	type SellRequestHref = '/sell-your-car/request' | `/sell-your-car/request?${string}`;

	const requestPath = $derived.by((): SellRequestHref => {
		const params = new SvelteURLSearchParams();
		if (plate.trim()) params.set('plate', plate.trim().toUpperCase());
		if (vin.trim()) params.set('vin', vin.trim().toUpperCase());
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		if (result.ok) {
			...
			sellSubmitMessage = '';
			await goto(resolve(requestPath));
			return;
		}
...
<div class="mobile-sell">
	<header class="ms-hero">
		<img
			...
			aria-hidden="true"
		/>
		<div class="ms-hero__bar">
			<a class="ms-hero__brand" href={resolve('/')} aria-label="Day Night Auto home">
				<img src={resolve('/brand/daynight-logo-generated.png')} alt={daynightSite.shortName} />
			</a>
			<a class="ms-hero__phone" href={phoneHref} aria-label="Обади се">
	...
	<main id="main-content" tabindex="-1" class={{ 'ms-main--sheet': !isRequestStep }}>
		{#if isRequestStep}
			<section class="ms-panel ms-panel--form" aria-labelledby="ms-confirm-title">
				<div class="ms-section-head">
					<div>
						<span>Готово</span>
						<h2 id="ms-confirm-title">Получихме данните</h2>
					</div>
				...
				<a class="ms-secondary-action" href={resolve(editPath)}>
					<span>Редактирай данните</span>
				</a>
			</section>
		{/if}
		...
		{#if !isRequestStep}
			<nav class="ms-disclosures" aria-label="Повече за услугата">
				<button
					class="ms-disclosures__card ms-disclosures__card--process"
					type="button"
					onclick={() => openInfo('process')}
				>
					<img
						src={resolve('/assets/images/sell/process-disclosure-list-v1.webp')}
						alt=""
						aria-hidden="true"
					/>
					<span class="ms-disclosures__copy">
						<img
							class="ms-disclosures__brand"
							src={resolve('/brand/daynight-logo-generated.png')}
							alt=""
							aria-hidden="true"
						/>
						<strong>Как работи</strong>
						<small>Три ясни стъпки до конкретна оценка.</small>
						<span class="ms-disclosures__cta">
							<span>Виж стъпките</span>
							<ChevronRight size={15} strokeWidth={2.6} aria-hidden="true" />
						</span>
					</span>
				</button>
				<button
					class="ms-disclosures__card ms-disclosures__card--trust"
					type="button"
					onclick={() => openInfo('benefits')}
				>
					<img
						src={resolve('/assets/images/sell/kristian-trust-disclosure-list-v1.webp')}
						alt=""
						aria-hidden="true"
					/>
					<span class="ms-disclosures__copy">
						<img
							...
							aria-hidden="true"
						/>
						<strong>Защо Day Night</strong>
						<small>Кристиян и екипът Ви дават ясен следващ вариант.</small>
						<span class="ms-disclosures__cta">
							<span>Виж предимствата</span>
							<ChevronRight size={15} strokeWidth={2.6} aria-hidden="true" />
						</span>
					</span>
				</button>
				...
				<div class="ms-sheet__scroll">
					...
					{:else}
						<div class="ms-benefit-list">
							<article>
								<span class="ms-benefit-list__icon"><CircleCheck size={20} strokeWidth={2.35} /></span>
								<span
...
<style>
	...
	.ms-disclosures > button::after {
		...
		background:
			...
			linear-gradient(180deg, rgba(3, 5, 8, 0.04) 28%, rgba(3, 5, 8, 0.72) 100%);
		content: '';
	}

	.ms-disclosures > button > img {
		position: absolute;
		inset: 0;
		z-index: -2;
		width: 100%;
	...
	.ms-disclosures > button:active > img {
		transform: scale(1.025);
	}

	.ms-disclosures > button:focus-visible {
		outline: 2px solid var(--ms-blue);
		outline-offset: 2px;
	}

	.ms-disclosures__copy,
	.ms-step__copy,
	...
	.ms-disclosures__cta {
		...
		border-radius: 999px;
		background: #fff;
		box-shadow: 0 7px 18px rgba(3, 5, 8, 0.2);
		color: #090b0e;
		font-size: 12px;
		font-weight: 850;
		line-height: 1;
	...
	.ms-disclosures__cta :global(svg),
	.ms-disclosures__cta :global(svg *) {
