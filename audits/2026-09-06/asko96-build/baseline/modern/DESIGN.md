...
The owner's phone review supersedes the older masthead guidance below. All mobile masthead actions share one background-free style from `mobile-header-icon-action.ts`: 44px transparent targets with 24px Lucide icons and 1.75px strokes, matching opacity feedback and focus outlines. Use white on black/red headers and dark foreground on yellow/pale headers for contrast. This includes Home/Cars category and filters, the scrolling header, service info/call, Contact location/call, secondary pages, and loading shells. Keep the 144px logo. Cars uses the simple Lucide Car side-profile glyph, full filters use Funnel, and service info uses Info. Mobile pill rails retain a 16px inset and a 12px fade only on edges with hidden content. Active filters retain the red count badge. Dialog close/back controls retain their existing surfaces.

The leasing vehicle picker initially focuses its dialog so visitors can browse without opening the keyboard; tapping search starts text entry. The combined About/Contact page uses the shared inline mobile hero geometry on a black background, with the original white/red wordmark, centered white title, and matching background-free location/call controls. Its white rounded content surface contains grey telephone, showroom, and service-link cards with 8px gaps. Keep copy to labels and the address; do not reintroduce separator lines or repeated marketing paragraphs. Desktop retains its existing content layout.
...
The dealer dock has five destinations: Cars, Import, Sell, Lease, and Menu. Each target is 60px high with a 22px Lucide outline icon and a 12px/16px label. Labels never wrap. A faint neutral top border separates the white dock from the page. Active state uses a red icon with a stronger stroke and a semibold red label. Keep the icon background transparent; do not add a colored block or a top indicator line. Cars uses CarFront, Import uses Globe, Sell uses Tag, Lease uses CirclePercent and Menu uses Menu; secondary pages orient through Menu.
