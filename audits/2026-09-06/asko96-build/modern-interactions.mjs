...
for(const width of [390,1440]){
 ...
 const input=p.getByRole('searchbox').filter({visible:true}).first();r.searchInputs=await input.count();await input.fill('Toyota');await input.press('Enter');await p.waitForTimeout(1200);r.searchUrl=p.url();r.text=(await p.locator('body').innerText()).slice(0,2500);await p.screenshot({path:`${out}/modern-search-${width}.png`});
 ...
 }catch(e){r.failure=e.message}results.push(r);await p.close();await fs.writeFile(`${out}/modern-interactions.json`,JSON.stringify(results,null,2));console.log(JSON.stringify(r));
}
await browser.close();
