...
const measureMobileHeroArtwork = async (page) => page.evaluate(() => {
  const band = document.querySelector('.dn-hero-vehicles--mobile').getBoundingClientRect();
  const cars = [...document.querySelectorAll('.dn-hero-vehicles--mobile .dn-hero-vehicles__front')].map(image => {
    const bounds = image.getBoundingClientRect();
    const canvas = new OffscreenCanvas(image.naturalWidth, image.naturalHeight);
    const context = canvas.getContext('2d');
    context.drawImage(image, 0, 0);
    const pixels = context.getImageData(0, 0, canvas.width, canvas.height).data;
    let first = canvas.height, last = 0;
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        if (pixels[(y * canvas.width + x) * 4 + 3] > 128) { first = Math.min(first, y); last = Math.max(last, y); break; }
      }
    }
    const scale = Math.min(bounds.width / canvas.width, bounds.height / canvas.height);
    const imageTop = bounds.top + (bounds.height - canvas.height * scale) / 2;
    return { height: (last - first) * scale, top: imageTop + first * scale, bottom: imageTop + last * scale, left: bounds.left, right: bounds.right };
  });
  return { cars, valid: cars.length === 1 && cars[0].height >= 110 && cars[0].height <= 128 && cars[0].top >= band.top - 1 && cars[0].bottom <= band.bottom + 1 && Math.abs((cars[0].left + cars[0].right) / 2 - innerWidth / 2) <= 1 && [...document.querySelectorAll('.dn-hero-vehicles--mobile .dn-hero-vehicles__car')].every(car => getComputedStyle(car).display === 'none') };
});

const measureDesktopHeroArtwork = async (page) => page.evaluate(() => {
  const band = document.querySelector('.dn-hero-vehicles');
  const images = [...band.querySelectorAll('.dn-hero-vehicles__car img')];
  const loaded = images.length === 2 && images.every(image => image.complete && image.naturalWidth > 0 && image.dataset.view === 'side-profile');
  if (innerWidth < 1440) return { valid: loaded && getComputedStyle(band).display === 'none', compact: true };
  const centerWidth = Math.min(1040, innerWidth - 560);
  const expectedHeight = Math.min(148, Math.max(100, innerWidth * .1 - 44));
  const cars = images.map((image, index) => {
    const box = image.getBoundingClientRect();
    const canvas = new OffscreenCanvas(image.naturalWidth, image.naturalHeight);
    const context = canvas.getContext('2d'); context.drawImage(image, 0, 0);
    const pixels = context.getImageData(0, 0, canvas.width, canvas.height).data;
    let first = canvas.height, last = 0, front = canvas.width;
    for (let y = 0; y < canvas.height; y++) for (let x = 0; x < canvas.width; x++) {
      if (pixels[(y * canvas.width + x) * 4 + 3] > 128) { first = Math.min(first,y); last = Math.max(last,y); front = Math.min(front,x); }
    }
    const scale = box.height / canvas.height;
    return { vehicle: image.parentElement.parentElement.dataset.vehicle, height: (last + 1 - first) * scale, baseline: box.top + (last + 1) * scale, front: index === 0 ? box.right - front * scale : box.left + front * scale };
  });
  const route = location.pathname;
  const expected = route === '/' ? ['gclass','urus'] : route === '/listing-grid' ? ['golf','a45'] : route === '/about-us' ? ['porsche','amggt'] : route === '/blog' ? ['m5','e63'] : ['m4','rs5'];
  const valid = loaded && cars.every((car,index) => car.vehicle === expected[index] && Math.abs(car.height - expectedHeight) <= 1 && Math.abs(car.baseline - 480) <= 1) && Math.abs(cars[0].front - ((innerWidth-centerWidth)/2 - 24)) <= 1 && Math.abs(cars[1].front - ((innerWidth+centerWidth)/2 + 24)) <= 1;
  return { valid, cars };
});

const outputRoot = path.resolve('J:/cars/audits/2026-09-08/auto-best/evidence/sveltekit-smoke');
const routes = [
  ['home', '/'],
  ...
  return {
    ...
    homeSectionHeaders: [...document.querySelectorAll('.dn-home-section-heading')].map((element) => {
      ...
      return {
        ...
        stacked: element.classList.contains('dn-home-section-heading--banner'),
        actionBelowCopy: actionBox.top >= (element.querySelector('p')?.getBoundingClientRect().bottom ?? Infinity) + 7,
...
for (const [width, height] of viewports) {
  ...
  for (const [name, route] of routes) {
    ...
    if (settled.routeHero) {
      const artwork = await measureDesktopHeroArtwork(page);
      if (!artwork.valid) failures.push('Shared lower hero artwork drifted: ' + JSON.stringify(artwork));
      ...
      if (route === '/') {
        ...
        const studio = await page.locator('.dn-route-hero--studio').evaluate((hero) => {
          const art = [...hero.querySelectorAll('.dn-hero-vehicles__car img')];
          ...
          return { background: getComputedStyle(hero).backgroundColor, loaded: art.length === 2 && art.every(image => image.complete && image.naturalWidth > 0 && image.currentSrc.includes('/day-night-cutout-') && getComputedStyle(image).objectFit === 'contain'), overlap: !left || !right || left.right > copy.left + 1 || right.left < copy.right - 1, artBottom: Math.max(left?.bottom ?? 0, right?.bottom ?? 0), heroBottom: (hero.closest('.dn-listing-stage') ?? hero).getBoundingClientRect().bottom };
        });
        ...
        if (settled.routeHeroHeadingTextAlign !== 'center' || Math.abs(routeHeroHeadingCenter - width / 2) > 1 || Math.abs(routeHeroHeadingY - expectedRouteHeroHeadingY) > 1 || Math.abs(settled.routeHeroHeadingFontSize - expectedHeadingSize) > .1 || settled.routeHeroHeadingWeight !== 600 || studio.background !== expectedStudioBackground || !studio.loaded) failures.push('Centered studio hero layout or media drifted: '+JSON.stringify(studio));
    ...
    if (route === '/') {
      ...
      const bannerToBodyTypesGap = await page.evaluate(() => document.querySelector('#body-types-title').getBoundingClientRect().top - document.querySelector('[data-banner-group="browse"]').getBoundingClientRect().bottom);
      if (bannerToBodyTypesGap < 49 || bannerToBodyTypesGap > 52) {
        failures.push(`homepage body-type heading spacing after the campaign banners drifted (${bannerToBodyTypesGap}px)`);
      }
      ...
      const heroComposition = await page.locator('.dn-hero').evaluate(el => {
        const media = [...el.querySelectorAll('.dn-hero-vehicles__car img')];
        const copy = el.querySelector('.dn-route-hero__copy').getBoundingClientRect();
        const [left,right] = media.map(image=>image.getBoundingClientRect());
        return { background: getComputedStyle(el).backgroundColor, color: getComputedStyle(el.querySelector('h1')).color, loaded: media.length===2 && media.every(image=>image.complete && image.naturalWidth>0 && image.currentSrc.includes('/day-night-cutout-') && getComputedStyle(image).objectFit==='contain'), overlap: !left || !right || left.right>copy.left+1 || right.left<copy.right-1, oldMediaHidden: getComputedStyle(el.querySelector('.dn-hero__media')).display==='none' };
      });
      if (heroComposition.background !== 'rgb(16, 17, 20)' || heroComposition.color !== 'rgb(255, 255, 255)' || !heroComposition.loaded || !heroComposition.oldMediaHidden) failures.push(`homepage black cutout hero drifted (${JSON.stringify(heroComposition)})`);
      ...
      if (settled.homeSectionHeaders.length !== 6 || settled.homeSectionHeaders.some((header) => header.titleSize !== '32px' || Math.abs(header.actionHeight - 44) > 1 || header.actionBackground !== 'rgb(255, 255, 255)' || (header.stacked ? !header.actionBelowCopy : header.alignmentDelta > 1))) {
      ...
      for (const group of await page.locator('.dn-trust-actions__grid').all()) await group.scrollIntoViewIfNeeded();
      await page.waitForFunction(()=>[...document.querySelectorAll('.dn-trust-card__vehicle img')].every(image=>image.complete && image.naturalWidth>0));
      const trustArtwork = await page.locator('.dn-trust-card').evaluateAll(cards=>cards.map((card,index)=>{
        const image=card.querySelector('.dn-trust-card__vehicle img'), box=image.getBoundingClientRect(), copy=card.querySelector('.dn-trust-card__content').getBoundingClientRect(), bounds=card.getBoundingClientRect();
        if (index < 2) {
          ...
          for(let y=0;y<canvas.height;y++) for(let x=0;x<canvas.width;x++) if(pixels[(y*canvas.width+x)*4+3]>128){x0=Math.min(x0,x);y0=Math.min(y0,y);x1=Math.max(x1,x);y1=Math.max(y1,y);}
          const scale=box.width/canvas.width;
          const visible={left:box.left+x0*scale,right:box.left+(x1+1)*scale,top:box.top+y0*scale,bottom:box.top+(y1+1)*scale};
          const heading=card.querySelector('h3').getBoundingClientRect(), description=card.querySelector('p').getBoundingClientRect(), action=card.querySelector('.dn-trust-card__action').getBoundingClientRect();
          const clearCopy=[description,action].every(rect=>rect.right+8<=visible.left || rect.bottom+4<=visible.top);
          return {contain:getComputedStyle(image).objectFit==='contain',loaded:image.complete && image.naturalWidth>0 && image.dataset.view===(index===0?'front-pair':'three-quarter') && image.currentSrc.includes(index===0?'/day-night-collection-banner-v2.webp':'/day-night-sell-banner-v2.webp'),clear:visible.right<=bounds.right-15 && visible.left>=bounds.left+15 && visible.top>=heading.bottom+8 && Math.abs(visible.bottom-bounds.top-206)<=1 && clearCopy,opticalHeight:visible.bottom-visible.top,y:bounds.y};
        }
        return {contain:getComputedStyle(image).objectFit==='contain', loaded:image.complete && image.naturalWidth>0 && image.dataset.view==='side-profile', clear:box.left>=copy.right && getComputedStyle(card).overflow==='hidden' && box.right>bounds.right && box.right-bounds.right<=box.width*.25 && box.bottom<=bounds.bottom+4, y:bounds.y};
      }));
      if(trustArtwork.length!==4 || trustArtwork.some(art=>!art.contain || !art.loaded || !art.clear) || Math.abs(trustArtwork[0].opticalHeight-trustArtwork[1].opticalHeight)>1 || trustArtwork[0]?.y!==trustArtwork[1]?.y || trustArtwork[2]?.y!==trustArtwork[3]?.y || trustArtwork[2]?.y<=trustArtwork[0]?.y) failures.push(`Browse artwork must match optical height/baseline, stay fully visible and clear of text; lower service artwork keeps edge framing: ${JSON.stringify(trustArtwork)}`);
      const trustDestinations = await page.locator('.dn-trust-actions .dn-trust-card__action').evaluateAll(links=>links.map(link=>link.getAttribute('href')));
      const bannerOrder = await page.evaluate(()=>{
        const box=selector=>document.querySelector(selector).getBoundingClientRect();
        const sections=['.dn-hero','[data-banner-group="browse"]','.dn-body-types','.dn-inventory','.dn-brand-section','.dn-editorial','[data-banner-group="ownership"]','.dn-videos','.dn-home-services'].map(box);
        return sections.every((section,index)=>index===0 || sections[index-1].bottom<=section.top+1);
      });
      if(trustDestinations.join('|')!=='/listing-grid|/contact?topic=trade-in|/contact?topic=import|/contact?topic=leasing' || !bannerOrder || await page.locator('.dn-trust-actions').count()!==2 || await page.locator('.dn-trust-actions__heading').count()!==0) failures.push('Homepage banners must form two separate pairs in the browsing journey');
      const serviceCards = await page.locator('.dn-home-services .dn-service-card').evaluateAll(cards=>cards.map(card=>({background:getComputedStyle(card).backgroundColor,icon:card.querySelector('svg')?.innerHTML,href:card.querySelector('.dn-trust-card__action').getAttribute('href'),actionHeight:card.querySelector('.dn-trust-card__action').getBoundingClientRect().height})));
      if(serviceCards.length!==4 || new Set(serviceCards.map(card=>card.icon)).size!==4 || serviceCards.some(card=>card.background!=='rgb(255, 255, 255)' || card.actionHeight<44) || serviceCards.map(card=>card.href).join('|')!==trustDestinations.join('|')) failures.push('Homepage must retain four white service cards with distinct icons and working destinations');
      ...
      if (trustCardRhythm.some((card) => card.headingGap !== 12 || card.descriptionGap !== 20 || Math.abs(card.descriptionLines - 2) > .01 || card.cardHeight < 234 || card.cardHeight > 300)) {
      ...
      const trustPanelStyles = await page.locator('.dn-trust-actions__panel').first().evaluate((element) => {
        const styles = getComputedStyle(element);
      ...
      if (editorialStyles.background !== 'rgb(255, 255, 255)' || editorialBannerStyles.image !== 'none' || Math.abs(editorialBannerStyles.radius - 20) > 1 || Math.abs(editorialBannerStyles.padding - 32) > 1 || editorialBannerStyles.height < 234 || editorialBannerStyles.height > 280 || brandHeroHeight < 234 || brandHeroHeight > 280 || editorialHeadingColor !== 'rgb(255, 255, 255)' || editorialIntro?.trim() !== 'Практични насоки за оглед, покупка и внос на автомобил.' || editorialCtaText?.trim() !== 'Вижте всички статии' || editorialCtaHref !== '/blog' || !editorialCtaVisible || Math.abs(editorialCardsOffset + 24) > 1 || Math.abs(editorialCardsWidth - brandPanelWidth) > 1 || editorialCardStyles.background !== 'rgb(255, 255, 255)' || editorialCardStyles.border !== 0 || Math.abs(editorialCardStyles.radius - 16) > 1 || trustPanelStyles.background !== 'rgba(0, 0, 0, 0)' || trustPanelStyles.radius !== 0 || trustPanelStyles.padding !== 0 || Math.abs(trustPanelStyles.width - brandPanelWidth) > 1 || trustCardStyles.background !== 'rgb(16, 17, 20)' || trustCardStyles.border !== 0 || Math.abs(trustCardStyles.radius - 16) > 1) {
    ...
    if (route === '/about-us') {
      ...
      const aboutStructure = await page.locator('.dn-about-process').evaluate(section=>{
        const hero=document.querySelector('.dn-about-hero').getBoundingClientRect(), bounds=section.getBoundingClientRect(), intro=section.querySelector('.dn-about-section-heading').getBoundingClientRect();
        const cards=[...section.querySelectorAll('.dn-about-service-card')];
        return {separate:bounds.top>=hero.bottom, cardsBelow:cards.every(card=>card.getBoundingClientRect().top>=intro.bottom+24), readable:cards.every(card=>{const box=card.getBoundingClientRect(),description=card.querySelector('p').getBoundingClientRect(),action=card.querySelector('a').getBoundingClientRect();return getComputedStyle(card).backgroundColor==='rgb(255, 255, 255)'&&box.height<=300&&action.height>=44&&action.top>=description.bottom&&action.bottom<=box.bottom;}), count:cards.length};
      });
      if(!aboutStructure.separate || !aboutStructure.cardsBelow || !aboutStructure.readable || aboutStructure.count!==4 || !await page.locator('.dn-about-intro__action').isVisible() || await page.locator('.dn-about-hero-panel').count()) failures.push('About needs a separate introduction and four readable white service cards: '+JSON.stringify(aboutStructure));
    ...
    if (route.startsWith('/blog?')) {
      ...
      const blogToolbarGeometry = await page.evaluate(() => {
        ...
        return {
          ...
          searchPanel: box('.dn-blog-search'),
          toolbarBackground: getComputedStyle(document.querySelector('.dn-blog-toolbar')).backgroundColor,
          submitCount: document.querySelectorAll('.dn-blog-toolbar button[type="submit"], .dn-blog-toolbar input[type="submit"]').length
        };
      });
      if (!blogToolbarGeometry.search || !blogToolbarGeometry.categories || !blogToolbarGeometry.searchPanel || blogToolbarGeometry.searchPanel.bottom + 16 > blogToolbarGeometry.categories.y || blogToolbarGeometry.toolbarBackground !== 'rgba(0, 0, 0, 0)' || blogToolbarGeometry.submitCount !== 0) {
        failures.push(`blog category pills must sit outside and below the white search panel (${JSON.stringify(blogToolbarGeometry)})`);
      }
      if (width === 1440 && (Math.abs((blogToolbarGeometry.toolbar?.width ?? 0) - 880) > 1 || Math.abs((blogToolbarGeometry.toolbar?.height ?? 0) - 152) > 1)) {
...
for (const [name, route] of mobileHeroRoutes) {
  ...
  const geometry = await page.evaluate(() => {
    ...
    return {
      ...
      hero: box(hero),
      mobileCars: [...(hero?.querySelectorAll('.dn-hero-vehicles--mobile .dn-hero-vehicles__front') ?? [])].map((image) => ({ ...box(image), loaded: image.complete && image.naturalWidth > 0, src: image.currentSrc })),
      mobileArtwork: box(hero?.querySelector('.dn-hero-vehicles--mobile')),
      mobileArtworkClipped: hero?.querySelector('.dn-hero-vehicles--mobile') ? getComputedStyle(hero.querySelector('.dn-hero-vehicles--mobile')).overflow === 'hidden' : false,
      heading: box(heading),
      lead: box(lead),
      control: box(control),
  ...
  const failures = [];
  if (!ready) failures.push('route readiness state did not settle');
  if (route === '/' && (!geometry.mobileArtworkClipped || !geometry.mobileArtwork || geometry.mobileArtwork.y < geometry.header.y + geometry.header.height || geometry.mobileArtwork.y + geometry.mobileArtwork.height > geometry.control.y || geometry.mobileCars.length !== 1 || geometry.mobileCars.some(car => !car.loaded || !car.src.includes('/day-night-urus-front-v1.webp')))) failures.push('mobile Home must show one loaded front-facing car between the header and search');
  if (route === '/') {
    const optical = await measureMobileHeroArtwork(page);
    if (!optical.valid) failures.push('mobile hero must show one centered front-facing car within its artwork band: ' + JSON.stringify(optical));
  }
  ...
  const compactExpectation = route === '/'
    ? { heroHeight: 252, controlY: 200, overlay: 'rgba(0, 0, 0, 0)' }
    : null;
  ...
  const sharedHeroDrifted = !compactExpectation && route !== '/listing-grid' && route !== '/contact' && (
    ...
    !geometry.lead || controlY === null || controlY < leadY + geometry.lead.height + (route === '/about-us' ? 12 : 16) ||
    !geometry.control || controlY + geometry.control.height > geometry.hero.height - 16 ||
    headingCenter === null || Math.abs(headingCenter - 195) > 1 ||
    geometry.headingAlign !== (route === '/about-us' ? 'center' : 'left') ||
  ...
  if (route === '/') {
    for (const width of [320, 390, 700, 767]) {
      ...
      const mobileDiscovery = await page.evaluate(() => {
        ...
        const tileSelectors = ['.dn-mobile-budget-card', '.dn-body-type', '.dn-vehicle-card', '.dn-brand-card', '.dn-editorial-item', '.dn-service-card'];
        ...
        return {
          ...
          campaigns: [...document.querySelectorAll('.dn-trust-card')].map(card => ({ background: getComputedStyle(card).backgroundColor, iconVisible: card.querySelector('.dn-trust-card__icon').getBoundingClientRect().width === 28, artworkHidden: card.querySelector('.dn-trust-card__vehicle img')?.getBoundingClientRect().width === 0, actionHeight: card.querySelector('.dn-trust-card__action').getBoundingClientRect().height })),
      ...
      if (
        ...
        mobileDiscovery.campaigns.length !== 4 || mobileDiscovery.campaigns.some(card => card.background !== 'rgb(255, 255, 255)' || !card.iconVisible || !card.artworkHidden || card.actionHeight < 44) ||
  ...
  for (const width of [320, 360, 390, 430]) {
    ...
    if (['trade-in', 'import'].includes(topic)) {
      const card = await page.evaluate(() => {
        ...
        const header = document.querySelector('.dn-header').getBoundingClientRect();
        const cars = [...document.querySelectorAll('.dn-hero-vehicles--mobile .dn-hero-vehicles__front')];
        const artwork = document.querySelector('.dn-hero-vehicles--mobile');
        const band = artwork.getBoundingClientRect();
        return { heroHeight: hero.height, overlap: hero.bottom - bounds.top, left: bounds.left, right: bounds.right,
          artworkVisible: header.height === 66 && getComputedStyle(artwork).overflow === 'hidden' && band.top >= header.bottom && band.bottom <= bounds.top && cars.length === 1 && cars.every(image => image.complete && image.naturalWidth > 0 && image.currentSrc.includes('/day-night-urus-front-v1.webp')),
      ...
      const optical = await measureMobileHeroArtwork(page);
      if (!optical.valid) failures.push('service hero must show one centered front-facing car within its artwork band: ' + JSON.stringify(optical));
      if (Math.abs(card.heroHeight - 252) > 1 || Math.abs(card.overlap - 52) > 1 || card.left < 11 || card.right > card.width - 11 || card.background !== 'rgb(255, 255, 255)' || !card.titleInside || !card.artworkVisible) {
