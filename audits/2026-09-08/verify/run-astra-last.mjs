...
const exec=promisify(execFile),runtimeDir=import.meta.dirname,out=process.env.QA_OUTPUT||runtimeDir;
...
if(process.argv[2]==='legend-auto'&&!process.env.QA_SKIP_PREFLIGHT)await exec('powershell.exe',['-NoProfile','-File','J:/cars/clients/legend-auto/qa-final/retry-modern.ps1'],{timeout:600000});
if(process.argv[2]==='automarket-varna'&&!process.env.QA_SKIP_PREFLIGHT)await exec(process.execPath,['J:/cars/audits/2026-09-08/verify/gallery-astra-last.mjs'],{timeout:240000});
...
async function nav(p,url){const r=await p.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await p.waitForTimeout(900);await p.waitForFunction(()=>[...document.querySelectorAll('main,header,h1,h2')].some(e=>{const r=e.getBoundingClientRect();return r.width>40&&r.height>15&&r.top<innerHeight&&r.bottom>0&&e.checkVisibility({checkOpacity:true,checkVisibilityCSS:true})&&e.innerText.trim().length>3}),{},{timeout:20000});return r;}
async function shot(p,file){await p.waitForTimeout(700);await p.screenshot({path:file,timeout:12000,animations:'disabled'}).catch(()=>{});}
...
async function metrics(p){
 await p.evaluate(async()=>{for(let y=0;y<Math.min(document.body.scrollHeight,14000);y+=800){scrollTo({top:y,behavior:"instant"});await new Promise(r=>setTimeout(r,40));}scrollTo({top:0,behavior:"instant"})});
 
 await p.waitForFunction(()=>[...document.images].filter(i=>i.getBoundingClientRect().width>0).every(i=>i.complete),{},{timeout:8000}).catch(()=>{});
 return p.evaluate(()=>({bodyClass:document.body.className,landmarks:[...document.querySelectorAll('main,header,.mobile-home,.daynight-home-shell')].map(e=>({tag:e.tagName,class:e.className,display:getComputedStyle(e).display,visibility:getComputedStyle(e).visibility,opacity:getComputedStyle(e).opacity,rect:{x:e.getBoundingClientRect().x,y:e.getBoundingClientRect().y,width:e.getBoundingClientRect().width,height:e.getBoundingClientRect().height}})),visibleContent:[...document.querySelectorAll('h1,h2,header,main')].some(e=>{const r=e.getBoundingClientRect();return r.width>40&&r.height>15&&r.top<innerHeight&&r.bottom>0&&e.checkVisibility({checkOpacity:true,checkVisibilityCSS:true})&&e.innerText.trim().length>3}),title:document.title,text:document.body.innerText.slice(0,22000),overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,broken:[...document.images].filter(i=>i.getBoundingClientRect().width>0&&(!i.complete||!i.naturalWidth)).map(i=>i.currentSrc||i.src),phones:[...new Set([...document.querySelectorAll('a[href^="tel:"]')].map(a=>a.getAttribute('href')))],links:[...document.querySelectorAll('a[href]')].filter(a=>a.getBoundingClientRect().width>0).map(a=>({text:a.innerText,href:a.getAttribute('href')})),overlay:!!document.querySelector('vite-error-overlay,[data-nextjs-dialog]')}));
}
async function interaction(p,base,t,width,r){
 ...
 let brand=(await p.locator(detailSelector(t)).filter({visible:true}).first().innerText()).match(/\b(Mercedes|BMW|Audi|VW|Volkswagen|Subaru|Toyota|Honda|Ford|Opel|Skoda|Cupra|Nissan|Peugeot|Renault|Kia|Hyundai|Mazda|Volvo|Seat|Porsche|Land Rover)\b/i)?.[1]||((await p.locator(detailSelector(t)).first().getAttribute('aria-label'))?.match(/Mercedes|BMW|Audi|Subaru|Toyota|Honda|Ford|Volvo/i)?.[0])||'Subaru';
 ...
 await p.waitForTimeout(750);if(t==='modern')await p.waitForURL(/[?&]q=/,{timeout:15000});r.searchUrl=p.url();r.afterCount=await p.locator(detailSelector(t)).filter({visible:true}).count();r.search=(/[?&](q|make|brand)=/.test(p.url())||r.afterCount<r.beforeCount)&&r.afterCount>0;
 const link=p.locator(detailSelector(t)).filter({visible:true}).first();r.detailHref=await link.getAttribute('href');await link.click();await p.waitForURL(/listing-detail-v1|\/listing\/|\/inventory\//,{timeout:15000});await p.waitForTimeout(900);r.navigation=/listing-detail-v1|\/listing\/|\/inventory\//.test(p.url());
 r.detailPhone=await p.locator('a[href^="tel:"]').count()>0;
 if(t==='modern'){const open=p.getByRole('button',{name:/^Отвори снимка|^Open photo/}).filter({visible:true}).first();if(await open.count())await open.click();}
 const next=p.getByRole('button',{name:/^Следваща снимка$|^Next image$/}).filter({visible:true}).first();
 if(await next.count()){
  const gallery=p.locator(t==='auto-best'?'.dn-detail-gallery > img':t==='modern'?'[data-slot=listing-gallery-lightbox] img':'.swiper-listing-details-main .swiper-slide-active img.img-main').filter({visible:true});const prev=await gallery.evaluateAll(es=>es.map(e=>e.currentSrc||e.src));await next.click();await p.waitForTimeout(500);const after=await gallery.evaluateAll(es=>es.map(e=>e.currentSrc||e.src));r.galleryChanged=JSON.stringify(prev)!==JSON.stringify(after);
 }
 if(t==='modern'){await p.keyboard.press('Escape');await p.waitForTimeout(300);}
...
for(const job of jobs){for(const [t,port]of [['auto-best',job.ports.autoBest],['modern',job.ports.modern],['carwow',job.ports.carwow]]){
 if(process.argv[2]&&job.slug!==process.argv[2])continue;if(process.env.QA_ONLY_TEMPLATE&&t!==process.env.QA_ONLY_TEMPLATE)continue;
 ...
 try{
  ...
  let detail=null; if(t==='carwow'){ const warm=await browser.newPage({viewport:{width:390,height:900}}); await nav(warm,base+'/'); await warm.waitForTimeout(1800); await warm.reload({waitUntil:'networkidle'}); await nav(warm,base+'/inventory'); await warm.waitForTimeout(1800); await warm.reload({waitUntil:'networkidle'}); await warm.close(); }
  for(const width of [390,1440]){
   ...
   const routeList=[t==='modern'?'/cars':'/',inventory,'DETAIL',contact,t==='auto-best'?'/about-us':t==='modern'?'/bg/imports':'/about'];
   if(t==='carwow')routeList.push('/financing'); for(const [i,key]of [...new Set(routeList)].entries()){
    ...
    try{
     ...
     row.pass=row.visibleContent&&row.status===200&&row.overflow<=0&&!row.broken.length&&!row.errors.length&&!row.overlay&&!row.staleIdentity.length;
...
if(process.argv[2]==='automarket-varna'&&!process.env.QA_SKIP_PREFLIGHT)await exec(process.execPath,[import.meta.filename,'legend-auto'],{timeout:600000,env:{...process.env,QA_OUTPUT:'J:/cars/clients/legend-auto/qa-final',QA_SKIP_PREFLIGHT:'1'}});

if(process.argv[2]==='automarket-varna'&&!process.env.QA_SKIP_PREFLIGHT)await exec(process.execPath,[import.meta.filename,'elit-auto-import'],{timeout:600000,env:{...process.env,QA_OUTPUT:'J:/cars/clients/elit-auto-import/qa-final',QA_SKIP_PREFLIGHT:'1'}});

if(process.argv[2]==='automarket-varna'&&!process.env.QA_SKIP_PREFLIGHT)await exec(process.execPath,['J:/cars/audits/2026-09-08/verify/console-astra-last.mjs'],{timeout:180000});
...
if(process.argv[2]==='automarket-varna'&&process.env.QA_SKIP_PREFLIGHT&&process.env.QA_ONLY_TEMPLATE==='modern')await exec('powershell.exe',['-NoProfile','-File','J:/cars/clients/legend-auto/qa-final/check-contact-copy.ps1'],{timeout:300000});
