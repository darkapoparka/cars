...
async function nav(p,url){const r=await p.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await p.locator('main,header,h1').filter({visible:true}).first().waitFor({state:'visible',timeout:20000});await p.waitForTimeout(900);return r;}
async function shot(p,file){await p.screenshot({path:file,timeout:12000,animations:'disabled'}).catch(()=>{});}
...
async function metrics(p){
 ...
 await p.waitForFunction(()=>[...document.images].filter(i=>{const r=i.getBoundingClientRect();return r.width>0&&r.right>0&&r.left<innerWidth&&r.bottom>0&&r.top<innerHeight}).every(i=>i.complete),{},{timeout:8000}).catch(()=>{});
 await p.evaluate(()=>scrollTo({top:0,behavior:'instant'}));await p.waitForTimeout(600);
 return p.evaluate(()=>({title:document.title,text:document.body.innerText.slice(0,22000),overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,broken:[...document.images].filter(i=>i.getBoundingClientRect().width>0&&((i.complete&&!i.naturalWidth)||(!i.complete&&i.getBoundingClientRect().left<innerWidth&&i.getBoundingClientRect().right>0&&i.getBoundingClientRect().top<innerHeight&&i.getBoundingClientRect().bottom>0))).map(i=>i.currentSrc||i.src),phones:[...new Set([...document.querySelectorAll('a[href^="tel:"]')].map(a=>a.getAttribute('href')))],links:[...document.querySelectorAll('a[href]')].filter(a=>a.getBoundingClientRect().width>0).map(a=>({text:a.innerText,href:a.getAttribute('href')})),overlay:!!document.querySelector('vite-error-overlay,[data-nextjs-dialog]')}));
}
async function interaction(p,base,t,width,r){
 ...
 if(t!=='carwow')await p.waitForURL(url=>/[?&](q|make|brand)=/.test(url.search),{timeout:20000});
 await p.waitForTimeout(750);r.searchUrl=p.url();r.afterCount=await p.locator(detailSelector(t)).filter({visible:true}).evaluateAll(es=>new Set(es.map(e=>e.getAttribute('href'))).size);r.search=(/[?&](q|make|brand)=/.test(p.url())||r.afterCount<r.beforeCount)&&r.afterCount>0;
 const link=p.locator(detailSelector(t)).filter({visible:true}).first();r.detailHref=await link.getAttribute('href');await link.click();await p.waitForURL(url=>/listing-detail-v1|\/listing\/|\/inventory\//.test(url.pathname),{timeout:20000});await p.locator('main,header,h1').filter({visible:true}).first().waitFor();r.navigation=/listing-detail-v1|\/listing\/|\/inventory\//.test(p.url());
 ...
 r.pass=r.search&&r.navigation&&r.enquiry&&r.detailPhone&&r.menuOpened!==false&&r.menuDismissed!==false&&r.menuFocus!==false&&r.filterOpened!==false&&r.filterDismissed!==false&&r.galleryChanged!==false;
}
try{
