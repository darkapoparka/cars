<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			...
			value: 'Демо форма; за контакт се обадете',
			icon: MessageCircle
		},
		{
			id: 'address',
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Работно време: по телефона',
			href: null,
			icon: Clock
		}
	] as const;
	...
	const importSteps = [
		{
			...
			copy: 'Изпратете обявата при разговор, за да уточните възможностите.'
		},
		{
		...
		},
		{
			...
			copy: 'Попитайте дали автокъщата предлага внос за конкретния автомобил.'
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		...
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Приселци начало">
					<img src={resolve('/assets/priselci/wordmark.svg')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Приселци'}
			</h1>
			<p>
				{isImportMode
