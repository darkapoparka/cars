import stock from './priselci-stock.json';
...
const listingToVehicle = (listing: CurrentDayNightListing): Car => {
	const original = stock.find(v => v.id === listing.id)!;
	...
	const conditionLine = isIncoming
		...
		: 'Обява от 07.09.2026 — потвърдете наличността и огледа.';
