...
const daynightFacebookUrl = 'https://priselci.mobile.bg/';
const daynightInstagramUrl = 'https://priselci.mobile.bg/';
...
function renderFooterSocialLinks() {
	...
	const links: readonly PresentationIconLink[] = [
		...
		},
		{
			...
			title: 'Отвори локацията на Приселци в Google Maps',
			kind: 'external',
...
<p class="text-sm text-muted mb-16">Приселци предлага актуална дигитална витрина с наличност, запитвания и ясни следващи стъпки.</p>
${renderFooterSocialLinks()}
</div>
<div>
<p class="text-sm text-white font-weight-600 mb-8">Имате въпрос?</p>
<div class="flex items-center gap-12">
<a href="/contact" class="btn btn-line-1 btn-small" title="Свържете се със Приселци">Свържете се</a>
</div>
</div>
</div>
</div>
...
<p class="text-sm text-muted">©2026 <a class="text-sm text-white" href="/">Приселци</a>. Всички права запазени.</p>
...
<a href="tel:${daynightSite.phone}" class="icon" aria-label="Обади се на Приселци">${phoneIcon()}</a>
...
<a href="${daynightSite.mapUrl}" class="icon" target="_blank" rel="noopener" aria-label="Отвори карта до Приселци">${locationIcon()}</a>
<div class="daynight-header-contact__body">
...
<a class="daynight-map-preview" href="${daynightSite.mapUrl}" target="_blank" rel="noopener" aria-label="Отвори Google карта до Приселци">
<span class="daynight-map-preview__thumb">
<span class="daynight-map-preview__pin"></span>
</span>
<span class="daynight-map-preview__body">
<strong>Приселци Варна</strong>
