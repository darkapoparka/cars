<script lang="ts">
  ...
  const budgetTiles = [
    {
      label: 'До 4 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur <= 4000).length} автомобила`,
      href: '/listing-grid?price_max=4000',
      image: featuredVehicles[1].image
    },
    {
      label: '4–5 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 4000 && vehicle.priceEur <= 5000).length} автомобила`,
      href: '/listing-grid?price_min=4000&price_max=5000',
      image: featuredVehicles[0].image
    },
    {
      label: 'Над 5 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 5000).length} автомобила`,
      href: '/listing-grid?price_min=5000',
