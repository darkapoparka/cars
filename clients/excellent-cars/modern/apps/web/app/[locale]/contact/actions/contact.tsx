...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Excellent Cars.",
          "The form is ready, but message delivery is not configured yet. Call Excellent Cars directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Excellent Cars.",
          "Message delivery is not configured yet. Call Excellent Cars directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Excellent Cars."
        : "Запитването е изпратено до екипа на Excellent Cars.",
      "Your request has been sent to the Excellent Cars team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Excellent Cars: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
