<script lang="ts">
	...
	const dealerJsonLd = $derived(
		JSON.stringify({
			...
			image: `${origin}/assets/excellent/logo-header.png`,
			logo: `${origin}/assets/excellent/logo-header.png`,
			url: `${origin}/`,
			telephone: '0895996559',
			...(daynightSite.email ? { email: daynightSite.email } : {}),
			address: {
				'@type': 'PostalAddress',
				streetAddress: 'гр. Варна, Възраждане 1, бул. Ян Хунияди / Цар Освободител',
