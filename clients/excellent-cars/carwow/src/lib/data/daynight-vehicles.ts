import excellentStock from './excellent-stock.json';
...
const listingToVehicle = (listing: CurrentDayNightListing): Car => {
	const original = excellentStock.find((vehicle) => vehicle.id === listing.id)!;
	...
	const conditionLine = isIncoming
		...
		: 'Публикувана обява — потвърдете наличността с Excellent Cars.';
