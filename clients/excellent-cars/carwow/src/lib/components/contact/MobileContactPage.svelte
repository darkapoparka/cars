<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Пон–Пет: 08:30–19:00 · Съб: 09:00–18:00 · Нед: почивен ден',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/excellent/vehicle-01-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Excellent Cars начало">
					<img src={resolve('/assets/excellent/logo-header.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Excellent Cars'}
			</h1>
			<p>
				{isImportMode
	...
	<main id="main-content" tabindex="-1" class={{ 'mobile-contact-main--sheet': isImportMode }}>
		...
		<section class="mobile-contact-map" aria-label="Карта">
			...
			<iframe
				...
				title="Карта до Excellent Cars Варна"
