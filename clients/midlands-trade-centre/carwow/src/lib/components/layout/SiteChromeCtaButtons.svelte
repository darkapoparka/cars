<script lang="ts">
	...
	let { mobile = false, compact = false }: { mobile?: boolean; compact?: boolean } = $props();
</script>

<div
	class={[
		'ml-auto flex items-center gap-5',
		mobile ? 'flex-wrap justify-center' : 'justify-end max-[1180px]:gap-3'
	]}
>
	<a
	...
	>
		{#if mobile}
			{compact ? 'Enquire' : daynightSite.sellCarCta}
		{:else}
