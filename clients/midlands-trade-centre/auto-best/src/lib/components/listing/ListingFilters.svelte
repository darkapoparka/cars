<script lang="ts">
  ...
  const activeChips = $derived.by(() => {
    const labels: Record<string, string> = {
      ...
      price_min: `From ${filters.priceMin?.toLocaleString('en-GB')} £`,
      price_max: `Up to ${filters.priceMax?.toLocaleString('en-GB')} £`,
      year_min: `From ${filters.yearMin}`,
      year_max: `Up to ${filters.yearMax}`,
      mileage_max: `Up to ${filters.mileageMax?.toLocaleString('en-GB')} miles`
    };
