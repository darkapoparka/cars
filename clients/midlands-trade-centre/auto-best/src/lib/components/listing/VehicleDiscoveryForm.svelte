<script lang="ts">
  ...
  const number = (value: string) => Number(value).toLocaleString('en-GB');
...
<form id="dn-desktop-discovery" class="dn-discovery" {@attach observePanel} method="GET" action={resolve('/listing-grid')} oninput={updateDraft} onchange={updateDraft} onformdata={clean}>
  ...
  <div class="dn-discovery__facets">
    ...
    <label><span>Mileage up to</span><select name="mileage_max" value={filters.mileageMax?.toString() ?? ''}>{#each mileages as value (value)}<option {value}>{value ? `${number(value)} miles` : 'No limit'}</option>{/each}</select></label>
  </div>
