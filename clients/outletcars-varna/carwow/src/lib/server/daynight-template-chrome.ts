...
const inventoryMegaMenuVehicles: readonly MegaMenuVehicleTile[] = daynightVehicles.slice(0, 4).map(vehicle => ({ label: vehicle.shortTitle, href: `/inventory/${vehicle.slug}`, image: vehicle.image, meta: `${vehicle.priceEur} · ${vehicle.fuel}` }));
...
function renderFooterSocialLinks() {
	...
	const links: readonly PresentationIconLink[] = [
		...
		},
		{
			...
			title: 'Отвори локацията на OUTLETCARS.BG — Варна в Google Maps',
			kind: 'external',
	...
	return `<ul class="widget-socical mb-12">${links
		.filter((link) => Boolean(link.href)).map((link) => renderIconLink(link, 'effect-svg-hover'))
		.join('')}</ul>`;
}
...
<p class="text-sm text-muted mb-16">OUTLETCARS.BG — Варна предлага актуална дигитална витрина с наличност, запитвания и ясни следващи стъпки.</p>
${renderFooterSocialLinks()}
</div>
<div>
<p class="text-sm text-white font-weight-600 mb-8">Имате въпрос?</p>
<div class="flex items-center gap-12">
<a href="/contact" class="btn btn-line-1 btn-small" title="Свържете се със OUTLETCARS.BG — Варна">Свържете се</a>
</div>
</div>
</div>
</div>
...
<p class="text-sm text-muted">©2026 <a class="text-sm text-white" href="/">OUTLETCARS.BG — Варна</a>. Всички права запазени.</p>
...
<a href="${daynightSite.mapUrl}" class="icon" target="_blank" rel="noopener" aria-label="Отвори карта до OUTLETCARS.BG — Варна">${locationIcon()}</a>
<div class="daynight-header-contact__body">
...
<a class="daynight-map-preview" href="${daynightSite.mapUrl}" target="_blank" rel="noopener" aria-label="Отвори Google карта до OUTLETCARS.BG — Варна">
