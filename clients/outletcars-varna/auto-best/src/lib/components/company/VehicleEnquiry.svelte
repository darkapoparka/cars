...
<div class="dn-enquiry-entry">
  ...
  {:else}
    ...
    <button class="dn-enquiry-alternative dn-action--enquiry" type="button" onclick={(event) => open(event, true)} aria-haspopup="dialog">Нямам обява — опиши търсенето <Icon name="arrow-right" size={17} /></button>
  {/if}
</div>
