<script lang="ts">
  ...
  const budgetTiles = [
    {
      label: 'До 20 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur <= 20000).length} автомобила`,
      href: '/listing-grid?price_max=20000',
      image: featuredVehicles.find(vehicle => vehicle.priceEur <= 20000)?.image ?? '/assets/images/icon-box/car-list2.png'
    },
    {
      label: '20–30 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 20000 && vehicle.priceEur <= 30000).length} автомобила`,
      href: '/listing-grid?price_min=20000&price_max=30000',
      image: featuredVehicles.find(vehicle => vehicle.priceEur > 20000 && vehicle.priceEur <= 30000)?.image ?? '/assets/images/icon-box/car-list1.png'
    },
    {
      label: 'Над 30 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 30000).length} автомобила`,
      href: '/listing-grid?price_min=30000',
      image: featuredVehicles.find(vehicle => vehicle.priceEur > 30000)?.image ?? '/assets/images/icon-box/car-list4.png'
    },
    {
      label: 'Виж всички',
