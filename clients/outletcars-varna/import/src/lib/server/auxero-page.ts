	...
	const daynightEarlyFormConfig = (form) => {
		if (form.matches('.daynight-contact-form')) {
			return {
				...
				status: 'Съобщението е подготвено локално за OUTLETCARS.BG',
				url: '/api/inquiries'
			};
		}
		...
		if (form.matches('.daynight-blog-comment-form')) {
			return {
				...
				status: 'Коментарът е запазен локално за преглед от OUTLETCARS.BG',
				url: '/api/messages'
			};
		}
		...
		if (form.matches('.daynight-sell-form')) {
			return {
				...
				status: 'Заявката е подготвена. OUTLETCARS.BG ще се свърже с вас.',
		...
		if (form.matches('.daynight-service-form')) {
			return {
				...
				status: 'Заявката за услуга е подготвена локално за OUTLETCARS.BG',
				url: '/api/inquiries'
			};
		}
