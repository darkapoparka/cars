<script lang="ts">
  ...
  const photos = featuredVehicles.slice(0, 3);
</script>
<section class="dn-videos" aria-labelledby="videos-title">
  <div class="container"><div class="dn-videos__panel">
    <div class="dn-videos__heading dn-home-section-heading">
      <h2 id="videos-title" class="dn-home-section-title">Автомобилите отблизо</h2>
      <p>Снимки от обявите към 09.09.2026 г. За актуално видео попитайте продавача.</p>
      <a class="dn-videos__channel dn-home-section-action" href={resolve('/listing-grid')}>Всички обяви<Icon name="arrow-right" size={16} /></a>
    </div>
    <div class="dn-videos__grid">{#each photos as vehicle (vehicle.id)}
      <article class="dn-video-card"><div class="dn-video-card__media"><a class="dn-video-card__play" href={resolve(vehicle.href as '/listing-detail-v1/1')} aria-label={vehicle.title}><img src={vehicle.gallery[1] ?? vehicle.image} alt={vehicle.title} width="720" height="404" loading="lazy" /></a></div>
        <div class="dn-video-card__content"><h3>{vehicle.title}</h3><div class="dn-video-card__actions"><a href={resolve(vehicle.href as '/listing-detail-v1/1')}>Вижте снимките<Icon name="arrow-right" size={16} /></a></div></div>
      </article>
    {/each}</div>
  </div></div>
</section>
