...
describe('native Carwow locale adapter', () => {
	...
	it('keeps route identity and dealer facts independent', () => {
		...
		expect(dealerLabel('en', 'city')).toBe('Пловдив');
		expect(dealerLabel('bg', 'city')).toBe('Пловдив');
		expect(dealerLabel('en', 'address')).toBe('18 Atanas Manchev Street, Studentski Grad, Пловдив');
