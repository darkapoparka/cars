		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Dealer Listing', 'Профил на K-G Team Auto Пловдив')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'K-G Team Auto публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'K-G Team Auto')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на K-G Team Auto')
					...
					.replaceAll('Heather Dick', 'Клиент на K-G Team Auto')
					.replaceAll(
...
const daynightPageAssets = {
	aboutExterior: '/dealer/11777536762184091-1.webp',
	aboutConsultation: '/dealer/11778753257201470-1.webp',
	aboutShowroomVehicle: '/dealer/11782979504874945-1.webp',
	servicesInspection: '/dealer/11777536762184091-1.webp',
	servicesConsultation: '/dealer/11777536762184091-1.webp',
...
> = {
	'about-us.html': {
		...
		description:
			'Обявени автомобили, ясна история и съдействие от избора до документите в Пловдив.',
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до K-G Team Auto Пловдив" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderTeamContactActions(member: (typeof daynightTeam)[number]) {
	...
	return `<ul class="contact">
								...
								<li>
									<a href="mailto:${member.email}" title="Имейл към K-G Team Auto" aria-label="Имейл към K-G Team Auto">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							...
							<li>
								<a href="tel:${daynightSite.phone}" class="hover-stroke-white" title="Свържете се с K-G Team Auto по телефон или Viber." aria-label="Свържете се с K-G Team Auto по телефон или Viber.">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на K-G Team Auto в Google Maps" aria-label="Отвори локацията на K-G Team Auto в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function replaceSharedCounterMetrics(html: string) {
	return html
		...
		.replace(
			/<p class="font-weight-500 text-muted h7 text-center">Обявени автомобили<\/p>/,
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до K-G Team Auto Пловдив" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
	...
	<div class="container">
		<div class="daynight-about-location">
			<div class="daynight-about-location__content">
				...
				<p class="text-secondary h7 line-height-28">K-G Team Auto предлага внимателно подбрани автомобили с реална наличност, съдействие при оглед, документи, регистрация, финансиране и бартер.</p>
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на K-G Team Auto">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със K-G Team Auto">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от K-G Team Auto', '');
...
function replacePdpCopy(html: string, routePath: string) {
	...
	let output = html
		...
		.replaceAll('Mike Hanley', salesTeam?.name ?? 'K-G Team Auto')
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от K-G Team Auto.</p>`;
	}
...
function renderBlogDetailSocialList(article: DayNightArticle) {
	...
	const shareUrl = encodeURIComponent(`https://team-auto.mobile.bg${blogArticleRoute(article)}`);
...
function renderBlogDetailAuthorBox() {
	...
	<div class="listing-details--contact-dealer mb-20">
		...
		<div class="content">
			<a href="/about/daynight-auto-plovdiv" class="h4 mb-4 font-weight-600">K-G Team Auto</a>
			...
			<ul class="blog-detail-social flex gap-12">
				...
				<li><a href="mailto:${daynightSite.email}" aria-label="Имейл към K-G Team Auto">${renderBlogSocialIcon('mail')}</a></li>
			</ul>
		</div>
	</div>
	<p class="h7 line-height-28">K-G Team Auto публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Пловдив.</p>
</div>`;
}
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'K-G Team Auto')
		...
		.replaceAll('Nelson Mandela', 'K-G Team Auto')
		.replaceAll(
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За K-G Team Auto')
			.replaceAll(
				'Premium vehicles for every lifestyle',
				'обявени употребявани автомобили, съдействие за документи, регистрация, финансиране и огледи в Пловдив.'
			)
			.replaceAll('Quality Services', 'обявени употребявани автомобили')
			.replaceAll(
				'Driving Your Ultimate Automotive Dreams Forward',
				'Автомобили с ясна история и подреден процес'
			)
			.replaceAll(
				'At K-G Team Auto, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на K-G Team Auto помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на K-G Team Auto')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на K-G Team Auto')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">K-G Team Auto предлага внимателно подбрани и автомобили от обяви с произход за потвърждение, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At K-G Team Auto, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'K-G Team Auto предлага внимателно подбрани и автомобили от обяви с произход за потвърждение, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от K-G Team Auto')
			.replaceAll('Certified Dealers', 'K-G Team Auto')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на K-G Team Auto ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на K-G Team Auto ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със K-G Team Auto')
			.replaceAll(
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на K-G Team Auto уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със K-G Team Auto за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от K-G Team Auto'
			)
			.replaceAll(
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на K-G Team Auto')
			.replaceAll(
			...
			.replaceAll('Emily Johnson', 'Клиент на K-G Team Auto')
			.replaceAll('Benjamin Parker', 'Клиент от Пловдив')
			.replaceAll('Olivia Williams', 'Клиент на K-G Team Auto')
			.replaceAll('Sophia Martinez', 'Клиент от Пловдив')
			.replaceAll('Daniel Wright', 'Клиент на K-G Team Auto')
			.replaceAll('Sarah Nguyen', 'Клиент от Пловдив')
			.replaceAll('Manager, NexTech', 'Клиент на K-G Team Auto')
			.replaceAll('Freelance Designer', 'Клиент от Пловдив')
			.replaceAll('Entrepreneur', 'Клиент на K-G Team Auto')
			.replaceAll('Accountant', 'Клиент от Пловдив')
			.replaceAll('CEO BMW', 'Клиент на K-G Team Auto')
			.replaceAll('CEO K-G Team Auto', 'Клиент на K-G Team Auto')
			.replaceAll('James Anderson', 'Клиент от Пловдив')
			.replaceAll('Project Manager', 'Клиент на K-G Team Auto')
			.replaceAll('Avitex', 'K-G Team Auto')
			.replaceAll('Tesla', 'K-G Team Auto')
			.replaceAll('Aurexo', 'K-G Team Auto');
	}

	if (templateFile === 'sale-agents.html') {
		let output = html
			.replaceAll('Sale Agents', 'Екипът на K-G Team Auto')
			.replaceAll('Our Agents', 'Консултанти')
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на K-G Team Auto')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на K-G Team Auto'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на K-G Team Auto');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на K-G Team Auto')
			...
			.replaceAll('Randynox', 'Клиент на K-G Team Auto')
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на K-G Team Auto Пловдив')
			.replaceAll('Dealer Details', 'Профил на K-G Team Auto Пловдив')
			.replaceAll('Dealer Detail', 'Профил на K-G Team Auto Пловдив')
			.replaceAll('Aurexo Atlanta', 'K-G Team Auto Пловдив')
			.replaceAll('Euro Workshop', 'K-G Team Auto Пловдив')
			...
			.replaceAll(
				...
				'K-G Team Auto Пловдив предлага обявени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със K-G Team Auto')
			.replaceAll('get in touch', 'Свържете се със K-G Team Auto')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със K-G Team Auto<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Пловдив, Пловдив, гр. Пловдив, Индустриална зона – Юг, Околовръстен път срещу хотел Чиирите<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Пловдив, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на K-G Team Auto.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на K-G Team Auto за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на K-G Team Auto съдейства с варианти за финансиране, лизинг и примерно изчисление без оферта според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на K-G Team Auto предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. K-G Team Auto прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'K-G Team Auto полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'K-G Team Auto може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със K-G Team Auto преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към K-G Team Auto')
			.replaceAll('Submit Listing', 'Изпрати към K-G Team Auto')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Darrell Steward', 'K-G Team Auto')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'K-G Team Auto')
			.replaceAll('K-G Team Auto Support', 'K-G Team Auto')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('K-G Team Auto') ? '' : ' K-G Team Auto');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('Why Choose Us?', 'Защо K-G Team Auto?')
		.replaceAll('Why Choose Us', 'Защо K-G Team Auto')
		.replaceAll(
		...
		.replaceAll('3,5k+', '100%')
		.replaceAll('Verified Dealers', 'Обявени автомобили')
		.replace(
		...
		.replaceAll('Certified Dealers', 'K-G Team Auto')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of K-G Team Auto who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на K-G Team Auto ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със K-G Team Auto')
		.replaceAll('Reach Out To Us', 'Свържете се със K-G Team Auto')
		.replaceAll('Get In Touch', 'Свържете се със K-G Team Auto')
		.replaceAll('get in touch', 'Свържете се със K-G Team Auto')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от K-G Team Auto.'
		)
		.replaceAll(
		...
		.replaceAll('Follow Us On social media:', 'Последвайте K-G Team Auto:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на K-G Team Auto.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със K-G Team Auto преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'K-G Team Auto може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'K-G Team Auto')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към K-G Team Auto')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към K-G Team Auto')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към K-G Team Auto')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към K-G Team Auto')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на K-G Team Auto')
		...
		.replaceAll('Verified Dealer', 'Проверена автокъща')
		.replaceAll('Emily Johnson', 'Клиент на K-G Team Auto')
		.replaceAll('CEO Avitex', 'Клиент на K-G Team Auto')
		.replaceAll('Benjamin Parker', 'Клиент от Пловдив')
		...
		.replaceAll('James Anderson', 'Клиент от Пловдив')
		.replaceAll('Project Manager', 'Клиент на K-G Team Auto')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'K-G Team Auto Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'K-G Team Auto Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'K-G Team Auto Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'K-G Team Auto Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'K-G Team Auto Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'K-G Team Auto Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'K-G Team Auto Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'K-G Team Auto Sales Team')
		.replaceAll('Los Angeles,California', 'Пловдив, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на K-G Team Auto')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Автомобил от обява от K-G Team Auto.')
		.replaceAll('How the adventure ended will be seen soon.', 'Автомобил от обява от K-G Team Auto.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със K-G Team Auto за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към K-G Team Auto">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на K-G Team Auto"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на K-G Team Auto"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">K-G Team Auto<\/a>/g,
				'<a href="/about/daynight-auto-plovdiv" class="h4 mb-8 font-weight-600">K-G Team Auto</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Пловдив, Пловдив, гр. Пловдив, Индустриална зона – Юг, Околовръстен път срещу хотел Чиирите\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на K-G Team Auto'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0877 34 62 62" class="text-sm text-white">0877 34 62 62<\/a>\s*<a href="tel:0877 34 62 62" class="text-sm text-white">0877 34 62 62<\/a>/,
				'<a href="tel:0877 34 62 62" class="text-sm text-white">0877 34 62 62</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">K-G Team Auto<\/a>/g,
				'<a class="text-white" href="/about/daynight-auto-plovdiv">K-G Team Auto</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'K-G Team Auto Пловдив')
		.replaceAll('Aurexo |', 'K-G Team Auto |');
}
