...
const publicE2E = process.env.AUTOMARKET_PUBLIC_E2E === 'true' || process.env.NEXT_PUBLIC_AUTOMARKET_PUBLIC_E2E === 'true';
const toolbarEnabled = !publicE2E && process.env.NODE_ENV !== 'production';
let nextConfig: NextConfig = toolbarEnabled ? withToolbar(withLogging(config)) : withLogging(config);
...
if (publicE2E) {
  const publicE2ERunId = (process.env.E2E_PUBLIC_RUN_ID ?? 'manual').replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 80);
...
nextConfig.images.remotePatterns = [
  ...
  // Precisely scoped source hosts while the local-photo transfer is in progress.
  ...
  { protocol: 'https', hostname: 'g1-bg.cars.bg' }
];

const inheritedHeaders = nextConfig.headers;
nextConfig.headers = async () => [
  ...((await inheritedHeaders?.()) ?? []),
  { source: '/:path*', headers: [{ key: 'X-Robots-Tag', value: 'noindex, nofollow, noarchive' }] }
];

if (process.env.NODE_ENV === 'production') {
  nextConfig.redirects = async () => [{ source: '/legal', destination: '/legal/privacy', statusCode: 301 }];
}
