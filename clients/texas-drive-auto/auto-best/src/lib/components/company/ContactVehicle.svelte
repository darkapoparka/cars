...
<a class="dn-contact-vehicle" class:dn-contact-vehicle--hero={hero} href={resolve('/listing-detail-v1/[id]', { id: String(vehicle.id) })} aria-label={`To ${vehicle.title}`}>
  <img src={vehicle.image} alt="" width="120" height="90" />
  <span><small>Selected vehicle</small><strong>{vehicle.title}</strong><span>{vehicle.year} · {formatVehiclePrice(vehicle.priceUsd)}</span></span>
