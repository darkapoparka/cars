<script lang="ts">
  ...
  const searchLabel = $derived(field === 'make' ? 'Search makes' : field === 'model' ? 'Search models' : `Search in ${title.toLocaleLowerCase('en-US')}`);
  ...
  const matchesSearch = (option: string) => search.trim().toLocaleLowerCase('en-US').split(/\s+/).every(term => optionLabel(option).toLocaleLowerCase('en-US').includes(term));
    ...
    <div class="content">
      {#if range}
        <div class="range">
          ...
          <label>To{field === 'price' ? ' ($)' : ''}<input type="number" inputmode="numeric" name={`${field}_max`} bind:value={maximum} min={field === 'year' ? 1900 : 0} max={field === 'year' ? new Date().getFullYear() + 1 : undefined} step="1" placeholder="No maximum" /></label>
        </div>
        {#if invalid}<p role="alert">The starting value must be lower than the ending value.</p>{/if}
      {:else if field === 'mileage_max'}
        <label class="mileage">Maximum mileage (miles)<input type="number" inputmode="numeric" name={field} bind:value={selected} min="0" step="1" placeholder="No limit" /></label>
      {:else}
        {#key field}
        <fieldset>
        ...
        {#if visibleChoices.length === 0}
          <div class="empty" role="status"><strong>No matches</strong><p>Try another name or clear your search.</p></div>
        {/if}
      {/if}
    </div>
