<script lang="ts">
  ...
  const mobileFields = $derived([
    ...
    { field: 'body', label: 'Body style', value: bodyLabel(draftBody) || 'All body styles' },
    ...
    { field: 'version', label: 'Trim', value: draftVersion || 'All' },
    ...
    <div class="dn-listing-filter__dialog-content">
      ...
      <div class="dn-listing-filter__filter-groups">
        <div class="dn-listing-filter__core-grid">
          ...
          <label>
            <span class="dn-listing-filter__field-label">Body style</span>
            <select name="body" aria-label="Body style" bind:value={draftBody}>
              {#each listingFilterOptions.bodies as option (option)}
                <option value={option}>{bodyLabel(option) || 'Body style'}</option>
              {/each}
            </select>
          </label>
          <label>
          ...
          <label>
            <span class="dn-listing-filter__field-label">Min price</span>
            <select name="price_min" aria-label="Min price" bind:value={draftPriceMin}>
              ...
              {#each listingFilterOptions.prices as option (option)}
                <option value={option}>{option ? `From ${new Intl.NumberFormat('en-US').format(Number(option))} $` : 'Min price'}</option>
              {/each}
            </select>
          </label>
          <label>
            <span class="dn-listing-filter__field-label">Max price</span>
            <select name="price_max" aria-label="Max price" bind:value={draftPriceMax}>
              ...
              {#each listingFilterOptions.prices as option (option)}
                <option value={option}>{option ? `Up to ${new Intl.NumberFormat('en-US').format(Number(option))} $` : 'Max price'}</option>
              {/each}
            </select>
          </label>
          <label>
          ...
          <label>
            <span class="dn-listing-filter__field-label">Max year</span>
            <select name="year_max" aria-label="Max year" bind:value={draftYearMax}>
              ...
              {#each listingFilterOptions.years as option (option)}
                <option value={option}>{option || 'Max year'}</option>
              {/each}
            </select>
          </label>
          <label>
            <span class="dn-listing-filter__field-label">Max mileage</span>
            <select name="mileage_max" aria-label="Max mileage" bind:value={draftMileageMax}>
              ...
              {#each listingFilterOptions.mileages as option (option)}
                <option value={option}>{option ? `Up to ${new Intl.NumberFormat('en-US').format(Number(option))} miles` : 'Max mileage'}</option>
              {/each}
            </select>
          </label>
          <label>
          ...
          <label>
            <span class="dn-listing-filter__field-label">Package or trim</span>
            <select name="version" aria-label="Package or trim" bind:value={draftVersion}>
              {#each listingFilterOptions.versions as option (option)}
                <option value={option}>{option || 'Trim'}</option>
              {/each}
            </select>
          </label>
        </div>
    ...
    <footer class="dn-listing-filter__dialog-footer">
      {#if hasInvalidRange}
        <p class="dn-listing-filter__range-error" role="alert">
          {hasInvalidPriceRange ? 'The minimum price must be lower than the maximum.' : 'The starting year must be earlier than the ending year.'}
        </p>
      {/if}
