...
<form id="dn-desktop-discovery" class="dn-discovery" {@attach observePanel} method="GET" action={resolve('/listing-grid')} oninput={updateDraft} onchange={updateDraft} onformdata={clean}>
  <div class="dn-discovery__toolbar">
    <div class="dn-discovery__search">
      <button class="dn-discovery__keyword" type="button" aria-label="Search by make, model, or keyword" aria-haspopup="dialog" aria-controls="dn-listing-filter-dialog" aria-expanded={filtersOpen} onclick={openFilters}>
  ...
  <div class="dn-discovery__facets">
    ...
    <label><span>Body style</span><select name="body" value={filters.body}>{#each listingFilterOptions.bodies as value (value)}<option {value}>{bodyLabel(value) || 'All'}</option>{/each}</select></label>
    <label><span>Max price</span><select name="price_max" value={filters.priceMax?.toString() ?? ''}>{#each prices as value (value)}<option {value}>{value ? `${number(value)} $` : 'No limit'}</option>{/each}</select></label>
    ...
    <label><span>Max mileage</span><select name="mileage_max" value={filters.mileageMax?.toString() ?? ''}>{#each mileages as value (value)}<option {value}>{value ? `${number(value)} miles` : 'No limit'}</option>{/each}</select></label>
  </div>
