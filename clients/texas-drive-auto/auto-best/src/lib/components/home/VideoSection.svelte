          ...
          <article class="dn-video-card">
            ...
            <div class="dn-video-card__content">
              ...
              <div class="dn-video-card__actions">
                <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer" aria-label={`${video.title} — watch on YouTube (new tab)`}>
                  Watch on YouTube <Icon name="arrow-right" size={16} />
                </a>
                {#if activeVideo === video.id}
                  <button type="button" onclick={stop} aria-label={`Close video: ${video.title}`}><Icon name="x" size={20} /></button>
                {/if}
              </div>
            </div>
          </article>
