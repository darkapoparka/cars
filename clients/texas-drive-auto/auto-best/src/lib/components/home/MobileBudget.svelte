<script lang="ts">
  ...
  const budgetTiles = [
    {
      label: 'Up to 60 000 USD',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceUsd <= 60000).length} vehicles`,
      href: '/listing-grid?price_max=60000',
      image: featuredVehicles[1].image
    },
    {
      label: '60–70 000 $',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceUsd > 60000 && vehicle.priceUsd <= 70000).length} vehicles`,
      href: '/listing-grid?price_min=60000&price_max=70000',
      image: featuredVehicles[0].image
    },
    {
      label: 'Over 70 000 USD',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceUsd > 70000).length} vehicles`,
      href: '/listing-grid?price_min=70000',
      image: featuredVehicles[6].image
    },
    {
      label: 'View all',
      detail: `${featuredVehicles.length} vehicles`,
      href: '/listing-grid',
