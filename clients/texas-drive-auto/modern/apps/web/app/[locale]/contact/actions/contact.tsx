...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Texas Drive Auto.",
          "The form is ready, but message delivery is not configured yet. Call Texas Drive Auto directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Texas Drive Auto.",
          "Message delivery is not configured yet. Call Texas Drive Auto directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Texas Drive Auto."
        : "Запитването е изпратено до екипа на Texas Drive Auto.",
      "Your request has been sent to the Texas Drive Auto team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Texas Drive Auto: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
