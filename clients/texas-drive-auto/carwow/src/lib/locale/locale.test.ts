...
describe('native Carwow locale adapter', () => {
	...
	it('keeps route identity and dealer facts independent', () => {
		...
		expect(dealerLabel('en', 'city')).toBe('Dallas');
		expect(dealerLabel('bg', 'city')).toBe('Dallas');
		expect(dealerLabel('en', 'address')).toBe('18 Atanas Manchev Street, Studentski Grad, Dallas');
