<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Свържете се за работно време и оглед.',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/ivo-auto/vehicle-01-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Иво Ауто начало">
					<img src={resolve('/assets/ivo-auto/wordmark.svg')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Иво Ауто'}
			</h1>
			<p>
				{isImportMode
			...
			<div class="mobile-import-sheet__surface">
				...
				>
					...
					<img
						...
						src={resolve('/assets/ivo-auto/wordmark.svg')}
						alt=""
						aria-hidden="true"
					/>
