<script lang="ts">
  ...
  let photoIndex = $derived(selectedPhotos[data.vehicle.id] ?? 0);
          ...
          <div class="dn-detail-main">
            ...
            <div class="dn-detail-card dn-detail-media-card">
              <figure class="dn-detail-gallery">
                ...
                <img
                  src={data.vehicle.gallery[photoIndex]}
                  ...
                  decoding="async"
                />
                <div class="dn-detail-gallery__count">{#each data.vehicle.gallery as photo, index (photo)}<button type="button" aria-label={`Снимка ${index + 1}`} aria-pressed={photoIndex === index} onclick={() => selectedPhotos[data.vehicle.id] = index}>{index + 1}</button>{/each}</div>
              </figure>
            </div>
