...
<section class="dn-videos" aria-labelledby="videos-title"><div class="container"><div class="dn-videos__panel"><div class="dn-videos__heading dn-home-section-heading"><h2 id="videos-title" class="dn-home-section-title">Разгледайте отблизо</h2><p>Снимки от публикуваните обяви на Перфект Ауто.</p><a class="dn-videos__channel dn-home-section-action" href="/listing-grid">Всички автомобили <Icon name="arrow-right" size={16}/></a></div><div class="dn-videos__grid">{#each featuredVehicles.slice(0,3) as vehicle (vehicle.id)}<article class="dn-video-card"><a class="dn-video-card__media" href={vehicle.href}><img style="width:100%;height:100%;object-fit:cover" src={vehicle.image} alt={vehicle.title} loading="lazy"/></a><div class="dn-video-card__content"><h3>{vehicle.title}</h3><div class="dn-video-card__actions"><a href={vehicle.href}>Снимки и детайли <Icon name="arrow-right" size={16}/></a></div></div></article>{/each}</div></div></div></section>
<style>
  ...
  @media (max-width: 767px) {
    ...
    .dn-videos__heading { min-height: 0; grid-template-columns: 1fr; gap: 10px; padding: 20px; border-radius: 14px; }
    .dn-videos__heading h2 { gap: 6px 8px; font-size: 24px; font-weight: 700; }
    .dn-videos__brand-mark :global(svg) { width: 32px; height: 32px; }
    
    .dn-videos__channel { grid-column: 1; grid-row: auto; justify-self: start; margin-top: 4px; border-radius: var(--dn-pill); font-size: 14px; }
