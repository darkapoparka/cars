...
<section class="dn-brand-section" aria-labelledby="brand-title">
  <div class="container dn-brand-shell">
    ...
    <div class="dn-brand-panel">
      <div class="dn-brand-grid" style:--brand-columns={Math.max(1, Math.min(brands.length, 6))} aria-label="Автомобили по марка">
      ...
      <button class="dn-discovery-toggle" aria-expanded={expanded} onclick={() => expanded = !expanded}>{expanded ? 'Покажи по-малко' : 'Всички марки'}</button>
    </div>
  </div>
</section>
