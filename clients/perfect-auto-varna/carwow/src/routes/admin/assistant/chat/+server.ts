...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const assistantTools = {
		getInventory: tool({
			description:
				'Read Перфект Ауто inventory records by status or search query. Returns admin links, public links, specs, pricing, and QA issues.',
		...
		getPosts: tool({
			description:
				'Read Перфект Ауто posts and news records by status, type, or search query. Returns admin/public links and editorial metadata.',
			inputSchema: z.object({
