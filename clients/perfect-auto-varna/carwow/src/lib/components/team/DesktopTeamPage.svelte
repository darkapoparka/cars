	...
	<ul class="contact">
		...
		{#if member.email}
			<li>
				<a
					href={`mailto:${member.email}`}
					title="Имейл към Перфект Ауто"
					aria-label="Имейл към Перфект Ауто"
				>
					{@render mailIcon(24)}
				</a>
			</li>
	...
	<section class="background-light mb-32">
		<div class="container">
			<ul class="breadcrumb">
				...
				<li>
					<span>Екипът на Перфект Ауто</span>
				</li>
			</ul>
		</div>
	</section>
...
<style>
	...
	/* Breadcrumb (verified: min-height 76, gap 10, ul 14/700 #5f6877, a #1c1c1c 14/400,
	   icon 14px). The /variant-3/assets/right.svg chevrons become @lucide ChevronRight. */
	.breadcrumb {
		display: flex;
		min-height: 76px;
		align-items: center;
