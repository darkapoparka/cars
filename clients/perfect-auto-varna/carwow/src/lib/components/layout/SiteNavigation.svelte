<script lang="ts">
	import { resolve, localPath } from '$lib/utils/preview-paths';
	...
	<ul id="menu-primary-menu" class={menuClasses}>
		{#each desktopNavItems as item (item.href)}
			<li class={topItemClasses}>
				<a class={topLinkClasses} href={resolve(item.href)} aria-current={localPath(page.url.pathname) === item.href || (item.href !== '/' && localPath(page.url.pathname).startsWith(`${item.href}/`)) ? 'page' : undefined}>{item.label}</a>
			</li>
		{/each}
	</ul>
</nav>
