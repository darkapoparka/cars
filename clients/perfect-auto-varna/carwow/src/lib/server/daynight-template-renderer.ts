...
function normalizeAssetUrls(html: string) {
	return html
		.replaceAll('./assets/', '/variant-3/assets/')
		.replaceAll('href="assets/', 'href="/variant-3/assets/')
		.replaceAll("href='assets/", "href='/variant-3/assets/")
		.replaceAll('src="assets/', 'src="/variant-3/assets/')
		.replaceAll("src='assets/", "src='/variant-3/assets/")
		.replaceAll('content="./assets/', 'content="/variant-3/assets/')
		.replaceAll('content="assets/', 'content="/variant-3/assets/');
}
...
function mountTemplateUrls(html: string) {
	return html.replace(/\b(href|src|action|poster)=(["'])(\/(?!\/)[^"']*)\2/g, (_match, attribute, quote, path) =>
		`${attribute}=${quote}${previewPath(path)}${quote}`
	);
}
...
export async function renderDayNightTemplate(
	...
	return mountTemplateUrls(injectLocalBehavior(withSharedHeader, templateFile));
}
