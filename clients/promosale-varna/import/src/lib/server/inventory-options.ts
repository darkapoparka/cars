...
const makeFilter = ({
	...
	placeholder: string;
	selected?: string | number;
	numericInput?: AuxeroInventoryFilter['numericInput'];
	...
	selectedSummary:
		numericInput && selected
			? `${locale === 'bg' ? 'До' : 'Up to'} ${new Intl.NumberFormat(locale).format(Number(selected))} ${numericInput.unit}`
			: selectedOptionLabels(options, selected, placeholder, locale),
...
const featureOptions = (locale: Locale): AuxeroInventoryFilterOption[] =>
	popularFeatureOptions.map((feature) => ({
		label: inventoryText(locale, feature),
		value: feature
	}));
...
const buildFilters = (state: InventoryState, locale: Locale) => {
	...
	const filters = [
		...
		makeFilter({
			...
			name: 'priceTo',
			numericInput: {
				label: locale === 'bg' ? 'Максимална цена' : 'Maximum price',
				unit: site.locale.currency
			},
		...
		makeFilter({
			...
			name: 'mileageTo',
			numericInput: {
				label: locale === 'bg' ? 'Максимален пробег' : 'Maximum mileage',
				unit: locale === 'bg' ? 'км' : 'km'
			},
	...
	return {
		...
		appointmentNote:
			locale === 'bg'
				...
				: `${dealerCopy.en.appointment} The exact viewing location is confirmed by phone.`,
