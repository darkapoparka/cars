...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Keen Auto Mall.",
          "The form is ready, but message delivery is not configured yet. Call Keen Auto Mall directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Keen Auto Mall.",
          "Message delivery is not configured yet. Call Keen Auto Mall directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Keen Auto Mall."
        : "Запитването е изпратено до екипа на Keen Auto Mall.",
      "Your request has been sent to the Keen Auto Mall team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Keen Auto Mall: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
