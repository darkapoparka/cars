<script lang="ts">
  ...
  let activeCount = $derived(Object.entries(pending).filter(([key, value]) => !['q', 'sort', 'equipment'].includes(key) && Boolean(value)).length + pending.equipment.length);
...
<form id="dn-desktop-discovery" class="dn-discovery" {@attach observePanel} method="GET" action={resolve('/listing-grid')} oninput={updateDraft} onchange={updateDraft} onformdata={clean}>
  ...
  <div class="dn-discovery__facets">
    ...
    <label><span>Купе</span><select name="body" value={filters.body}>{#each listingFilterOptions.bodies as value (value)}<option {value}>{value || 'Всички'}</option>{/each}</select></label>
...
<style>
  ...
  .dn-discovery__submit { display: inline-flex; flex: 0 0 48px; align-items: center; justify-content: center; width: 48px; height: 48px; padding: 0; border: 0; border-radius: var(--dn-pill); background: var(--dn-red); color: var(--dn-accent-ink); cursor: pointer; }
  ...
  .dn-discovery__count { position: absolute; top: -6px; right: -6px; display: grid; place-items: center; min-width: 20px; height: 20px; padding: 0 4px; border: 2px solid white; border-radius: var(--dn-pill); background: var(--dn-red); color: var(--dn-accent-ink); font-size: 11px; }
  ...
  .dn-discovery-sticky__submit { display: grid; place-items: center; flex: 0 0 48px; width: 48px; height: 48px; padding: 0; border: 0; border-radius: var(--dn-pill); background: var(--dn-red); color: var(--dn-accent-ink); cursor: pointer; }
