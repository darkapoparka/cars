<script lang="ts">
  ...
  let { vehicle, showPrice = false, priority = false, layout = 'default' }: Props = $props();
</script>

<article class:dn-vehicle-card--listing={layout === 'listing'} class="dn-vehicle-card">
  <a class="dn-vehicle-card__link" href={resolve('/listing-detail-v1/[id]', { id: String(vehicle.id) })} aria-label={`Вижте ${vehicle.title}`}>
...
<style>
  ...
  .dn-vehicle-card__link:focus-visible {
    outline: 3px solid rgba(215, 174, 53, 0.24);
    outline-offset: -3px;
  }
