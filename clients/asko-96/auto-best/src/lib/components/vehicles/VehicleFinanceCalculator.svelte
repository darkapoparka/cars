<script lang="ts">
  ...
  let { priceEur, vehicleTitle }: { priceEur: number; vehicleTitle: string } = $props();
...
<div class="dn-finance-calculator">
  ...
  <a href={resolve(`/contact?topic=leasing&vehicle=${encodeURIComponent(vehicleTitle)}`)}>
    Получете реална оферта
  </a>
</div>
