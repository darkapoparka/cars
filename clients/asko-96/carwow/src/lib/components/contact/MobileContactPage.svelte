<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Пн–Пт: 09:00–18:30 · Сб: 10:00–16:00 · Нд: почивен ден',
			href: null,
			icon: Clock
		}
	] as const;
	...
	const leadErrorMessage = `Демо форма без доставка. Свържете се с АСКО 96 на ${daynightSite.phoneLabel}.`;
	const importErrorMessage = `Демо форма без доставка. За внос се свържете с АСКО 96 на ${daynightSite.phoneLabel}.`;
	...
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		...
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="АСКО 96 начало">
					<img src={resolve('/assets/asko96/asko96-wordmark.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със АСКО 96'}
			</h1>
			<p>
				{isImportMode
		...
		{#if isImportMode}
			<p class="text-sm">Локален демо преглед: формата не изпраща до автокъщата. За реално запитване: <a href="tel:+359899769696">0899 76 96 96</a>.</p>
<form
...
<style>
	...
	.mobile-contact-hero--import::after {
		background:
			...
			linear-gradient(90deg, rgba(215, 174, 53, 0.13), transparent 48%);
	}
