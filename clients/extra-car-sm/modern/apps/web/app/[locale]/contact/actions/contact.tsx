...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на ЕКСТРА КАР-СМ.",
          "The form is ready, but message delivery is not configured yet. Call ЕКСТРА КАР-СМ directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на ЕКСТРА КАР-СМ.",
          "Message delivery is not configured yet. Call ЕКСТРА КАР-СМ directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на ЕКСТРА КАР-СМ."
        : "Запитването е изпратено до екипа на ЕКСТРА КАР-СМ.",
      "Your request has been sent to the ЕКСТРА КАР-СМ team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `ЕКСТРА КАР-СМ: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
