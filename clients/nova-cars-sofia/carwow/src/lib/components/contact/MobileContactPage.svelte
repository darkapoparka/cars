...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/dealer/inventory/21788867190181219-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="NOVA CARS начало">
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със NOVA CARS'}
			</h1>
			<p>
				{isImportMode
		...
		{#if isImportMode}
			<nav class="mobile-import-disclosures" aria-label="Повече за вноса">
				<MobilePromoCard
					...
					label="Виж стъпките"
					image="/assets/dealer/inventory/21788528450494675-1.webp"
					tone="red"
