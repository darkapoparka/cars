...
- `carwow/`: actual full retained application copied independently. Site/navigation configuration, current-inventory adapter and vehicle adapter use the same source snapshot. Its FAQ, review data, business-contact pages and shared metadata are now personalized. Remaining supporting consumers and final assets are unfinished.
...
## Latest content checkpoints

`6b35e33e68acae5de46fb664d355d19f850b370d` replaces the Carwow FAQ copy while retaining its 12 accordion entries and removes invented customer reviews. `43a6ba1db28b38f9cded3ea287b66ccd216be548` replaces fictional staff with one explicitly identified business contact, retains compatible profile URLs, preserves the actual team components, and updates shared route/vehicle metadata.

Executed checks: 12 FAQ/review assertions; 28 team/metadata/source assertions; strict TypeScript checking of seven data/metadata modules. All exited 0. These are not application builds or rendered checks. Exact commands, input blob hashes, results and limits are in [checks/TEAM-SEO-VERIFICATION.md](checks/TEAM-SEO-VERIFICATION.md), [checks/FAQ-REVIEW-RESULTS.json](checks/FAQ-REVIEW-RESULTS.json) and [checks/TEAM-SEO-RESULTS.json](checks/TEAM-SEO-RESULTS.json). All desktop/mobile/identity/contact-path QA flags remain false.
