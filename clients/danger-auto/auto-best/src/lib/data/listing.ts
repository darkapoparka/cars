...
/** Retained template URL contract for forms, chips, return links and dialogs. */
...
// Client-only facet values: no inherited luxury-car years, budgets or equipment.
export const listingFilterOptions = {
  ...
  versions: ['', 'TDI', 'CDI', 'D-4D', 'COSMO'],
  equipment: [...new Set(featuredVehicles.flatMap(vehicle => vehicle.equipment))],
  years: ['', ...[...new Set(featuredVehicles.map(vehicle => vehicle.yearNumber))].sort((a, b) => a - b).map(String)],
  prices: ['', '3000', '4000', '5000', '6000', '7000', '8000', '9000'],
...
export const vehicleMatchesQuery = (vehicle: Vehicle, query: string) => {
  ...
  return normalize([
    vehicle.title, vehicle.make, vehicle.category, vehicle.body, vehicle.year,
    vehicle.mileage, vehicle.fuel, vehicle.transmission, ...vehicle.equipment
