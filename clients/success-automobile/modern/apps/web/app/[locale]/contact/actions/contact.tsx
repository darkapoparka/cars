...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Success Automobile.",
          "The form is ready, but message delivery is not configured yet. Call Success Automobile directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Success Automobile.",
          "Message delivery is not configured yet. Call Success Automobile directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Success Automobile."
        : "Запитването е изпратено до екипа на Success Automobile.",
      "Your request has been sent to the Success Automobile team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Success Automobile: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
