...
export function vehicleSeo(vehicle: DayNightVehicle): PageSeo {
	...
	return {
		title: `${vehicle.title} | Success Automobile`,
		description: `${vehicle.title}${facts.length ? ` - ${facts.join(', ')}` : ''}. Проверен автомобил от Success Automobile с опция за финансиране.`
	};
}
