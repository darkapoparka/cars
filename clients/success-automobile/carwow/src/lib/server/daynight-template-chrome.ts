...
function renderFooterSocialLinks() {
	...
	const links: readonly PresentationIconLink[] = [
		...
		},
		{
			...
			title: 'Отвори локацията на Success Automobile в Google Maps',
			kind: 'external',
...
<p class="text-sm text-muted mb-16">Success Automobile предлага актуална дигитална витрина с наличност, запитвания и ясни следващи стъпки.</p>
${renderFooterSocialLinks()}
</div>
<div>
<p class="text-sm text-white font-weight-600 mb-8">Имате въпрос?</p>
<div class="flex items-center gap-12">
<a href="/contact" class="btn btn-line-1 btn-small" title="Свържете се със Success Automobile">Свържете се</a>
</div>
</div>
</div>
</div>
...
<p class="text-sm text-muted">©2026 <a class="text-sm text-white" href="/">Success Automobile</a>. Демонстрационна концепция. Не е официалният сайт.</p>
...
<a href="tel:${daynightSite.phone}" class="icon" aria-label="Обади се на Success Automobile">${phoneIcon()}</a>
...
<a href="${daynightSite.mapUrl}" class="icon" target="_blank" rel="noopener" aria-label="Отвори карта до Success Automobile">${locationIcon()}</a>
<div class="daynight-header-contact__body">
...
<a class="daynight-map-preview" href="${daynightSite.mapUrl}" target="_blank" rel="noopener" aria-label="Отвори Google карта до Success Automobile">
