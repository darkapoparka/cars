		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Dealer Listing', 'Профил на Черно море Варна')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'Черно море публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'Черно море')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на Черно море')
					...
					.replaceAll('Heather Dick', 'Клиент на Черно море')
					.replaceAll(
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до Черно море Варна" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderTeamContactActions(member: (typeof daynightTeam)[number]) {
	...
	return `<ul class="contact">
								...
								<li>
									<a href="mailto:${member.email}" title="Имейл към Черно море" aria-label="Имейл към Черно море">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							...
							<li>
								<a href="tel:${daynightSite.phone}" class="hover-stroke-white" title="Свържете се с Черно море по телефон или Viber." aria-label="Свържете се с Черно море по телефон или Viber.">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на Черно море в Google Maps" aria-label="Отвори локацията на Черно море в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до Черно море Варна" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
	...
	<div class="container">
		<div class="daynight-about-location">
			<div class="daynight-about-location__content">
				...
				<p class="text-secondary h7 line-height-28">Черно море предлага внимателно подбрани автомобили с реална наличност, съдействие при оглед, документи, регистрация, финансиране и бартер.</p>
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на Черно море">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със Черно море">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от Черно море', '');
...
function replacePdpCopy(html: string, routePath: string) {
	...
	let output = html
		...
		.replaceAll('Mike Hanley', salesTeam?.name ?? 'Черно море')
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от Черно море.</p>`;
	}
...
function renderBlogDetailAuthorBox() {
	...
	<div class="listing-details--contact-dealer mb-20">
		...
		<div class="content">
			<a href="/about/daynight-auto-plovdiv" class="h4 mb-4 font-weight-600">Черно море</a>
			...
			<ul class="blog-detail-social flex gap-12">
				...
				<li><a href="mailto:${daynightSite.email}" aria-label="Имейл към Черно море">${renderBlogSocialIcon('mail')}</a></li>
			</ul>
		</div>
	</div>
	<p class="h7 line-height-28">Черно море публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Варна.</p>
</div>`;
}
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'Черно море')
		...
		.replaceAll('Nelson Mandela', 'Черно море')
		.replaceAll(
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За Черно море')
			.replaceAll(
			...
			.replaceAll(
				'At Черно море, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на Черно море помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на Черно море')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на Черно море')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">Черно море предлага внимателно подбрани и проверени автомобили с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At Черно море, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'Черно море предлага внимателно подбрани и проверени автомобили с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от Черно море')
			.replaceAll('Certified Dealers', 'Черно море')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на Черно море ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на Черно море ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със Черно море')
			.replaceAll(
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на Черно море уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със Черно море за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от Черно море'
			)
			.replaceAll(
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на Черно море')
			.replaceAll(
			...
			.replaceAll('Emily Johnson', 'Клиент на Черно море')
			.replaceAll('Benjamin Parker', 'Клиент от Варна')
			.replaceAll('Olivia Williams', 'Клиент на Черно море')
			.replaceAll('Sophia Martinez', 'Клиент от Варна')
			.replaceAll('Daniel Wright', 'Клиент на Черно море')
			.replaceAll('Sarah Nguyen', 'Клиент от Варна')
			.replaceAll('Manager, NexTech', 'Клиент на Черно море')
			.replaceAll('Freelance Designer', 'Клиент от Варна')
			.replaceAll('Entrepreneur', 'Клиент на Черно море')
			.replaceAll('Accountant', 'Клиент от Варна')
			.replaceAll('CEO BMW', 'Клиент на Черно море')
			.replaceAll('CEO Черно море', 'Клиент на Черно море')
			.replaceAll('James Anderson', 'Клиент от Варна')
			.replaceAll('Project Manager', 'Клиент на Черно море')
			.replaceAll('Avitex', 'Черно море')
			.replaceAll('Tesla', 'Черно море')
			.replaceAll('Aurexo', 'Черно море');
	}

	if (templateFile === 'sale-agents.html') {
		let output = html
			.replaceAll('Sale Agents', 'Екипът на Черно море')
			.replaceAll('Our Agents', 'Консултанти')
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на Черно море')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на Черно море'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на Черно море');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на Черно море')
			...
			.replaceAll('Randynox', 'Клиент на Черно море')
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на Черно море Варна')
			.replaceAll('Dealer Details', 'Профил на Черно море Варна')
			.replaceAll('Dealer Detail', 'Профил на Черно море Варна')
			.replaceAll('Aurexo Atlanta', 'Черно море Варна')
			.replaceAll('Euro Workshop', 'Черно море Варна')
			...
			.replaceAll(
				...
				'Черно море Варна предлага проверени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със Черно море')
			.replaceAll('get in touch', 'Свържете се със Черно море')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със Черно море<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Варна, Варна, гр. Варна, бул. Цар Освободител 289, пресечка с бул. Константин и Фружин, бул. Цар Освободител 289, пресечка с бул. Константин и Фружин<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Варна, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на Черно море.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на Черно море за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на Черно море съдейства с варианти за финансиране, лизинг и собствено разсрочено плащане според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на Черно море предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. Черно море прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'Черно море полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'Черно море може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със Черно море преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към Черно море')
			.replaceAll('Submit Listing', 'Изпрати към Черно море')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Darrell Steward', 'Черно море')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'Черно море')
			.replaceAll('Черно море Support', 'Черно море')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('Черно море') ? '' : ' Черно море');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('Why Choose Us?', 'Защо Черно море?')
		.replaceAll('Why Choose Us', 'Защо Черно море')
		.replaceAll(
		...
		.replaceAll('Certified Dealers', 'Черно море')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of Черно море who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на Черно море ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със Черно море')
		.replaceAll('Reach Out To Us', 'Свържете се със Черно море')
		.replaceAll('Get In Touch', 'Свържете се със Черно море')
		.replaceAll('get in touch', 'Свържете се със Черно море')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от Черно море.'
		)
		.replaceAll(
		...
		.replaceAll('Follow Us On social media:', 'Последвайте Черно море:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на Черно море.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със Черно море преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'Черно море може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'Черно море')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към Черно море')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към Черно море')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към Черно море')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към Черно море')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на Черно море')
		...
		.replaceAll('Verified Dealer', 'Проверена автокъща')
		.replaceAll('Emily Johnson', 'Клиент на Черно море')
		.replaceAll('CEO Avitex', 'Клиент на Черно море')
		.replaceAll('Benjamin Parker', 'Клиент от Варна')
		...
		.replaceAll('James Anderson', 'Клиент от Варна')
		.replaceAll('Project Manager', 'Клиент на Черно море')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'Черно море Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'Черно море Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'Черно море Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'Черно море Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'Черно море Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'Черно море Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'Черно море Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'Черно море Sales Team')
		.replaceAll('Los Angeles,California', 'Варна, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на Черно море')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Проверен автомобил от Черно море.')
		.replaceAll('How the adventure ended will be seen soon.', 'Проверен автомобил от Черно море.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със Черно море за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към Черно море">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на Черно море"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на Черно море"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">Черно море<\/a>/g,
				'<a href="/about/daynight-auto-plovdiv" class="h4 mb-8 font-weight-600">Черно море</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Варна, Варна, гр. Варна, бул. Цар Освободител 289, пресечка с бул. Константин и Фружин, бул. Цар Освободител 289, пресечка с бул. Константин и Фружин\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на Черно море'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0888 596 963" class="text-sm text-white">0888 596 963<\/a>\s*<a href="tel:0888 596 963" class="text-sm text-white">0888 596 963<\/a>/,
				'<a href="tel:0888 596 963" class="text-sm text-white">0888 596 963</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">Черно море<\/a>/g,
				'<a class="text-white" href="/about/daynight-auto-plovdiv">Черно море</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'Черно море Варна')
		.replaceAll('Aurexo |', 'Черно море |');
}
