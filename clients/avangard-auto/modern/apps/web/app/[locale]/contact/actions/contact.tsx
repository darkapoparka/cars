...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на AVANGARD AUTO.",
          "The form is ready, but message delivery is not configured yet. Call AVANGARD AUTO directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на AVANGARD AUTO.",
          "Message delivery is not configured yet. Call AVANGARD AUTO directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на AVANGARD AUTO."
        : "Запитването е изпратено до екипа на AVANGARD AUTO.",
      "Your request has been sent to the AVANGARD AUTO team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `AVANGARD AUTO: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
