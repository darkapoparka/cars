<script lang="ts">
  ...
  const heroDescriptions = {
    ...
    'trade-in': 'Изпратете въпрос за своя автомобил'
  };
</script>
...
<section class="dn-contact-hero dn-route-hero dn-route-hero--studio" class:dn-contact-hero--general={topic.id === 'general'} class:dn-contact-hero--workflow={topic.id === 'trade-in' || topic.id === 'import'} aria-labelledby="contact-title">
  ...
  <picture>
    {#if topic.id === 'trade-in'}
      <source media="(max-width: 991px)" srcset="/assets/avangard/vehicle-01-1.webp" />
    {:else if topic.id === 'import'}
      <source media="(max-width: 991px)" srcset="/assets/avangard/vehicle-02-1.webp" />
    {/if}
  <img
