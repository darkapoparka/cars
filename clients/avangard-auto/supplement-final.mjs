...
try {
 for(const [c,t,port] of [['avangard-auto','auto-best',6626],['avangard-auto','carwow',6628],['astracar','auto-best',6636],['champion-auto-pro','modern',6632],['champion-auto-pro','carwow',6633]]) {
  const dir=`J:/cars/clients/${c}/qa-final/supplement`;await fs.mkdir(dir,{recursive:true});let owned;const rows=[];
  try {
   owned=JSON.parse((await exec('powershell.exe',['-NoProfile','-File',runtime,'-Client',c,'-Template',t,'-Port',String(port)])).stdout);
