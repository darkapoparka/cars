<script lang="ts">
	...
	const budgetTiles = $derived(data.budgetTiles.slice(0, 6));
	...
	const brandCountLabel = (count: number) =>
		count > 0 ? `${count} ${count === 1 ? 'автомобил' : 'автомобила'}` : 'Транспорт в България';
...
<div class="mobile-home" aria-label="AVANGARD AUTO — начало">
	<header class="mh-hero">
		<div class="mh-hero__bar">
			<a class="mh-hero__brand" href={resolve('/')} aria-label="AVANGARD AUTO home">
				<img
					class="mh-hero__logo"
			...
			<div class="mh-hero__bar-actions">
				<button
					...
					type="button"
					aria-label="Локация: Индустриална зона, Варна"
				...
				<a
					...
					href={phoneHref}
					aria-label="Обади се на AVANGARD AUTO"
				>
		...
		<h1 class="mh-hero__title">AVANGARD AUTO</h1>
		...
		<div class={`mh-hero__box${heroBox ? ' mh-hero__box--card' : ''}`}>
			...
			{:else}
				<button class="mh-hero__all" type="button" onclick={openSearch}>
					<span>Попитай за транспорт</span>
					<ChevronRight size={12} strokeWidth={2.8} aria-hidden="true" />
				</button>
			{/if}
		</div>
		...
		<section class="mh-cta-wrap" aria-label="Призив за действие">
			<div class="mh-cta">
				<div class="mh-cta__copy">
					<strong>{total} обяви · Оглед във Варна</strong>
					<span>Лизинг · регистрация · транспорт</span>
				</div>
	...
	<footer class="mh-footer">
		<div class="mh-footer__brand">
			...
			/>
			<p>
				Автокъща във Варна с подбрани употребявани автомобили. Съдействие за документи, регистрация,
		...
		<span class="mh-footer__copy">© 2026 AVANGARD AUTO Варна. Всички права запазени.</span>
	</footer>
	...
	>
		...
		{:else}
			<div class="mh-search-sheet mh-search-sheet--sell">
				<header>
					<div>
						<span>Съдействие при регистрация</span>
	...
	<MobileDrawer bind:open={locationOpen} labelledBy="mh-location-title">
		<div class="mh-location-sheet">
			<header>
				<div>
					<span>Локация</span>
					<strong id="mh-location-title">Индустриална зона, Варна</strong>
				</div>
			...
			<div class="mh-location-sheet__map" aria-hidden="true">
				...
				<span>Варна</span>
				<strong>гр. Варна, Владислав Варненчик, бул. Цар Освободител 289</strong>
			</div>
...
<style>
	...
	.mh-car__tools button:focus-visible {
		outline: 3px solid rgba(47, 122, 255, 0.42);
		outline-offset: 2px;
	}
