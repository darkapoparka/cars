<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Обадете се за работно време',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/avangard/vehicle-09-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="AVANGARD AUTO начало">
					<img src={resolve('/assets/avangard/wordmark.svg')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със AVANGARD AUTO'}
			</h1>
			<p>
				{isImportMode
		...
		{#if isImportMode}
			<p>Демонстрационна форма — не изпраща съобщение до автокъщата. За реално запитване: 0877 800 921.</p>
<form
	...
	<main id="main-content" tabindex="-1" class={{ 'mobile-contact-main--sheet': isImportMode }}>
		...
		<section class="mobile-contact-map" aria-label="Карта">
			...
			<iframe
				...
				title="Карта до AVANGARD AUTO Варна"
