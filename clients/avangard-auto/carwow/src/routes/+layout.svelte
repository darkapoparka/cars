<script lang="ts">
	...
	const dealerJsonLd = $derived(
		JSON.stringify({
			...
			image: `${origin}/assets/avangard/wordmark.svg`,
			logo: `${origin}/assets/avangard/wordmark.svg`,
			url: `${origin}/`,
			telephone: '0877800921',
			...(daynightSite.email ? { email: daynightSite.email } : {}),
			address: {
				'@type': 'PostalAddress',
				streetAddress: 'гр. Варна, Владислав Варненчик, бул. Цар Освободител 289',
