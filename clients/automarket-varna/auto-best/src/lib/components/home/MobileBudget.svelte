<script lang="ts">
  ...
  const budgetTiles = [
    ...
    {
      label: '5–10 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 5000 && vehicle.priceEur <= 10000).length} автомобила`,
      href: '/listing-grid?price_min=5000&price_max=10000',
      image: featuredVehicles[0].image
    },
    {
      label: 'Над 10 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 10000).length} автомобила`,
      href: '/listing-grid?price_min=10000',
