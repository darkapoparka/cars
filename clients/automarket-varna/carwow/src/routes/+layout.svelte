<script lang="ts">
	...
	const dealerJsonLd = $derived(
		JSON.stringify({
			...
			image: `${origin}/assets/automarket/cover.png`,
			logo: `${origin}/assets/automarket/cover.png`,
			url: `${origin}/`,
			telephone: '0886424400',
			...(daynightSite.email ? { email: daynightSite.email } : {}),
			address: {
				'@type': 'PostalAddress',
				streetAddress: 'гр. Варна, бул. Цар Освободител — 300 м вдясно след Дом на Камиона, посока летището',
