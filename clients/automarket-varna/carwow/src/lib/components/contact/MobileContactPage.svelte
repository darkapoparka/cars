<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			...
			value: 'Демо форма — за реален контакт се обадете',
			icon: MessageCircle
		},
		{
			id: 'address',
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'За работно време се обадете предварително.',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/automarket/vehicle-14-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Аутомаркет Варна начало">
					<img src={resolve('/assets/automarket/cover.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Аутомаркет Варна'}
			</h1>
			<p>
				{isImportMode
	...
	<main id="main-content" tabindex="-1" class={{ 'mobile-contact-main--sheet': isImportMode }}>
		...
		<section class="mobile-contact-map" aria-label="Карта">
			...
			<iframe
				...
				title="Карта до Аутомаркет Варна Варна"
