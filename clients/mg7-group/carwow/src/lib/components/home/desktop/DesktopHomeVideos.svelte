...
<section class="home-videos" aria-label="Снимки от обявите"><div class="daynight-home-container"><DesktopSectionHeading title="Огледайте в детайл" href="/inventory" label="Всички обяви"/><div class="home-videos__grid">{#each daynightVehicles.slice(0,3) as car (car.slug)}<article class="home-video"><a class="home-video__image" href={'/inventory/'+car.slug} aria-label={'Галерия: '+car.shortTitle}><img src={car.gallery[1] ?? car.image} alt={car.shortTitle} width="480" height="270" loading="lazy" style="object-fit:cover"/></a></article>{/each}</div></div></section>
<style>
	...
	.home-video__image img { display: block; width: 100%; height: 100%; min-height: 200px; border: 0; }
</style>
