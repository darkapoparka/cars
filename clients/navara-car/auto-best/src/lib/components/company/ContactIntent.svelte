<script lang="ts">
  ...
  import { brand, dealerMapUrl, dealerSource } from '$config/brand';
  ...
  const directionsUrl = dealerMapUrl;
  ...
  ] as const).filter(platform => Boolean(platform.href));
</script>
...
<div class="dn-contact-intent" class:dn-contact-intent--general={topic.id === 'general'} class:dn-contact-hero-panel={topic.id === 'general'} class:dn-contact-intent--workflow={topic.id === 'trade-in' || topic.id === 'import'}>
  <div class="dn-contact-intent__main">
    {#if topic.id === 'trade-in' || topic.id === 'import'}<h1 class="dn-contact-workflow-title">{topic.title}</h1>{/if}
    <div class="dn-contact-intent__heading">
      <h2><span class:dn-contact-mobile-copy={topic.id === 'general'}>Контакт с продавача</span>{#if topic.id === 'general'}<span class="dn-contact-desktop-copy">Обадете се на Навара кар</span>{/if}</h2>
    </div>
    {#if vehicle && topic.id !== 'leasing'}<ContactVehicle {vehicle} />{/if}
    <div class="dn-contact-selected">
      {#if topic.id !== 'general'}<h3>{topic.title}</h3>{/if}
    ...
    {:else if preparation}
      <div class="dn-contact-preparation">
        ...
        <ul>{#each preparation.items as item (item)}<li>{item}</li>{/each}</ul>
      </div>
    {/if}
    {#if topic.id === 'import' && importUrl}
      <div class="dn-contact-import" aria-label="Избрана обява">
        <strong>Линк към обява</strong>
        <a href={importUrl} target="_blank" rel="noopener noreferrer">{importUrl}<Icon name="arrow-right" size={18} /></a>
        <p>Линкът не е изпратен. Свържете се с продавача и уточнете дали може да съдейства по вашия въпрос.</p>
      </div>
    {/if}
    ...
    {#if topic.id !== 'trade-in' && topic.id !== 'import' && socialPlatforms.length > 0}
      <div class="dn-contact-social" role="group" aria-label="Социални мрежи">
        <span>Социални мрежи</span>
        <div>{#each socialPlatforms as platform (platform.name)}
          <a href={platform.href} target="_blank" rel="noopener noreferrer" aria-label={platform.label} title={platform.label}><SocialBrandIcon name={platform.name} /></a>
        {/each}</div>
      </div>
    {/if}
  </div>
  {#if topic.id === 'trade-in' || topic.id === 'import'}
    <a class="dn-contact-workflow-call dn-action--dark" href={brand.phoneHref}>
      <Icon name="phone" size={18} strokeWidth={1.8} />Обадете се · {brand.phone}
    </a>
  {/if}
  <aside class="dn-contact-card" aria-label="Контакти на автокъщата">
    <div class="dn-contact-card__heading">
      <h2><span class:dn-contact-mobile-copy={topic.id === 'general'}>Контакти</span>{#if topic.id === 'general'}<span class="dn-contact-desktop-copy">Посетете автокъщата</span>{/if}</h2>
      <p class:dn-contact-mobile-copy={topic.id === 'general'}>Публикуван адрес, телефон и ориентир за посещение.</p>
    </div>
    {#if topic.id === 'general'}
      <div class="dn-contact-visit">
        ...
        <a class="dn-contact-button" href={directionsUrl} target="_blank" rel="noreferrer">Търсене на адреса<Icon name="arrow-right" size={20} /></a>
      </div>
    {/if}
    <nav class="dn-contact-card__links" aria-label="Бързи действия за контакт">
      <a class="dn-contact-card__link" href={directionsUrl} target="_blank" rel="noreferrer" aria-label={`Потърсете публикувания адрес в Google Maps: ${brand.address}`}>
        <span class="dn-contact-card__icon"><Icon name="map-pin" size={24} strokeWidth={1.8} /></span>
        <span class="dn-contact-card__copy"><strong>Адрес</strong><span>{brand.address}</span></span>
        <span class="dn-contact-card__cue" aria-hidden="true"><Icon name="arrow-right" size={20} strokeWidth={1.8} /><span role="tooltip">Отвори карта</span></span>
      </a>
      <a class="dn-contact-card__link" href={brand.phoneHref} aria-label={`Обадете се за посещение. ${brand.appointment}`}>
        <span class="dn-contact-card__icon"><Icon name="clock" size={24} strokeWidth={1.8} /></span>
        <span class="dn-contact-card__copy"><strong>Посещения</strong><span>{brand.appointment}</span></span>
        <span class="dn-contact-card__cue" aria-hidden="true"><Icon name="arrow-right" size={20} strokeWidth={1.8} /><span role="tooltip">Уточни посещение</span></span>
      </a>
      <a class="dn-contact-card__link dn-contact-card__link--phone" href={brand.phoneHref} aria-label={`Обадете се на ${brand.phone}`}>
        <span class="dn-contact-card__icon"><Icon name="phone" size={24} strokeWidth={1.8} /></span>
        <span class="dn-contact-card__copy"><strong>Телефон</strong><span>{brand.phone}</span></span>
        <span class="dn-contact-card__cue" aria-hidden="true"><Icon name="arrow-right" size={20} strokeWidth={1.8} /><span role="tooltip">Позвъни</span></span>
      </a>
    </nav>
    <div class="dn-contact-card__actions">
      <a class="dn-contact-card__call" href={brand.phoneHref}><Icon name="phone" size={19} strokeWidth={1.8} />Обадете се</a>
      <a class="dn-contact-card__route" href={directionsUrl} target="_blank" rel="noreferrer"><Icon name="map-pin" size={19} strokeWidth={1.8} />Карта</a>
    </div>
    <p>{dealerSource.previewNotice}</p>
  </aside>
</div>
...
<style>
  ...
  .dn-contact-intent--workflow .dn-contact-intent__main > .dn-contact-button {
    min-height: 44px; align-self: center; width: auto; margin-top: 14px;
    padding: 8px 0; background: transparent; color: #525a66; font-size: 14px;
  ...
  @media (max-width: 767px) {
    ...
    .dn-contact-workflow-call {
      ...
      gap: 8px; min-height: 44px; max-width: 100%; margin-top: 16px; padding: 10px 16px;
      border-radius: var(--dn-radius-button); font-size: 14px; font-weight: 600;
  ...
  .dn-contact-import { display: grid; gap: 8px; min-width: 0; margin: 20px 0 0; text-align: left; }
  .dn-contact-import > a {
    display: flex; min-height: 44px; align-items: center; gap: 12px;
    color: var(--dn-red); overflow-wrap: anywhere; text-decoration: underline; text-underline-offset: 3px;
  }
  .dn-contact-import > p { margin: 0; color: #525a66; font-size: 14px; line-height: 1.5; }
</style>
