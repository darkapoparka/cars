<script lang="ts">
	...
	// Contact forms are local demonstrations; use the verified telephone for a real enquiry.
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Понеделник – събота: 09:00–17:30 · Неделя: почивен ден',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/legend-auto/vehicle-01-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="LEGEND AUTO начало">
					<img src={resolve('/assets/legend-auto/logo-original.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със LEGEND AUTO'}
			</h1>
			<p>
				{isImportMode
	...
	<main id="main-content" tabindex="-1" class={{ 'mobile-contact-main--sheet': isImportMode }}>
		...
		<section class="mobile-contact-map" aria-label="Карта">
			...
			<iframe
				...
				title="Карта до LEGEND AUTO Варна"
