<script lang="ts">
	...
	const dealerJsonLd = $derived(
		JSON.stringify({
			...
			image: `${origin}/assets/legend-auto/logo-original.png`,
			logo: `${origin}/assets/legend-auto/logo-original.png`,
			url: `${origin}/`,
			telephone: '0899877305',
			...(daynightSite.email ? { email: daynightSite.email } : {}),
			address: {
				'@type': 'PostalAddress',
				streetAddress: 'гр. Варна, бул. Цар Освободител 289, срещу МАКАО',
...
{#if /^\/(financing|calculator|sell-your-car|services|reviews|team)/.test(page.url.pathname)}<p role="note" class="client-service-note">Демонстрационна страница. LEGEND AUTO не е потвърдил тази услуга. За реални условия се обадете на 0899 877 305. Формите не изпращат съобщение до автокъщата.</p>{/if}
{@render children()}
...
<style>
	.client-service-note { margin: 0; padding: 12px 24px; background: #fff4e5; color: #573700; font-size: 14px; }
	:global(html) {
