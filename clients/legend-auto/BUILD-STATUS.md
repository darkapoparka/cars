...
- QA: pending. Allocated ports 6666 / 6667 / 6668. Initial Auto Best preview PID 736 exited with system OOM before listening. Task-specific agent-browser session legend-auto closed successfully. No owned preview/browser remains running.
- Resource coordination: after parent update, ALL preview/browser QA will run inside with-build-slot.ps1 as launch + QA + owned teardown jobs. No automatic OOM retries outside the queue.
- CRM: not registered. Public previews: none.
- Resume reconciliation: prior queued Modern job no longer exists and no owned client process/listener remains. Submitted one fresh Modern validation job through current wrapper; do not duplicate.
