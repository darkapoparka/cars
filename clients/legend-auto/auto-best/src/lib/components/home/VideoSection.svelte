...
<section class="dn-videos" aria-labelledby="videos-title"><div class="container"><div class="dn-videos__panel"><div class="dn-videos__heading dn-home-section-heading"><h2 id="videos-title" class="dn-home-section-title">От нашите обяви</h2><p>Оригинални снимки на автомобили от LEGEND AUTO.</p><a class="dn-videos__channel dn-home-section-action" href="/listing-grid">Всички автомобили <Icon name="arrow-right" size={16}/></a></div><div class="dn-videos__grid">{#each featuredVehicles.slice(0,3) as vehicle (vehicle.id)}<article class="dn-video-card"><div class="dn-video-card__media"><a class="dn-video-card__play" href={vehicle.href}><img src={vehicle.image} alt={vehicle.title} width="720" height="404" loading="lazy"/></a></div><div class="dn-video-card__content"><h3>{vehicle.title}</h3><div class="dn-video-card__actions"><a href={vehicle.href}>Снимки и подробности <Icon name="arrow-right" size={16}/></a></div></div></article>{/each}</div></div></div></section>
<style>
