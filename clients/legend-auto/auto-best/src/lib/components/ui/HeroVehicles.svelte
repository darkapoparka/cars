...
<div class="dn-hero-vehicles" class:dn-hero-vehicles--mobile={mobile} data-pair={pair} aria-hidden="true">
  {#if mobile}<img class="dn-hero-vehicles__front" src="/assets/images/lead/day-night-urus-front-v1.webp" alt="" width="600" height="600" decoding="async" />{/if}
  {#each sides as side (side)}
    ...
    <div class="dn-hero-vehicles__car dn-hero-vehicles__car--{side}"
      ...
      <VehicleCutout {vehicle} mobileVehicle={mobile ? mobileVehicle : undefined} eager />
    </div>
  {/each}
</div>
