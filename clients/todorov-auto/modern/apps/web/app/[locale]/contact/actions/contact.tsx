...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Автосалон Тодоров.",
          "The form is ready, but message delivery is not configured yet. Call Автосалон Тодоров directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Автосалон Тодоров.",
          "Message delivery is not configured yet. Call Автосалон Тодоров directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Автосалон Тодоров."
        : "Запитването е изпратено до екипа на Автосалон Тодоров.",
      "Your request has been sent to the Автосалон Тодоров team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Автосалон Тодоров: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
