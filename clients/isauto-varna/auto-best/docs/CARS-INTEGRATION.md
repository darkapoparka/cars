...
Standalone entry: /. Use Node 22.12+ on the 22 line and the retained npm lockfile. Run npm ci, npm run check and npm run build for relevant application changes. A documentation-only workflow change needs focused link/command checks, not the whole application suite.

Mounted dealer entry: /. Base paths, Services routing, locale/raw links, CSS/static assets, requests, redirects and the shared switcher are handled by the versioned Cars packaging layer for compatible releases. Standalone defaults stay standalone. Do not hardcode a dealer mount into this master or claim mounted QA from a standalone build. Unknown source shapes require a deliberate packaging update and tests.
...
The actual content boundaries include `src/lib/config/brand.ts`, `src/lib/config/template.ts`, `src/lib/data/`, `src/lib/styles/tokens.css`, `static/`. Read TEMPLATE.md and the reuse/QA references for complete technical detail. A fact-pack JSON is not an application setting unless code reads it. Source/demo identity and forms remain unverified until the dealer implementation establishes them.
