...
export const editorial = blogPosts.slice(0, 3).map(post => ({title: post.title, text: post.text, image: post.image, href: `/blog-detail/${post.id}`, meta: 'Guide', category: post.category }));
