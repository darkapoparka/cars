...
<div class="dn-enquiry-entry">
  ...
  {:else}
    ...
    <button class="dn-enquiry-alternative dn-action--dark" type="button" onclick={(event) => open(event, true)} aria-haspopup="dialog">I have no listing — describe your search <Icon name="arrow-right" size={17} /></button>
  {/if}
</div>
    ...
    <form class="dn-enquiry-body" bind:this={form} onsubmit={(event) => { event.preventDefault(); if (step < 2) void move(step + 1); }}>
      {#if step === 0}
        ...
        <div class="dn-enquiry-fields">
          ...
          {#if selling}<label>Mileage, km<input bind:value={mileage} name="mileage" inputmode="numeric" pattern={'[0-9]{1,7}'} maxlength={7} placeholder="E.g. 85000" /></label>{/if}
