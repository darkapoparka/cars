<script lang="ts">
  ...
  const searchLabel = $derived(field === 'make' ? 'Search makes' : field === 'model' ? 'Search models' : `Search in ${title.toLocaleLowerCase('en-AE')}`);
  ...
  const matchesSearch = (option: string) => search.trim().toLocaleLowerCase('en-AE').split(/\s+/).every(term => optionLabel(option).toLocaleLowerCase('en-AE').includes(term));
