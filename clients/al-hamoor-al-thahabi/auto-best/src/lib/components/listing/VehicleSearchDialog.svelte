<script lang="ts">
  ...
  const mobileFields = $derived([
    ...
    { field: 'mileage_max', label: 'Mileage', value: draftMileageMax ? `Up to ${draftMileageMax} km` : 'No limit' },
          ...
          <label>
            ...
            <select name="price_min" aria-label="Price from" bind:value={draftPriceMin}>
              ...
              {#each listingFilterOptions.prices as option (option)}
                <option value={option}>{option ? `From ${new Intl.NumberFormat('en-AE').format(Number(option))} €` : 'Price from'}</option>
              {/each}
            </select>
          </label>
          <label>
            ...
            <select name="price_max" aria-label="Price up to" bind:value={draftPriceMax}>
              ...
              {#each listingFilterOptions.prices as option (option)}
                <option value={option}>{option ? `Up to ${new Intl.NumberFormat('en-AE').format(Number(option))} €` : 'Price up to'}</option>
              {/each}
            </select>
          </label>
          <label>
          ...
          <label>
            ...
            <select name="mileage_max" aria-label="Mileage up to" bind:value={draftMileageMax}>
              ...
              {#each listingFilterOptions.mileages as option (option)}
                <option value={option}>{option ? `Up to ${new Intl.NumberFormat('en-AE').format(Number(option))} km` : 'Mileage up to'}</option>
              {/each}
            </select>
          </label>
          <label>
