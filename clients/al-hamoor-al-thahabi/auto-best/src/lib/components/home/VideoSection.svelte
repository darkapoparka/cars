...
<section class="dn-videos" aria-labelledby="videos-title">
  <div class="container"><div class="dn-videos__panel">
    <div class="dn-videos__heading dn-home-section-heading">
      <h2 id="videos-title" class="dn-home-section-title">A closer look</h2>
      <p>Source photographs of sampled vehicles. Dealer videos are not connected.</p>
      <a class="dn-videos__channel dn-home-section-action" href="/listing-grid">All vehicles <Icon name="arrow-right" size={16} /></a>
    </div>
    <div class="dn-videos__grid">
      {#each featuredVehicles.slice(0,3) as vehicle (vehicle.id)}
        <article class="dn-video-card">
          <div class="dn-video-card__media"><a class="dn-video-card__play" href={vehicle.href} aria-label={`View photographs of ${vehicle.title}`}><img src={vehicle.image} alt={vehicle.title} width="720" height="404" loading="lazy" decoding="async" /><span class="dn-video-card__duration">{vehicle.gallery.length} photographs</span></a></div>
          <div class="dn-video-card__content"><h3>{vehicle.title}</h3><div class="dn-video-card__actions"><a href={vehicle.href}>Photographs and details <Icon name="arrow-right" size={16} /></a></div></div>
        </article>
      {/each}
      <a class="dn-videos__all-card" href="/listing-grid"><span class="dn-videos__all-icon"><Icon name="car" size={40} /></span><strong>All sampled vehicles</strong><span>Browse specifications and original photographs.</span><span class="dn-videos__all-arrow">View catalogue <Icon name="arrow-right" size={18} /></span></a>
    </div>
  </div></div>
</section>
<style>
