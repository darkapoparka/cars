...
<svelte:head>
  ...
  <meta
    name="description"
    content={`${data.vehicle.title}, ${data.vehicle.year}, ${data.vehicle.mileage}. Dated listing sample from ${brand.name}; confirm availability and viewing location.`}
  />
</svelte:head>
        ...
        <div class="dn-detail-layout">
          <div class="dn-detail-main">
            ...
            <div class="dn-detail-card dn-detail-media-card">
              ...
              <div class="dn-detail-gallery-controls" aria-label="Vehicle photographs">
                <button type="button" onclick={() => changePhoto(photoIndex - 1)} disabled={gallery.length < 2} aria-label="Previous photograph">Previous</button>
                <span aria-live="polite">Photograph {photoIndex + 1} of {gallery.length}</span>
                <button type="button" onclick={() => changePhoto(photoIndex + 1)} disabled={gallery.length < 2} aria-label="Next photograph">Next</button>
              </div>
              <div class="dn-detail-gallery-thumbs">
                {#each gallery as photo, index (photo)}
                  <button type="button" aria-label={`Show photograph ${index + 1}`} aria-pressed={photoIndex === index} onclick={() => changePhoto(index)}><img src={photo} alt="" width="120" height="90" loading="lazy" /></button>
                {/each}
              </div>
            </div>
          ...
          <aside class="dn-detail-sidebar" aria-label="Vehicle information">
            <section class="dn-detail-card dn-detail-summary">
              ...
              <p class="dn-detail-summary__availability">Dated advertised sample. Confirm availability, condition and final price directly.</p>
              <div class="dn-detail-summary__actions">
                <a class="dn-detail-button dn-detail-button--primary" {...phoneLinkAttributes}>Call us</a>
                <a class="dn-detail-button dn-detail-button--dark" href={resolve(vehicleContactHref(data.vehicle.id))}>Request a vehicle viewing</a>
              </div>
            </section>

            <section class="dn-detail-card dn-detail-finance-card" aria-label="Finance calculator">
              {#key data.vehicle.id}
                {#if data.vehicle.priceAmount !== null}
                  <VehicleFinanceCalculator priceAmount={data.vehicle.priceAmount} vehicleId={data.vehicle.id} />
                {:else}
                  <p>Price on request. No payment estimate is available.</p>
                {/if}
              {/key}
            </section>
            ...
            <section class="dn-detail-card dn-detail-dealer" aria-labelledby="seller-title">
              ...
              <div class="dn-detail-dealer__actions">
                <a class="dn-detail-button dn-detail-button--dark" href={resolve(vehicleContactHref(data.vehicle.id))}>Discuss this vehicle</a>
              </div>
            </section>
        ...
        <section class="dn-detail-related" aria-labelledby="related-title">
          <div class="dn-detail-related__header">
            <div>
              ...
              <p>More dated listing samples</p>
            </div>
