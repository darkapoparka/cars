...
function renderFooterSocialLinks() {
	...
	const links: readonly PresentationIconLink[] = [
		...
		},
		{
			...
			title: 'Отвори локацията на icars в Google Maps',
			kind: 'external',
...
<p class="text-sm text-muted mb-16">icars предлага актуална дигитална витрина с наличност, запитвания и ясни следващи стъпки.</p>
${renderFooterSocialLinks()}
</div>
<div>
<p class="text-sm text-white font-weight-600 mb-8">Имате въпрос?</p>
<div class="flex items-center gap-12">
<a href="/contact" class="btn btn-line-1 btn-small" title="Свържете се със icars">Свържете се</a>
</div>
</div>
</div>
</div>
...
<p class="text-sm text-muted">©2026 <a class="text-sm text-white" href="/">icars</a>. Демонстрационна концепция. Не е официалният сайт.</p>
...
<a href="tel:${daynightSite.phone}" class="icon" aria-label="Обади се на icars">${phoneIcon()}</a>
...
<a href="${daynightSite.mapUrl}" class="icon" target="_blank" rel="noopener" aria-label="Отвори карта до icars">${locationIcon()}</a>
<div class="daynight-header-contact__body">
...
<a class="daynight-map-preview" href="${daynightSite.mapUrl}" target="_blank" rel="noopener" aria-label="Отвори Google карта до icars">
