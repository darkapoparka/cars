...
These are exact proposed entries, not running servers. After safe integration into a clean/preserved coordinator workspace, use scripts/start-client.ps1 -Client voivodov-auto-antonio -Prepare, then scripts/start-client.ps1 -Client voivodov-auto-antonio. The launcher checks its own listeners and ports; do not kill unrelated processes. No active server or port availability is claimed here. Modern starts from apps/web in its retained monorepo, using its existing static demo configuration and distinct local origins.
