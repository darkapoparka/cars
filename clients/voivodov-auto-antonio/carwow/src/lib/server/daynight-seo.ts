...
export function vehicleSeo(vehicle: DayNightVehicle): PageSeo {
	...
	return {
		title: `${vehicle.title} | VOIVODOV AUTO & ANTONIO`,
		description: `${vehicle.title}${facts.length ? ` - ${facts.join(', ')}` : ''}. Проверен автомобил от VOIVODOV AUTO & ANTONIO с опция за финансиране.`
	};
}
