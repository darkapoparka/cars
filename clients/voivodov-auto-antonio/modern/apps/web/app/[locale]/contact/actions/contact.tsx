...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на VOIVODOV AUTO & ANTONIO.",
          "The form is ready, but message delivery is not configured yet. Call VOIVODOV AUTO & ANTONIO directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на VOIVODOV AUTO & ANTONIO.",
          "Message delivery is not configured yet. Call VOIVODOV AUTO & ANTONIO directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на VOIVODOV AUTO & ANTONIO."
        : "Запитването е изпратено до екипа на VOIVODOV AUTO & ANTONIO.",
      "Your request has been sent to the VOIVODOV AUTO & ANTONIO team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `VOIVODOV AUTO & ANTONIO: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
