...
// Dated seller-advertised sample used by the existing static dealer mode.
// publishedAt is the snapshot observation date, not the seller's original publication timestamp.
const bodyMap: Record<string,VehicleListing['spec']['bodyType']> = {'Джип':'suv','Седан':'sedan','Хечбек':'hatchback','Комби':'wagon','Купе':'coupe','Миниван':'van','Кабрио':'convertible'};
const fuelMap: Record<string,VehicleListing['spec']['fuelType']> = {'Дизелов':'diesel','Бензинов':'gasoline','Хибриден':'hybrid','Електрически':'electric','Газ/Бензин':'lpg'};
export const mockListings: VehicleListing[] = dealer.vehicles.map(v=>({id:v.sourceId,slug:`${v.make}-${v.model}-${v.sourceId.slice(-6)}`.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/-$/,''),category:'car',dealerOrgId:dealer.slug,status:'active',title:v.title,description:v.description+' Източник: '+v.sourceUrl,price:{amount:v.price,currency:'EUR'},priceType:'fixed',images:v.images.map(url=>({url,alt:v.title})),badges:['used'],location:{city:v.location,region:dealer.city,country:'България'},features:v.equipment.map(bg=>({bg,en:bg})),spec:{make:v.make,model:v.model,year:v.year,bodyType:bodyMap[v.body]??'other',fuelType:fuelMap[v.fuel]??'other',transmission:v.transmission==='Автоматична'?'automatic':'manual',mileageValue:v.mileageKm,mileageUnit:'km',enginePowerHp:Number.parseInt(v.power,10)||undefined,colorExterior:v.color},seller:{id:dealer.slug,type:'dealer',displayName:dealer.name,verificationStatus:'unverified',city:dealer.city,logoUrl:'/dealer/logo-light.png'},publishedAt:dealer.observedAt+'T00:00:00.000Z',promoted:false}));
