...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Champion Auto Pro.",
          "The form is ready, but message delivery is not configured yet. Call Champion Auto Pro directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Champion Auto Pro.",
          "Message delivery is not configured yet. Call Champion Auto Pro directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Champion Auto Pro."
        : "Запитването е изпратено до екипа на Champion Auto Pro.",
      "Your request has been sent to the Champion Auto Pro team."
    ),
  };
};
    ...
    deliver: async (request, context) => {
      ...
      return await getReliableEmailDelivery().deliver({
        ...
        payload: {
          ...
          subject: `Champion Auto Pro: ${topic.en}`,
          to: env.RESEND_FROM,
        },
      });
    },
