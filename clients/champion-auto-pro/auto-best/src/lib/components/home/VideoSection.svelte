  ...
  <div class="container">
    <div class="dn-videos__panel">
      <div class="dn-videos__heading dn-home-section-heading">
        <h2 id="videos-title" class="dn-home-section-title">Автомобили отблизо</h2>
        <p>Снимки от публикуваните обяви на Champion Auto Pro.</p>
        <a class="dn-videos__channel dn-home-section-action" href="https://championautopro.mobile.bg/" target="_blank" rel="noopener noreferrer">
          ...
          <article class="dn-video-card">
            <div class="dn-video-card__media">
              <button
                ...
                aria-label={`Разгледайте: ${video.title}`}
              ...
              {#if activeVideo === video.id}
                <img src={video.thumbnail} alt={video.title} width="720" height="404" />
              {/if}
            </div>
            <div class="dn-video-card__content">
              <h3>{video.title}</h3>
              <div class="dn-video-card__actions">
                <a href={video.id} target="_blank" rel="noopener noreferrer" aria-label={`${video.title} — гледайте в Mobile.bg (нов раздел)`}>
                  Вижте обявата <Icon name="arrow-right" size={16} />
                </a>
                {#if activeVideo === video.id}
...
<style>
  ...
  .dn-video-card__media > img { position: absolute; inset: 0; width: 100%; height: 100%; border: 0; }
