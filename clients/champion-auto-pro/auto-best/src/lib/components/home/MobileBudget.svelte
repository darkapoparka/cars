<script lang="ts">
  ...
  const budgetTiles = [
    ...
    {
      label: '20–40 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 20000 && vehicle.priceEur <= 40000).length} автомобила`,
      href: '/listing-grid?price_min=20000&price_max=40000',
      image: featuredVehicles[1].image
    },
    {
      label: 'Над 40 000 €',
      detail: `${featuredVehicles.filter((vehicle) => vehicle.priceEur > 40000).length} автомобила`,
      href: '/listing-grid?price_min=40000',
