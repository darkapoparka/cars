import championStock from './champion-stock.json';
...
const listingToVehicle = (listing: CurrentDayNightListing): Car => {
	const original = championStock.find(v => v.id === listing.id)!;
	...
	const conditionLine = isIncoming
		...
		: 'Публикувана обява — оглед във Варна — свържете се за оглед.';
