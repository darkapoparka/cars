<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Понеделник – петък: 09:30–18:00 · Събота: 10:00–15:00 · Неделя: почивен ден',
			href: null,
			icon: Clock
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		...
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Champion Auto Pro начало">
					<img src={resolve('/assets/champion/logo.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Champion Auto Pro'}
			</h1>
			<p>
				{isImportMode
	...
	<main id="main-content" tabindex="-1" class={{ 'mobile-contact-main--sheet': isImportMode }}>
		...
		<section class="mobile-contact-map" aria-label="Карта">
			...
			<iframe
				...
				title="Карта до Champion Auto Pro Варна"
