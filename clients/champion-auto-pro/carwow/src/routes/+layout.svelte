<script lang="ts">
	...
	const dealerJsonLd = $derived(
		JSON.stringify({
			...
			image: `${origin}/assets/champion/logo.png`,
			logo: `${origin}/assets/champion/logo.png`,
			url: `${origin}/`,
			telephone: '0885072555',
			...(daynightSite.email ? { email: daynightSite.email } : {}),
			address: {
				'@type': 'PostalAddress',
				streetAddress: 'гр. Варна, Цар Освободител 302, бул. Цар Освободител 302',
