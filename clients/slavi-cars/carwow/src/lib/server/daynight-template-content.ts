		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Dealer Listing', 'Профил на Slavi Cars Дупница')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'Slavi Cars публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'Slavi Cars')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на Slavi Cars')
					.replaceAll('Sophia Carter', 'Клиент от Дупница')
					.replaceAll('Contact', 'Контакти')
					.replaceAll('About', 'За нас')
					.replaceAll('News', 'Полезно')
					.replaceAll('Контакти Us', 'Контакти')
					.replaceAll('Heather Dick', 'Клиент на Slavi Cars')
					.replaceAll(
...
const selectedTemplateFileToRoute: Record<string, string> = {
	...
	'dealers-listing.html': '/about/slavi-cars',
	'dealer-details.html': '/about/slavi-cars',
	'about-us.html': '/about',
...
const daynightPageAssets = {
	aboutExterior: '/dealer/stock/21784972577846700-1.webp',
	aboutConsultation: '/dealer/stock/21784972577846700-1.webp',
	aboutShowroomVehicle: '/dealer/stock/21784972577846700-1.webp',
	servicesInspection: '/dealer/stock/21784972577846700-1.webp',
	servicesConsultation: '/dealer/stock/21784972577846700-1.webp',
...
> = {
	'about-us.html': {
		...
		title: 'За Slavi Cars',
		description:
			'Подбрани обяви, ясна история и съдействие от избора до документите в Дупница.',
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до Slavi Cars Дупница" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderTeamContactActions(member: (typeof daynightTeam)[number]) {
	...
	return `<ul class="contact">
								...
								<li>
									<a href="mailto:${member.email}" title="Имейл към Slavi Cars" aria-label="Имейл към Slavi Cars">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							...
							<li>
								<a href="tel:${daynightSite.phone}" class="hover-stroke-white" title="Свържете се с Slavi Cars по телефон или Viber." aria-label="Свържете се с Slavi Cars по телефон или Viber.">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на Slavi Cars в Google Maps" aria-label="Отвори локацията на Slavi Cars в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до Slavi Cars Дупница" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
	...
	<div class="container">
		<div class="daynight-about-location">
			<div class="daynight-about-location__content">
				<p class="eyebrow">Шоурум в Дупница</p>
				<h2>Автомобили с ясна история и подреден процес</h2>
				<p class="text-secondary h7 line-height-28">Slavi Cars предлага внимателно подбрани автомобили с реална наличност, съдействие при оглед, документи, регистрация, финансиране и бартер.</p>
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на Slavi Cars">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със Slavi Cars">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от Slavi Cars', '');
...
function replacePdpCopy(html: string, routePath: string) {
	...
	let output = html
		...
		.replaceAll('Mike Hanley', salesTeam?.name ?? 'Slavi Cars')
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от Slavi Cars.</p>`;
	}
...
function renderBlogDetailMetaList(article: DayNightArticle) {
	const items = [
		{ type: 'author' as const, label: article.author, href: '/about/slavi-cars' },
...
function renderBlogDetailAuthorBox() {
	...
	<div class="listing-details--contact-dealer mb-20">
		...
		<div class="content">
			<a href="/about/slavi-cars" class="h4 mb-4 font-weight-600">Slavi Cars</a>
			<p class="text-secondary mb-18">Автокъща в Дупница</p>
			<ul class="blog-detail-social flex gap-12">
				<li></li>
				<li></li>
				<li><a href="mailto:${daynightSite.email}" aria-label="Имейл към Slavi Cars">${renderBlogSocialIcon('mail')}</a></li>
			</ul>
		</div>
	</div>
	<p class="h7 line-height-28">Slavi Cars публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Дупница.</p>
</div>`;
}
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'Slavi Cars')
		...
		.replaceAll('Nelson Mandela', 'Slavi Cars')
		.replaceAll(
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За Slavi Cars')
			.replaceAll(
				'Premium vehicles for every lifestyle',
				'проверени употребявани автомобили, съдействие за документи, регистрация, финансиране и огледи в Дупница.'
			)
			...
			.replaceAll(
				'At Slavi Cars, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на Slavi Cars помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на Slavi Cars')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на Slavi Cars')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">Slavi Cars предлага внимателно подбрани и подбрани обяви с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At Slavi Cars, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'Slavi Cars предлага внимателно подбрани и подбрани обяви с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от Slavi Cars')
			.replaceAll('Certified Dealers', 'Slavi Cars')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на Slavi Cars ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на Slavi Cars ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със Slavi Cars')
			.replaceAll(
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на Slavi Cars уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със Slavi Cars за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от Slavi Cars'
			)
			.replaceAll(
				...
				'Съдействаме при оглед, документи, финансиране, бартер и регистрация в Дупница.'
			)
			.replaceAll(
				'Experience hassle-free car shopping with our app. Browse, compare, and buy used cars wherever you are – it’s fast, simple, and convenient.',
				'Съдействаме при оглед, документи, финансиране, бартер и регистрация в Дупница.'
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на Slavi Cars')
			.replaceAll(
			...
			.replaceAll('Emily Johnson', 'Клиент на Slavi Cars')
			.replaceAll('Benjamin Parker', 'Клиент от Дупница')
			.replaceAll('Olivia Williams', 'Клиент на Slavi Cars')
			.replaceAll('Sophia Martinez', 'Клиент от Дупница')
			.replaceAll('Daniel Wright', 'Клиент на Slavi Cars')
			.replaceAll('Sarah Nguyen', 'Клиент от Дупница')
			.replaceAll('Manager, NexTech', 'Клиент на Slavi Cars')
			.replaceAll('Freelance Designer', 'Клиент от Дупница')
			.replaceAll('Entrepreneur', 'Клиент на Slavi Cars')
			.replaceAll('Accountant', 'Клиент от Дупница')
			.replaceAll('CEO BMW', 'Клиент на Slavi Cars')
			.replaceAll('CEO Slavi Cars', 'Клиент на Slavi Cars')
			.replaceAll('James Anderson', 'Клиент от Дупница')
			.replaceAll('Project Manager', 'Клиент на Slavi Cars')
			.replaceAll('Avitex', 'Slavi Cars')
			.replaceAll('Tesla', 'Slavi Cars')
			.replaceAll('Aurexo', 'Slavi Cars');
	}

	if (templateFile === 'sale-agents.html') {
		let output = html
			.replaceAll('Sale Agents', 'Екипът на Slavi Cars')
			.replaceAll('Our Agents', 'Консултанти')
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на Slavi Cars')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на Slavi Cars'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на Slavi Cars');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на Slavi Cars')
			...
			.replaceAll('Randynox', 'Клиент на Slavi Cars')
			.replaceAll('Mista Nyroom', 'Клиент от Дупница')
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на Slavi Cars Дупница')
			.replaceAll('Dealer Details', 'Профил на Slavi Cars Дупница')
			.replaceAll('Dealer Detail', 'Профил на Slavi Cars Дупница')
			.replaceAll('Aurexo Atlanta', 'Slavi Cars Дупница')
			.replaceAll('Euro Workshop', 'Slavi Cars Дупница')
			...
			.replaceAll(
				...
				'Slavi Cars Дупница предлага проверени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със Slavi Cars')
			.replaceAll('get in touch', 'Свържете се със Slavi Cars')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със Slavi Cars<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Дупница, Дупница, път E79, след отбивката за Блатино, Дупница<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Дупница, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на Slavi Cars.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на Slavi Cars за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на Slavi Cars съдейства с варианти за финансиране, лизинг и собствено разсрочено плащане според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на Slavi Cars предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. Slavi Cars прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'Slavi Cars полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'Slavi Cars може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Огледът и тестът се организират предварително по уговорка в Дупница.'
				)
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със Slavi Cars преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към Slavi Cars')
			.replaceAll('Submit Listing', 'Изпрати към Slavi Cars')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Brooklyn Simmons', 'Клиент от Дупница')
			.replaceAll('Arlene McCoy', 'Клиент от Дупница')
			.replaceAll('Darrell Steward', 'Slavi Cars')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'Slavi Cars')
			.replaceAll('Slavi Cars Support', 'Slavi Cars')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('Slavi Cars') ? '' : ' Slavi Cars');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('Why Choose Us?', 'Защо Slavi Cars?')
		.replaceAll('Why Choose Us', 'Защо Slavi Cars')
		.replaceAll(
		...
		.replaceAll('Certified Dealers', 'Slavi Cars')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of Slavi Cars who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на Slavi Cars ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със Slavi Cars')
		.replaceAll('Reach Out To Us', 'Свържете се със Slavi Cars')
		.replaceAll('Get In Touch', 'Свържете се със Slavi Cars')
		.replaceAll('get in touch', 'Свържете се със Slavi Cars')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от Slavi Cars.'
		)
		.replaceAll(
		...
		.replaceAll('Follow Us On social media:', 'Последвайте Slavi Cars:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на Slavi Cars.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със Slavi Cars преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'Slavi Cars може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'Slavi Cars')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към Slavi Cars')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към Slavi Cars')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към Slavi Cars')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към Slavi Cars')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на Slavi Cars')
		...
		.replaceAll('Verified Dealer', 'Проверена автокъща')
		.replaceAll('Emily Johnson', 'Клиент на Slavi Cars')
		.replaceAll('CEO Avitex', 'Клиент на Slavi Cars')
		.replaceAll('Benjamin Parker', 'Клиент от Дупница')
		.replaceAll('CEO Tesla', 'Клиент от Дупница')
		.replaceAll('Olivia Williams', 'Клиент на Slavi Cars')
		.replaceAll('CEO BMW', 'Клиент на Slavi Cars')
		.replaceAll('CEO Slavi Cars', 'Клиент на Slavi Cars')
		.replaceAll('James Anderson', 'Клиент от Дупница')
		.replaceAll('Project Manager', 'Клиент на Slavi Cars')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'Slavi Cars Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'Slavi Cars Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'Slavi Cars Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'Slavi Cars Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'Slavi Cars Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'Slavi Cars Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'Slavi Cars Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'Slavi Cars Sales Team')
		.replaceAll('Los Angeles,California', 'Дупница, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на Slavi Cars')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Автомобил по обява от Slavi Cars.')
		.replaceAll('How the adventure ended will be seen soon.', 'Автомобил по обява от Slavi Cars.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със Slavi Cars за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към Slavi Cars">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на Slavi Cars"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на Slavi Cars"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">Slavi Cars<\/a>/g,
				'<a href="/about/slavi-cars" class="h4 mb-8 font-weight-600">Slavi Cars</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Дупница, Дупница, път E79, след отбивката за Блатино, Дупница\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на Slavi Cars'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0895 714 484" class="text-sm text-white">0895 714 484<\/a>\s*<a href="tel:0895 714 484" class="text-sm text-white">0895 714 484<\/a>/,
				'<a href="tel:0895 714 484" class="text-sm text-white">0895 714 484</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">Slavi Cars<\/a>/g,
				'<a class="text-white" href="/about/slavi-cars">Slavi Cars</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'Slavi Cars Дупница')
		.replaceAll('Aurexo |', 'Slavi Cars |');
}
