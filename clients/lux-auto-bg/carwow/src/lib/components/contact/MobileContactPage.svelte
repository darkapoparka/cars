...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/dealer/inventory/11776195342993015-1.webp')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="ЛУКС АВТО начало">
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със ЛУКС АВТО'}
			</h1>
			<p>
				{isImportMode
		...
		{#if isImportMode}
			<nav class="mobile-import-disclosures" aria-label="Повече за вноса">
				<MobilePromoCard
					...
					label="Виж стъпките"
					image="/assets/dealer/inventory/21782618805725512-1.webp"
					tone="red"
