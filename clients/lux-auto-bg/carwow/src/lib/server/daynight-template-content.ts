		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Tampa, FL', 'Стара Загора')
					.replaceAll('GA 30328', '')
					...
					.replaceAll('Dealer Listing', 'Профил на ЛУКС АВТО Стара Загора')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'ЛУКС АВТО публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'ЛУКС АВТО')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на ЛУКС АВТО')
					.replaceAll('Sophia Carter', 'Клиент от Стара Загора')
					.replaceAll('Contact', 'Контакти')
					.replaceAll('About', 'За нас')
					.replaceAll('News', 'Полезно')
					.replaceAll('Контакти Us', 'Контакти')
					.replaceAll('Heather Dick', 'Клиент на ЛУКС АВТО')
					.replaceAll(
...
const daynightPageAssets = {
	aboutExterior: '/assets/dealer/inventory/11776195342993015-1.webp',
	aboutConsultation: '/assets/dealer/inventory/11776195342993015-1.webp',
	aboutShowroomVehicle: '/assets/dealer/inventory/11776195342993015-1.webp',
	servicesInspection: '/assets/dealer/inventory/11776195342993015-1.webp',
	servicesConsultation: '/assets/dealer/inventory/11776195342993015-1.webp',
	dealerProfileMark: '/assets/dealer/brand/logo.jpg'
} as const;
...
> = {
	'about-us.html': {
		...
		title: 'За ЛУКС АВТО',
		description:
			'Проверени автомобили, ясна история и съдействие от избора до документите в Стара Загора.',
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до ЛУКС АВТО Стара Загора" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderTeamContactActions(member: (typeof daynightTeam)[number]) {
	...
	return `<ul class="contact">
								...
								<li>
									<a href="mailto:${member.email}" title="Имейл към ЛУКС АВТО" aria-label="Имейл към ЛУКС АВТО">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							<li>
								<a href="https://luxauto.mobile.bg/" target="_blank" rel="noopener" class="hover-fill-white" title="Facebook" aria-label="Facebook">
							...
							<li>
								<a href="https://luxauto.mobile.bg/" target="_blank" rel="noopener" class="hover-stroke-white" title="Instagram" aria-label="Instagram">
									<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="tel:${daynightSite.phone}" class="hover-stroke-white" title="Свържете се с ЛУКС АВТО по телефон или Viber." aria-label="Свържете се с ЛУКС АВТО по телефон или Viber.">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на ЛУКС АВТО в Google Maps" aria-label="Отвори локацията на ЛУКС АВТО в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до ЛУКС АВТО Стара Загора" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
	...
	<div class="container">
		<div class="daynight-about-location">
			<div class="daynight-about-location__content">
				<p class="eyebrow">Шоурум в Стара Загора</p>
				<h2>Автомобили с ясна история и подреден процес</h2>
				<p class="text-secondary h7 line-height-28">ЛУКС АВТО предлага внимателно подбрани автомобили с реална наличност, съдействие при оглед, документи, регистрация, финансиране и бартер.</p>
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на ЛУКС АВТО">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със ЛУКС АВТО">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от ЛУКС АВТО', '');
...
function replacePdpCopy(html: string, routePath: string) {
	...
	let output = html
		...
		.replaceAll('Mike Hanley', salesTeam?.name ?? 'ЛУКС АВТО')
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от ЛУКС АВТО.</p>`;
	}
...
function renderBlogDetailSocialList(article: DayNightArticle) {
	...
	const shareUrl = encodeURIComponent(`https://luxauto.mobile.bg${blogArticleRoute(article)}`);
...
function renderBlogDetailAuthorBox() {
	...
	<div class="listing-details--contact-dealer mb-20">
		...
		<div class="content">
			<a href="/about/daynight-auto-plovdiv" class="h4 mb-4 font-weight-600">ЛУКС АВТО</a>
			<p class="text-secondary mb-18">Автокъща в Стара Загора</p>
			<ul class="blog-detail-social flex gap-12">
				<li><a href="https://luxauto.mobile.bg/" target="_blank" rel="noopener" aria-label="Facebook канал">${renderBlogSocialIcon('facebook')}</a></li>
				<li><a href="https://luxauto.mobile.bg/" target="_blank" rel="noopener" aria-label="Instagram канал">${renderBlogSocialIcon('instagram')}</a></li>
				<li><a href="mailto:${daynightSite.email}" aria-label="Имейл към ЛУКС АВТО">${renderBlogSocialIcon('mail')}</a></li>
			</ul>
		</div>
	</div>
	<p class="h7 line-height-28">ЛУКС АВТО публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Стара Загора.</p>
</div>`;
}
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'ЛУКС АВТО')
		...
		.replaceAll('Nelson Mandela', 'ЛУКС АВТО')
		.replaceAll(
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За ЛУКС АВТО')
			.replaceAll(
				'Premium vehicles for every lifestyle',
				'проверени употребявани автомобили, съдействие за документи, регистрация, финансиране и огледи в Стара Загора.'
			)
			...
			.replaceAll(
				'At ЛУКС АВТО, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на ЛУКС АВТО помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на ЛУКС АВТО')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на ЛУКС АВТО')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">ЛУКС АВТО предлага внимателно подбрани и проверени автомобили с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At ЛУКС АВТО, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'ЛУКС АВТО предлага внимателно подбрани и проверени автомобили с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от ЛУКС АВТО')
			.replaceAll('Certified Dealers', 'ЛУКС АВТО')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на ЛУКС АВТО ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на ЛУКС АВТО ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със ЛУКС АВТО')
			.replaceAll(
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на ЛУКС АВТО уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със ЛУКС АВТО за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от ЛУКС АВТО'
			)
			.replaceAll(
				...
				'Съдействаме при оглед, документи, финансиране, бартер и регистрация в Стара Загора.'
			)
			.replaceAll(
				'Experience hassle-free car shopping with our app. Browse, compare, and buy used cars wherever you are – it’s fast, simple, and convenient.',
				'Съдействаме при оглед, документи, финансиране, бартер и регистрация в Стара Загора.'
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на ЛУКС АВТО')
			.replaceAll(
			...
			.replaceAll('Emily Johnson', 'Клиент на ЛУКС АВТО')
			.replaceAll('Benjamin Parker', 'Клиент от Стара Загора')
			.replaceAll('Olivia Williams', 'Клиент на ЛУКС АВТО')
			.replaceAll('Sophia Martinez', 'Клиент от Стара Загора')
			.replaceAll('Daniel Wright', 'Клиент на ЛУКС АВТО')
			.replaceAll('Sarah Nguyen', 'Клиент от Стара Загора')
			.replaceAll('Manager, NexTech', 'Клиент на ЛУКС АВТО')
			.replaceAll('Freelance Designer', 'Клиент от Стара Загора')
			.replaceAll('Entrepreneur', 'Клиент на ЛУКС АВТО')
			.replaceAll('Accountant', 'Клиент от Стара Загора')
			.replaceAll('CEO BMW', 'Клиент на ЛУКС АВТО')
			.replaceAll('CEO ЛУКС АВТО', 'Клиент на ЛУКС АВТО')
			.replaceAll('James Anderson', 'Клиент от Стара Загора')
			.replaceAll('Project Manager', 'Клиент на ЛУКС АВТО')
			.replaceAll('Avitex', 'ЛУКС АВТО')
			.replaceAll('Tesla', 'ЛУКС АВТО')
			.replaceAll('Aurexo', 'ЛУКС АВТО');
	}

	if (templateFile === 'sale-agents.html') {
		let output = html
			.replaceAll('Sale Agents', 'Екипът на ЛУКС АВТО')
			.replaceAll('Our Agents', 'Консултанти')
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на ЛУКС АВТО')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на ЛУКС АВТО'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на ЛУКС АВТО');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на ЛУКС АВТО')
			...
			.replaceAll('Randynox', 'Клиент на ЛУКС АВТО')
			.replaceAll('Mista Nyroom', 'Клиент от Стара Загора')
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на ЛУКС АВТО Стара Загора')
			.replaceAll('Dealer Details', 'Профил на ЛУКС АВТО Стара Загора')
			.replaceAll('Dealer Detail', 'Профил на ЛУКС АВТО Стара Загора')
			.replaceAll('Aurexo Atlanta', 'ЛУКС АВТО Стара Загора')
			.replaceAll('Euro Workshop', 'ЛУКС АВТО Стара Загора')
			...
			.replaceAll(
				...
				'ЛУКС АВТО Стара Загора предлага проверени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със ЛУКС АВТО')
			.replaceAll('get in touch', 'Свържете се със ЛУКС АВТО')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със ЛУКС АВТО<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Стара Загора, Стара Загора, ул. Христина Морфова 4, кв. Кольо Ганчев, Стара Загора<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Стара Загора, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на ЛУКС АВТО.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на ЛУКС АВТО за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на ЛУКС АВТО съдейства с варианти за финансиране, лизинг и собствено разсрочено плащане според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на ЛУКС АВТО предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. ЛУКС АВТО прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'ЛУКС АВТО полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'ЛУКС АВТО може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Огледът и тестът се организират предварително по уговорка в Стара Загора.'
				)
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със ЛУКС АВТО преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към ЛУКС АВТО')
			.replaceAll('Submit Listing', 'Изпрати към ЛУКС АВТО')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Brooklyn Simmons', 'Клиент от Стара Загора')
			.replaceAll('Arlene McCoy', 'Клиент от Стара Загора')
			.replaceAll('Darrell Steward', 'ЛУКС АВТО')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'ЛУКС АВТО')
			.replaceAll('ЛУКС АВТО Support', 'ЛУКС АВТО')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('ЛУКС АВТО') ? '' : ' ЛУКС АВТО');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('Why Choose Us?', 'Защо ЛУКС АВТО?')
		.replaceAll('Why Choose Us', 'Защо ЛУКС АВТО')
		.replaceAll(
		...
		.replaceAll('Certified Dealers', 'ЛУКС АВТО')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of ЛУКС АВТО who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на ЛУКС АВТО ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със ЛУКС АВТО')
		.replaceAll('Reach Out To Us', 'Свържете се със ЛУКС АВТО')
		.replaceAll('Get In Touch', 'Свържете се със ЛУКС АВТО')
		.replaceAll('get in touch', 'Свържете се със ЛУКС АВТО')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от ЛУКС АВТО.'
		)
		.replaceAll(
		...
		.replaceAll('Follow Us On social media:', 'Последвайте ЛУКС АВТО:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на ЛУКС АВТО.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със ЛУКС АВТО преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'ЛУКС АВТО може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'ЛУКС АВТО')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към ЛУКС АВТО')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към ЛУКС АВТО')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към ЛУКС АВТО')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към ЛУКС АВТО')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на ЛУКС АВТО')
		...
		.replaceAll('Verified Dealer', 'Проверена автокъща')
		.replaceAll('Emily Johnson', 'Клиент на ЛУКС АВТО')
		.replaceAll('CEO Avitex', 'Клиент на ЛУКС АВТО')
		.replaceAll('Benjamin Parker', 'Клиент от Стара Загора')
		.replaceAll('CEO Tesla', 'Клиент от Стара Загора')
		.replaceAll('Olivia Williams', 'Клиент на ЛУКС АВТО')
		.replaceAll('CEO BMW', 'Клиент на ЛУКС АВТО')
		.replaceAll('CEO ЛУКС АВТО', 'Клиент на ЛУКС АВТО')
		.replaceAll('James Anderson', 'Клиент от Стара Загора')
		.replaceAll('Project Manager', 'Клиент на ЛУКС АВТО')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'ЛУКС АВТО Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'ЛУКС АВТО Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'ЛУКС АВТО Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'ЛУКС АВТО Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'ЛУКС АВТО Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'ЛУКС АВТО Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'ЛУКС АВТО Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'ЛУКС АВТО Sales Team')
		.replaceAll('Los Angeles,California', 'Стара Загора, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на ЛУКС АВТО')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Проверен автомобил от ЛУКС АВТО.')
		.replaceAll('How the adventure ended will be seen soon.', 'Проверен автомобил от ЛУКС АВТО.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със ЛУКС АВТО за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към ЛУКС АВТО">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на ЛУКС АВТО"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на ЛУКС АВТО"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">ЛУКС АВТО<\/a>/g,
				'<a href="/about/daynight-auto-plovdiv" class="h4 mb-8 font-weight-600">ЛУКС АВТО</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Стара Загора, Стара Загора, ул. Христина Морфова 4, кв. Кольо Ганчев, Стара Загора\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на ЛУКС АВТО'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0883 338 833" class="text-sm text-white">0883 338 833<\/a>\s*<a href="tel:0883 338 833" class="text-sm text-white">0883 338 833<\/a>/,
				'<a href="tel:0883 338 833" class="text-sm text-white">0883 338 833</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">ЛУКС АВТО<\/a>/g,
				'<a class="text-white" href="/about/daynight-auto-plovdiv">ЛУКС АВТО</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'ЛУКС АВТО Стара Загора')
		.replaceAll('Aurexo |', 'ЛУКС АВТО |');
}
