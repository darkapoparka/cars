<script lang="ts">
	...
	const mapEmbedSrc = 'https://maps.google.com/maps?q=43.22861,27.8253778&z=15&output=embed';
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Работно време: уточнете по телефона',
			href: null,
			icon: Clock
		}
	] as const;
	...
	const importSteps = [
		{
			...
			copy: 'Обсъдете конкретния автомобил и условията в обявата.'
		},
		{
		...
		{
			title: 'Уточнете възможностите',
			copy: 'Вносът и доставката не са потвърдени услуги. Попитайте по телефона.'
		}
	] as const;
...
<div class="mobile-contact-app">
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		<img
			...
			src={resolve('/assets/autolife/cover.jpg')}
			alt=""
			aria-hidden="true"
		/>
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="Аутолайф начало">
					<img src={resolve('/assets/autolife/wordmark.svg')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със Аутолайф'}
			</h1>
			<p>
				{isImportMode
