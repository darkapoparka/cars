...
const daynightFacebookUrl = 'https://autolife.mobile.bg/';
const daynightInstagramUrl = 'https://autolife.mobile.bg/';
...
function renderFooterSocialLinks() {
	...
	const links: readonly PresentationIconLink[] = [
		...
		},
		{
			...
			title: 'Отвори локацията на Аутолайф в Google Maps',
			kind: 'external',
...
<p class="text-sm text-muted mb-16">Аутолайф предлага актуална дигитална витрина с наличност, запитвания и ясни следващи стъпки.</p>
${renderFooterSocialLinks()}
</div>
<div>
<p class="text-sm text-white font-weight-600 mb-8">Имате въпрос?</p>
<div class="flex items-center gap-12">
<a href="/contact" class="btn btn-line-1 btn-small" title="Свържете се със Аутолайф">Свържете се</a>
</div>
</div>
</div>
</div>
...
<p class="text-sm text-muted">©2026 <a class="text-sm text-white" href="/">Аутолайф</a>. Всички права запазени.</p>
...
<a href="tel:${daynightSite.phone}" class="icon" aria-label="Обади се на Аутолайф">${phoneIcon()}</a>
...
<a href="${daynightSite.mapUrl}" class="icon" target="_blank" rel="noopener" aria-label="Отвори карта до Аутолайф">${locationIcon()}</a>
<div class="daynight-header-contact__body">
...
<a class="daynight-map-preview" href="${daynightSite.mapUrl}" target="_blank" rel="noopener" aria-label="Отвори Google карта до Аутолайф">
<span class="daynight-map-preview__thumb">
<span class="daynight-map-preview__pin"></span>
</span>
<span class="daynight-map-preview__body">
<strong>Аутолайф Варна</strong>
