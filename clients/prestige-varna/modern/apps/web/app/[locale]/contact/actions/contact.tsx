...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на ПРЕСТИЖ.",
          "The form is ready, but message delivery is not configured yet. Call ПРЕСТИЖ directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на ПРЕСТИЖ.",
          "Message delivery is not configured yet. Call ПРЕСТИЖ directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на ПРЕСТИЖ."
        : "Запитването е изпратено до екипа на ПРЕСТИЖ.",
      "Your request has been sent to the ПРЕСТИЖ team."
    ),
  };
};
