...
export const vehicles: Vehicle[] = stock.map(v=>({slug:v.slug,title:v.title,brand:v.make,model:v.model,bodyType:v.body,condition:'Used' as const,price:v.priceEur,monthly:null,year:v.year,mileage:v.mileageKm,fuel:v.fuel,transmission:v.transmission,engine:String(v.engineCc)+' см³',exterior:v.color,interior:'Не е посочен',location:daynightSite.location,vin:'Не е публикуван',stockNumber:v.sourceId,tag:'По обява · 09.09.2026',image:v.images[0].path,images:v.images.map(i=>i.path),gallery:v.images.map(i=>i.path),dealerSlug:'g-auto',agentSlug:'prodazhbi-showroom',rating:null,description:v.notes+' Източник: '+v.sourceUrl,features:v.features}));
