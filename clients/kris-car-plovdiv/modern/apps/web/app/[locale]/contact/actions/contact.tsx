...
const getCopy = (isBg: boolean, isImportRequest: boolean) => {
  ...
  return {
    ...
    notConfigured: isImportRequest
      ? localized(
          "Формата е готова, но каналът за съобщения още не е конфигуриран. Обадете се директно на Крис Кар.",
          "The form is ready, but message delivery is not configured yet. Call Крис Кар directly."
        )
      : localized(
          "Каналът за съобщения още не е конфигуриран. Обадете се директно на Крис Кар.",
          "Message delivery is not configured yet. Call Крис Кар directly."
        ),
    ...
    success: localized(
      isImportRequest
        ? "Заявката е изпратена до екипа на Крис Кар."
        : "Запитването е изпратено до екипа на Крис Кар.",
      "Your request has been sent to the Крис Кар team."
    ),
  };
};
...
export const submitContactRequest = async (
  ...
  if (leadSite.staticDemoMode) return { status: "error", message: copy.notConfigured };
