<script lang="ts">
  ...
  const socialPlatforms = [{name:"phone",label:"Телефон",href:brand.phoneHref},{name:"map-pin",label:"Карта",href:brand.mapUrl},{name:"arrow-right",label:"Mobile.bg",href:brand.sourceInventory}] as const;
</script>
...
<div class="dn-contact-intent" class:dn-contact-intent--general={topic.id === 'general'} class:dn-contact-hero-panel={topic.id === 'general'} class:dn-contact-intent--workflow={topic.id === 'trade-in' || topic.id === 'import'}>
  <div class="dn-contact-intent__main">
    ...
    {#if topic.id !== 'trade-in' && topic.id !== 'import'}
    <div class="dn-contact-social" role="group" aria-label="Публикувани контакти">
      <span>Публикувани контакти</span>
      <div>
