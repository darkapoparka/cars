<script lang="ts">
  ...
  let galleryIndex = $state<Record<number,number>>({});
  const photos = $derived(data.vehicle.gallery.length ? data.vehicle.gallery : [data.vehicle.image]);
  const currentPhoto = $derived(photos[galleryIndex[data.vehicle.id] ?? 0] ?? data.vehicle.image);
...
<svelte:head>
  ...
  <meta
    name="description"
    content={`${data.vehicle.title}, ${data.vehicle.year}, ${data.vehicle.mileage}. Демонстрационна обява от ${brand.name} в ${brand.city}.`}
  />
</svelte:head>
          ...
          <div class="dn-detail-main">
            ...
            <div class="dn-detail-card dn-detail-media-card">
              ...
              <div class="dn-detail-thumbnails" aria-label="Снимки от обявата">
                {#each photos as photo, index (photo)}
                  <button type="button" aria-pressed={(galleryIndex[data.vehicle.id] ?? 0) === index} aria-label={`Снимка ${index + 1} — ${data.vehicle.title}`} onclick={() => galleryIndex[data.vehicle.id] = index}><img src={photo} alt="" width="120" height="90" loading="lazy" /></button>
                {/each}
              </div>
            </div>
            ...
            <section class="dn-detail-card dn-detail-info-card" aria-label="Информация за автомобила">
              ...
              {:else if activeDetailTab === 'description'}
                ...
                >
                  <p>
                    {data.vehicle.disclosure}
                  </p>
                  <a href={data.vehicle.evidenceUrl} target="_blank" rel="noopener noreferrer">Публикувана обява</a>
              ...
              {:else}
                <div id="detail-panel-equipment" role="tabpanel" aria-labelledby="detail-tab-equipment">
                  <ul class="dn-detail-equipment">
                    {#each data.vehicle.sourceFeatures as feature (feature)}
...
<style>
.dn-detail-thumbnails{display:flex;gap:8px;padding:12px;overflow-x:auto}.dn-detail-thumbnails button{flex:0 0 94px;border:2px solid transparent;border-radius:8px;padding:0;background:transparent;overflow:hidden;cursor:pointer}.dn-detail-thumbnails button[aria-pressed="true"]{border-color:var(--dn-red)}.dn-detail-thumbnails img{display:block;width:100%;height:66px;object-fit:cover}
</style>
