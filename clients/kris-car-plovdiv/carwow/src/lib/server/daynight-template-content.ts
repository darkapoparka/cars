		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Dealer Listing', 'Профил на Крис Кар Пловдив')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'Крис Кар публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'Крис Кар')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на Крис Кар')
					...
					.replaceAll('Heather Dick', 'Клиент на Крис Кар')
					.replaceAll(
...
const selectedTemplateFileToRoute: Record<string, string> = {
	...
	'dealers-listing.html': '/about/kris-car-plovdiv',
	'dealer-details.html': '/about/kris-car-plovdiv',
	'about-us.html': '/about',
...
const daynightPageAssets = {
	aboutExterior: '/dealer/stock/21788856265448667-1.webp',
	aboutConsultation: '/dealer/stock/11788863173361582-1.webp',
	aboutShowroomVehicle: '/dealer/stock/21781080017250424-1.webp',
	servicesInspection: '/dealer/stock/11788352260592650-1.webp',
	servicesConsultation: '/dealer/stock/11780494813207389-1.webp',
...
> = {
	'about-us.html': {
		...
		title: 'За Крис Кар',
		description:
			'Обяви за автомобили, ясна история и съдействие от избора до документите в Пловдив.',
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до Крис Кар Пловдив" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							...
							<li>
								<a href="${daynightSite.sourceInventory}" target="_blank" rel="noopener" class="hover-stroke-white" title="Виж автомобилите в селекцията в mobile.bg" aria-label="Виж автомобилите в селекцията в mobile.bg">
									<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на Крис Кар в Google Maps" aria-label="Отвори локацията на Крис Кар в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
					...
					<div class="header-container-fluid relative">
						<div class="header-inner daynight-dashboard-header__inner" id="site-header-inner">
							...
							<div class="daynight-dashboard-header__actions">
								<a class="daynight-dashboard-header__ghost" href="/inventory">Демонстрационна селекция</a>
...
function replaceSharedCounterMetrics(html: string) {
	return html
		...
		.replace(
			/<p class="font-weight-500 text-muted h7 text-center">Обяви за автомобили<\/p>/,
...
function replaceInventoryCopy(html: string, templateFile: string) {
	...
	const output = replaceInventoryCardCollections(
		...
		.replaceAll('Show 1,029 Matches', 'Виж автомобилите в селекцията')
		...
		.replaceAll('Tampa, FL', 'Пловдив')
		.replaceAll('Listing GridStyle Half Map', 'Карта на автомобилите в селекцията')
		.replaceAll('Listing GridStyle HalfMap', 'Карта на автомобилите в селекцията')
		.replaceAll('Grid Style Half Map', 'Карта на автомобилите в селекцията')
		.replaceAll('Автомобили GridStyle Half Map', 'Карта на автомобилите в селекцията')
		.replaceAll('Автомобили GridStyle HalfMap', 'Карта на автомобилите в селекцията')
		.replaceAll('Listing Grid 4 Columns', 'Автомобили в наличност')
		.replaceAll('Автомобили Grid 4 Columns', 'Демонстрационна селекция')
	...
	return templateFile === 'listing-gridstyle-halfmap.html'
		? output.replace(
				...
				`<p class="md-hidden">Карта на автомобилите в селекцията · ${daynightVehicles.length} автомобила</p>`
			)
		: output;
}
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до Крис Кар Пловдив" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
		...
		<div class="daynight-about-location">
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на Крис Кар">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със Крис Кар">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от Крис Кар', '');
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от Крис Кар.</p>`;
	}
...
function renderBlogDetailMetaList(article: DayNightArticle) {
	const items = [
		{ type: 'author' as const, label: article.author, href: '/about/kris-car-plovdiv' },
...
function renderBlogDetailAuthorBox() {
	...
	<p class="h7 line-height-28">Крис Кар публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Пловдив.</p>
</div>`;
}
...
function renderBlogDetailComments() {
	const comments = [
		...
		},
		{
			name: 'Крис Кар',
			time: 'преди 2 дни',
			text: 'Да, най-добре е да сравним бюджет, пробег, гориво и наличност за потвърждение преди оглед.',
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'Крис Кар')
		...
		.replaceAll('Nelson Mandela', 'Крис Кар')
		.replaceAll(
		...
		.replaceAll(
			...
			'Да, най-добре е да сравним бюджет, пробег, гориво и наличност за потвърждение.'
		)
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За Крис Кар')
			.replaceAll(
			...
			.replaceAll(
				'At Крис Кар, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на Крис Кар помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на Крис Кар')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на Крис Кар')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">Крис Кар предлага внимателно подбрани и обяви за автомобили с произход за проверка, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At Крис Кар, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'Крис Кар предлага внимателно подбрани и обяви за автомобили с произход за проверка, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от Крис Кар')
			.replaceAll('Certified Dealers', 'Крис Кар')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на Крис Кар ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на Крис Кар ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със Крис Кар')
			.replaceAll(
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на Крис Кар уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със Крис Кар за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от Крис Кар'
			)
			.replaceAll(
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на Крис Кар')
			.replaceAll(
			...
			.replaceAll('Olivia Williams', 'Клиент на Крис Кар')
			.replaceAll('Sophia Martinez', 'Клиент от Пловдив')
			.replaceAll('Daniel Wright', 'Клиент на Крис Кар')
			.replaceAll('Sarah Nguyen', 'Клиент от Пловдив')
			.replaceAll('Manager, NexTech', 'Клиент на Крис Кар')
			.replaceAll('Freelance Designer', 'Клиент от Пловдив')
			.replaceAll('Entrepreneur', 'Клиент на Крис Кар')
			.replaceAll('Accountant', 'Клиент от Пловдив')
			.replaceAll('CEO BMW', 'Клиент на Крис Кар')
			.replaceAll('CEO Крис Кар', 'Клиент на Крис Кар')
			.replaceAll('James Anderson', 'Клиент от Пловдив')
			.replaceAll('Project Manager', 'Клиент на Крис Кар')
			.replaceAll('Avitex', 'Крис Кар')
			.replaceAll('Tesla', 'Крис Кар')
			.replaceAll('Aurexo', 'Крис Кар');
	}
	...
	if (templateFile === 'sale-agents.html') {
		let output = html
			...
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на Крис Кар')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на Крис Кар'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на Крис Кар');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на Крис Кар')
			...
			.replaceAll('Randynox', 'Клиент на Крис Кар')
			...
			.replaceAll(
				...
				'Получих ясна информация за автомобилите в селекцията, условията за оглед и следващите стъпки. Комуникацията беше точна и навременна.'
			)
			.replaceAll(
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на Крис Кар Пловдив')
			.replaceAll('Dealer Details', 'Профил на Крис Кар Пловдив')
			.replaceAll('Dealer Detail', 'Профил на Крис Кар Пловдив')
			...
			.replaceAll(
				...
				'Крис Кар Пловдив предлага проверени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със Крис Кар')
			.replaceAll('get in touch', 'Свържете се със Крис Кар')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със Крис Кар<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Пловдив, Пловдив, Цариградско шосе, до бензиностанция Алпи, Индустриална зона - Изток, Пловдив<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Пловдив, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на Крис Кар.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на Крис Кар за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на Крис Кар съдейства с варианти за финансиране, лизинг и собствено разсрочено плащане според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на Крис Кар предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. Крис Кар прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'Крис Кар полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'Крис Кар може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със Крис Кар преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към Крис Кар')
			.replaceAll('Submit Listing', 'Изпрати към Крис Кар')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Darrell Steward', 'Крис Кар')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'Крис Кар')
			.replaceAll('Крис Кар Support', 'Крис Кар')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('Крис Кар') ? '' : ' Крис Кар');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('New Vehicles', 'Демонстрационна селекция')
		...
		.replaceAll('Why Choose Us?', 'Защо Крис Кар?')
		.replaceAll('Why Choose Us', 'Защо Крис Кар')
		.replaceAll(
			'Explore our wide selection, competitive prices, and exceptional service for a hassle-free car-buying experience.',
			'Изберете автомобил с ясна информация, наличност за потвърждение и съдействие при оглед, документи, регистрация и финансиране.'
		)
		.replaceAll(
			'Wide Selection – A variety of cars to fit every need.',
			'Демонстрационна селекция за различен бюджет и нужди.'
		)
		.replaceAll(
		...
		.replaceAll('Find Your Car Now!', 'Виж автомобилите в селекцията')
		...
		.replaceAll('3,5k+', '100%')
		.replaceAll('Verified Dealers', 'Обяви за автомобили')
		.replace(
			/<a href="\/sell-your-car" class="btn btn-white btn-large font-weight-600 max-w-min text-primary">\s*Виж автомобилите в селекцията\s*<\/a>/g,
			'<a href="/inventory" class="btn btn-white btn-large font-weight-600 max-w-min text-primary">Виж автомобилите в селекцията</a>'
		)
		...
		.replaceAll('Certified Dealers', 'Крис Кар')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of Крис Кар who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на Крис Кар ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със Крис Кар')
		.replaceAll('Reach Out To Us', 'Свържете се със Крис Кар')
		.replaceAll('Get In Touch', 'Свържете се със Крис Кар')
		.replaceAll('get in touch', 'Свържете се със Крис Кар')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от Крис Кар.'
		)
		.replaceAll(
		...
		.replaceAll('Follow Us On social media:', 'Последвайте Крис Кар:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на Крис Кар.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със Крис Кар преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'Крис Кар може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'Крис Кар')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към Крис Кар')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към Крис Кар')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към Крис Кар')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към Крис Кар')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на Крис Кар')
		...
		.replaceAll('James Anderson', 'Клиент от Пловдив')
		.replaceAll('Project Manager', 'Клиент на Крис Кар')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'Крис Кар Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'Крис Кар Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'Крис Кар Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'Крис Кар Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'Крис Кар Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'Крис Кар Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'Крис Кар Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'Крис Кар Sales Team')
		.replaceAll('Los Angeles,California', 'Пловдив, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на Крис Кар')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Проверен автомобил от Крис Кар.')
		.replaceAll('How the adventure ended will be seen soon.', 'Проверен автомобил от Крис Кар.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със Крис Кар за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към Крис Кар">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на Крис Кар"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на Крис Кар"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">Крис Кар<\/a>/g,
				'<a href="/about/kris-car-plovdiv" class="h4 mb-8 font-weight-600">Крис Кар</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Пловдив, Пловдив, Цариградско шосе, до бензиностанция Алпи, Индустриална зона - Изток, Пловдив\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на Крис Кар'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0885 232 858" class="text-sm text-white">0885 232 858<\/a>\s*<a href="tel:0885 232 858" class="text-sm text-white">0885 232 858<\/a>/,
				'<a href="tel:0885 232 858" class="text-sm text-white">0885 232 858</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">Крис Кар<\/a>/g,
				'<a class="text-white" href="/about/kris-car-plovdiv">Крис Кар</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'Крис Кар Пловдив')
		.replaceAll('Aurexo |', 'Крис Кар |');
}
