...
export const cars:Car[]=stock.map(record=>({
  ...
  fuel:record.fuel,transmission:record.transmission,body:record.bodyBg,doors:0,
  engine:record.engineCc?`${number.format(record.engineCc)} см³`:'Не е посочено',
  power:`${record.powerHp} к.с.`,drive:record.features.includes('4x4')?'4x4':'Не е посочено',color:record.color,
  price:record.priceEur,priceEur:`${number.format(record.priceEur)} €`,priceBgn:'',monthly:'',
  image:record.image,gallery:record.gallery,badges:['Публична обява'],
  conditionLine:'Наличността и състоянието се потвърждават от продавача.',
  ...
  lot:record.sourceId||'ID на обявата не е потвърден',sourceUrl:record.sourceUrl
}));
...
// Retained filter bucket for public advertisements, not independent stock verification.
export const getDayNightVehicleAvailability=(vehicle:Pick<Car,'highlights'>):DayNightVehicleAvailability=>vehicle.highlights.some(value=>/очакван|предстоящо/i.test(value))?'incoming':'available';
