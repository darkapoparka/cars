...
/** No independent template stock, invented VINs, ratings or monthly offers. */
export const vehicles: Vehicle[] = cars.map((car) => ({
  ...
  monthly: null,
  year: car.year,
  mileage: car.mileageValue,
  fuel: car.fuel,
  displayFuel: car.fuel,
  transmission: car.transmission,
  engine: car.engine,
  exterior: car.color,
  interior: 'По запитване',
...
export function getRelatedVehicles(vehicle: Vehicle, limit = 4) {
  const closeMatches = vehicles.filter((candidate) =>
    candidate.slug !== vehicle.slug && (candidate.brand === vehicle.brand || candidate.bodyType === vehicle.bodyType));
  const fallback = vehicles.filter((candidate) =>
    candidate.slug !== vehicle.slug && !closeMatches.some((match) => match.slug === candidate.slug));
  return [...closeMatches, ...fallback].slice(0, limit);
}

export function filterVehicles(source: Vehicle[], filters: InventoryFilters) {
  const query = filters.query?.trim().toLocaleLowerCase('bg-BG') ?? '';
  return source.filter((vehicle) => {
    const matchesQuery = !query || [vehicle.title, vehicle.brand, vehicle.model, vehicle.bodyType, vehicle.location]
      .join(' ').toLocaleLowerCase('bg-BG').includes(query);
    ...
    const matchesPrice = filters.maxPrice == null || vehicle.price <= filters.maxPrice;
    const matchesYear = filters.minYear == null || vehicle.year >= filters.minYear;
