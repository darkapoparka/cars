...
function buildBrandList(vehicles: Car[]) {
	const count = new Map<string, number>();
	for (const car of vehicles) count.set(car.brand, (count.get(car.brand) ?? 0) + 1);
...
// Feature this dealer's records rather than the former dealer's hardcoded IDs.
...
function buildFeaturedCars(vehicles: Car[]) {
	...
	const fallback = vehicles
		.filter((car) => !pickedSlugs.has(car.slug))
		.sort((a, b) =>
			Number(placeholderImageSlugs.has(a.slug)) - Number(placeholderImageSlugs.has(b.slug)) ||
			b.price - a.price || b.year - a.year
		);
	// A missing photograph must keep its explicit unavailable-image state,
	// not silently remove an otherwise valid listing from the mobile homepage.
...
function buildBudgetTiles(vehicles: Car[]) {
	const budgets = [
		...
		{
			label: 'Без бюджет', value: 'all',
			image: '/assets/images/budget/open-budget-supercar-v2.webp',
			caption: `Всички ${vehicles.length} коли`, variant: 'open' as const
		}
	];
	return budgets.map(({ label, value, limit, min, image, caption, variant }) => ({
		label, value, image, caption, variant,
		count: value === 'all' ? vehicles.length : vehicles.filter((car) => {
...
export function loadHomePageData(
	home: HomePageHeadData = { title: `${dealer.name} · ${dealer.city}` },
	initialViewport: HomeInitialViewport = 'desktop',
	vehicles?: Car[]
) {
	// Keep the retained explicit-input contract; [] must not revive demo stock.
	const displayVehicles = vehicles ?? cars;
	return {
		home,
		initialViewport,
		homeBrandStrip: buildHomeBrandStripItems(displayVehicles),
		desktopHome: { vehicles: displayVehicles },
		mobileHome: {
