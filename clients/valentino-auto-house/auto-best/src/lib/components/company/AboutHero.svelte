<script lang="ts">
  ...
  const socialProfiles = brand.profiles.map(profile => ({ name: profile.label, label: profile.label, href: profile.url }));
</script>
...
<section class="dn-about-hero dn-route-hero dn-route-hero--studio dn-route-hero--charcoal" aria-labelledby="about-title">
  ...
  <div class="container dn-about-hero__content dn-route-hero__layout">
    <div class="dn-about-hero__copy dn-route-hero__copy">
      ...
      <p class="dn-about-hero__lead">Автомобили в {brand.city} · Горубляне · Оглед по уговорка</p>
    </div>
    ...
    <nav class="dn-about-socials" aria-label="Публикувани профили на автокъщата">
      <span>Публикувани профили</span>
