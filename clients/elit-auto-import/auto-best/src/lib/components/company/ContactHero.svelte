<script lang="ts">
  ...
  let { topic }: { topic: ContactTopic } = $props();
...
<section class="dn-contact-hero dn-route-hero dn-route-hero--studio" class:dn-contact-hero--general={topic.id === 'general'} class:dn-contact-hero--workflow={topic.id === 'trade-in' || topic.id === 'import'} aria-labelledby="contact-title">
  <HeroVehicles pair="contact" mobile={topic.id === 'trade-in' || topic.id === 'import'} />
  <picture>
    {#if topic.id === 'trade-in'}
      <source media="(max-width: 991px)" srcset="/assets/elit/cover.png" />
  ...
  <div class="container dn-contact-hero__content dn-route-hero__layout">
    <div class="dn-contact-hero__copy dn-route-hero__copy">
      <h1 id="contact-title"><span class="dn-contact-hero__desktop-title">{topic.id === 'general' ? 'Свържете се с нас' : topic.id === 'trade-in' ? 'Проверка в сервиз' : topic.title}</span><span class="dn-contact-hero__mobile-title">{topic.id === 'general' ? 'Контакти' : topic.id === 'trade-in' ? 'Проверка в сервиз' : topic.title}</span></h1>
