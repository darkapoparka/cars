		...
		replaceHeaderTopBarControls(
			replaceHeaderContact(
				html
					...
					.replaceAll('Dealer Listing', 'Профил на ELIT AUTO Варна')
					.replaceAll(
						'Mike Hanley (@mike_hanley) is a writer who draws. He’s the Bestselling author of “Number of The Year”.',
						'ELIT AUTO публикува практични съвети за покупка, оглед, документи, финансиране и последващи стъпки.'
					)
					.replaceAll('Mike Hanley', 'ELIT AUTO')
					.replaceAll('@mike_hanley', '@daynightauto')
					.replaceAll('Michael Carter', 'Клиент на ELIT AUTO')
					...
					.replaceAll('Heather Dick', 'Клиент на ELIT AUTO')
					.replaceAll(
...
const daynightPageAssets = {
	aboutExterior: '/assets/elit/cover.png',
	aboutConsultation: '/assets/elit/cover.png',
	aboutShowroomVehicle: '/assets/elit/cover.png',
	servicesInspection: '/assets/elit/cover.png',
	servicesConsultation: '/assets/elit/cover.png',
	dealerProfileMark: '/assets/elit/logo.png'
} as const;
...
> = {
	'about-us.html': {
		...
		title: 'За ELIT AUTO',
		description:
			'Автомобили по обяви, ясна история и съдействие от избора до документите в Варна.',
...
function buildDayNightMapIframe(height: string, loading: 'lazy' | 'eager' = 'lazy') {
	...
	return `<iframe title="Карта до ELIT AUTO Варна" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map height="${height}" style="border:0;width: 100%;" allowfullscreen="" loading="${loading}" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
}
...
function renderTeamContactActions(member: (typeof daynightTeam)[number]) {
	...
	return `<ul class="contact">
								...
								<li>
									<a href="mailto:${member.email}" title="Имейл към ELIT AUTO" aria-label="Имейл към ELIT AUTO">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function renderContactPageSocialLinks() {
	return `<ul class="contact-page-info-social flex gap-8">
							...
							<li>
								<a href="tel:${daynightSite.phone}" class="hover-stroke-white" title="Свържете се с ELIT AUTO по телефон или Viber." aria-label="Свържете се с ELIT AUTO по телефон или Viber.">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							...
							<li>
								<a href="${daynightSite.mapUrl}" target="_blank" rel="noopener" class="hover-stroke-white" title="Отвори локацията на ELIT AUTO в Google Maps" aria-label="Отвори локацията на ELIT AUTO в Google Maps">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function replaceMapModeEmbed(html: string, templateFile: string) {
	...
	const mapEmbed = `<div id="map" data-map-zoom="16" data-map-scroll="true"><iframe title="Карта до ELIT AUTO Варна" data-map-src="${daynightMapEmbedSrc}" data-daynight-scroll-map width="100%" height="100%" style="border:0;display:block;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
	...
	<div class="container">
		<div class="daynight-about-location">
			<div class="daynight-about-location__content">
				...
				<p class="text-secondary h7 line-height-28">ELIT AUTO предлага внимателно подбрани автомобили с реална наличност, съдействие при оглед, документи, регистрация, финансиране и бартер.</p>
			...
			<div class="daynight-about-location__visual">
				<div class="daynight-about-location__images">
					<img src="${daynightPageAssets.aboutShowroomVehicle}" alt="Шоурум на ELIT AUTO">
					<img src="${daynightPageAssets.aboutConsultation}" alt="Консултация със ELIT AUTO">
				</div>
...
function polishServicesPageStructure(html: string) {
	...
	output = replaceSectionByMarker(output, 'Изберете автомобил с подкрепа от ELIT AUTO', '');
...
function replacePdpCopy(html: string, routePath: string) {
	...
	let output = html
		...
		.replaceAll('Mike Hanley', salesTeam?.name ?? 'ELIT AUTO')
...
function renderBlogStandardContent(articles: DayNightArticle[]) {
	...
	if (!featured) {
		return `<p class="h5 text-secondary mb-40">Очаквайте публикации от ELIT AUTO.</p>`;
	}
...
function renderBlogDetailSocialList(article: DayNightArticle) {
	...
	const shareUrl = encodeURIComponent(`https://elitautoimport.mobile.bg${blogArticleRoute(article)}`);
...
function renderBlogDetailAuthorBox() {
	...
	<div class="listing-details--contact-dealer mb-20">
		...
		<div class="content">
			<a href="/about/daynight-auto-plovdiv" class="h4 mb-4 font-weight-600">ELIT AUTO</a>
			<p class="text-secondary mb-18">Автокъща в Варна</p>
			<ul class="blog-detail-social flex gap-12">
				<li></li>
				<li></li>
				<li><a href="mailto:${daynightSite.email}" aria-label="Имейл към ELIT AUTO">${renderBlogSocialIcon('mail')}</a></li>
			</ul>
		</div>
	</div>
	<p class="h7 line-height-28">ELIT AUTO публикува практични съвети и новини за наличност, покупка, оглед, документи, регистрация и финансиране на употребявани автомобили в Варна.</p>
</div>`;
}
...
function replaceBlogDetailCopy(
	...
	let output = html
		...
		.replaceAll('John Smith', 'ELIT AUTO')
		...
		.replaceAll('Nelson Mandela', 'ELIT AUTO')
		.replaceAll(
...
function replaceSupportPageCopy(html: string, templateFile: string, routePath: string) {
	if (templateFile === 'about-us.html') {
		let output = html
			.replaceAll('About Us', 'За ELIT AUTO')
			.replaceAll(
			...
			.replaceAll(
				'At ELIT AUTO, we make car ownership simple and accessible with expert guidance, personalized solutions, and exceptional service. Our team is committed to helping you find the perfect vehicle while ensuring a hassle-free experience.',
				'Екипът на ELIT AUTO помага с избор на автомобил, оглед, документи, регистрация, финансиране и следващи стъпки без излишен шум.'
			)
			...
			.replaceAll('Individual Service Team', 'Консултанти на ELIT AUTO')
			.replaceAll('Индивидуална услуга Team', 'Консултанти на ELIT AUTO')
			...
			.replace(
				...
				'<p class="text-secondary h7 line-height-28 mb-32">ELIT AUTO предлага внимателно подбрани и автомобили по обяви с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.</p>'
			)
			.replace(
				/At ELIT AUTO, we make car ownership simple and accessible[\s\S]*?hassle-free experience\./,
				'ELIT AUTO предлага внимателно подбрани и автомобили по обяви с гарантиран произход, коректна оценка при изкупуване и пълно съдействие от избора до документите, регистрацията и финансирането.'
			)
	...
	if (templateFile === 'sell-your-car.html') {
		return html
			...
			.replaceAll('Get a fair price', 'Получете предложение за покупка или бартер от ELIT AUTO')
			.replaceAll('Certified Dealers', 'ELIT AUTO')
			.replaceAll(
				...
				'Опишете автомобила, изпратете снимки и екипът на ELIT AUTO ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll(
				...
				'Екипът на ELIT AUTO ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
			)
			...
			.replaceAll('Get In Touch With Us', 'Свържете се със ELIT AUTO')
			.replaceAll(
			...
			.replaceAll('Monday - Saturday: 08:00AM - 17:00PM', 'Работно време: по телефона')
			...
			.replaceAll(
				...
				'Обикновено са нужни талон, документ за собственост, лична карта и допълнителни документи според конкретния автомобил. Екипът на ELIT AUTO уточнява точния набор при оглед.'
			)
			.replaceAll(
	...
	if (templateFile === 'services-center.html') {
		let output = html
			...
			.replaceAll(
				...
				'Свържете се със ELIT AUTO за документи, регистрация, финансиране, бартер, оглед, доставка или търсене на автомобил по задание.'
			)
			...
			.replaceAll(
				...
				'Изберете автомобил с подкрепа от ELIT AUTO'
			)
			.replaceAll(
	...
	if (templateFile === 'clients-reviews.html') {
		return html
			.replaceAll('Clients Reviews', 'Отзиви от клиенти')
			.replaceAll('What our clients say', 'Реални впечатления от клиенти на ELIT AUTO')
			.replaceAll(
			...
			.replaceAll('Emily Johnson', 'Клиент на ELIT AUTO')
			.replaceAll('Benjamin Parker', 'Клиент от Варна')
			.replaceAll('Olivia Williams', 'Клиент на ELIT AUTO')
			.replaceAll('Sophia Martinez', 'Клиент от Варна')
			.replaceAll('Daniel Wright', 'Клиент на ELIT AUTO')
			.replaceAll('Sarah Nguyen', 'Клиент от Варна')
			.replaceAll('Manager, NexTech', 'Клиент на ELIT AUTO')
			.replaceAll('Freelance Designer', 'Клиент от Варна')
			.replaceAll('Entrepreneur', 'Клиент на ELIT AUTO')
			.replaceAll('Accountant', 'Клиент от Варна')
			.replaceAll('CEO BMW', 'Клиент на ELIT AUTO')
			.replaceAll('CEO ELIT AUTO', 'Клиент на ELIT AUTO')
			.replaceAll('James Anderson', 'Клиент от Варна')
			.replaceAll('Project Manager', 'Клиент на ELIT AUTO')
			.replaceAll('Avitex', 'ELIT AUTO')
			.replaceAll('Tesla', 'ELIT AUTO')
			.replaceAll('Aurexo', 'ELIT AUTO');
	}

	if (templateFile === 'sale-agents.html') {
		let output = html
			.replaceAll('Sale Agents', 'Екипът на ELIT AUTO')
			.replaceAll('Our Agents', 'Консултанти')
			.replaceAll('Brooklyn Simmons', daynightTeam[0]?.name ?? 'Екипът на ELIT AUTO')
			.replaceAll(
				'Darrell Steward',
				daynightTeam[1]?.name ?? daynightTeam[0]?.name ?? 'Екипът на ELIT AUTO'
			)
			.replaceAll(
				'Senior Dealer Partner',
				daynightTeam[0]?.role.toLowerCase() ?? 'консултант продажби'
			)
			.replaceAll('Mike Hanley', daynightTeam[0]?.name ?? 'Екипът на ELIT AUTO');
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		let output = html
			...
			.replaceAll('Mike Hanley', member?.name ?? 'Екипът на ELIT AUTO')
			...
			.replaceAll('Randynox', 'Клиент на ELIT AUTO')
	...
	if (templateFile === 'dealer-details.html') {
		let output = html
			.replaceAll('Dealer Listing', 'Профил на ELIT AUTO Варна')
			.replaceAll('Dealer Details', 'Профил на ELIT AUTO Варна')
			.replaceAll('Dealer Detail', 'Профил на ELIT AUTO Варна')
			.replaceAll('Aurexo Atlanta', 'ELIT AUTO Варна')
			.replaceAll('Euro Workshop', 'ELIT AUTO Варна')
			...
			.replaceAll(
				...
				'ELIT AUTO Варна предлага проверени употребявани автомобили, съдействие за документи, регистрация, финансиране, бартер и оглед в доверен сервиз. Екипът работи с ясна информация за цена, пробег, състояние и следващи стъпки след избора на автомобил.'
			)
			.replaceAll(
	...
	if (templateFile === 'contact-us.html') {
		let output = promoteContactContentBeforeMap(html)
			...
			.replaceAll('Get In Touch', 'Свържете се със ELIT AUTO')
			.replaceAll('get in touch', 'Свържете се със ELIT AUTO')
		...
		output = output
			.replace(
				/<div class="bg-white radius-20 contact-page-form">\s*<p class="h3 mb-12">Свържете се със ELIT AUTO<\/p>/,
				'<div class="bg-white radius-20 contact-page-form">\n\t\t\t\t\t\t<p class="h3 mb-12">Пишете ни за автомобил</p>'
			)
			.replace(
				/<p class="text-secondary">гр\. Варна, Варна, гр. Варна, м-т Пчелина, ул. Прилеп 74А<\/p>/,
	...
	if (templateFile === 'faqs.html') {
		return html
			...
			.replaceAll(
				...
				'Изберете автомобил от наличността онлайн или на място в Варна, запишете оглед и тест драйв, след което уточняваме финансиране, бартер или лизинг с екипа на ELIT AUTO.'
			)
			.replaceAll(
			...
			.replaceAll(
				...
				'Условията зависят от конкретния автомобил и избраната схема. Свържете се с екипа на ELIT AUTO за актуална информация, оценка на замяна и съдействие по документите.'
			);
	}
	...
	if (templateFile === 'calculator.html') {
		...
		const localized = html
			...
			.replace(
				...
				'Екипът на ELIT AUTO съдейства с варианти за финансиране, лизинг и собствено разсрочено плащане според автомобила и бюджета. Получавате ориентировъчни условия предварително и избирате най-подходящия вариант.'
			)
			...
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Първоначалната вноска намалява финансираната сума и месечната вноска. По-висока първоначална вноска обикновено означава по-добри условия — екипът на ELIT AUTO предлага варианти според бюджета.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
				'Можете да дадете настоящия си автомобил като бартер — оценката му се приспада от цената и намалява финансираната сума. ELIT AUTO прави оглед и ясна оценка преди сделката.'
			)
			.replace(
				/An auto loan is a sum of money[\s\S]*?monthly payments\./,
		...
		return (
			html
				...
				.replaceAll(
					...
					'ELIT AUTO полага усилия съдържанието да е точно и актуално, но не носи отговорност за непълноти или технически грешки в обявите.'
				)
				.replaceAll(
					...
					'ELIT AUTO може да актуализира услугите и условията по всяко време, като валидна е версията, публикувана към момента на ползване на сайта. '
				)
				.replaceAll(
					'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
				...
				.replaceAll(
					...
					'Информацията в този сайт има информативен характер. Актуалните цени, наличност и условия за финансиране се потвърждават директно със ELIT AUTO преди сделка.'
				)
		);
	}
...
function replaceAccountPageCopy(html: string, templateFile: string) {
	...
	if (templateFile === 'add-listings.html') {
		return withAccountChrome
			...
			.replaceAll('Save & Publish', 'Изпрати към ELIT AUTO')
			.replaceAll('Submit Listing', 'Изпрати към ELIT AUTO')
	...
	if (templateFile === 'message.html') {
		return withAccountChrome
			...
			.replaceAll('Darrell Steward', 'ELIT AUTO')
			.replaceAll('Theresa Webb', 'Клиент от сайта')
			.replaceAll('Aurexo Support', 'ELIT AUTO')
			.replaceAll('ELIT AUTO Support', 'ELIT AUTO')
			.replaceAll('Send Message', 'Изпрати съобщение')
			.concat(html.includes('ELIT AUTO') ? '' : ' ELIT AUTO');
	}
...
function replaceVisibleTemplateFiller(html: string, templateFile: string) {
	...
	let output = html
		...
		.replaceAll('Why Choose Us?', 'Защо ELIT AUTO?')
		.replaceAll('Why Choose Us', 'Защо ELIT AUTO')
		.replaceAll(
		...
		.replaceAll('Certified Dealers', 'ELIT AUTO')
		.replaceAll(
			'Answer a few questions about your vehicle, and then connect with one of thousands of ELIT AUTO who can pay you directly for your used car.',
			'Опишете автомобила, изпратете снимки и екипът на ELIT AUTO ще Ви изпрати следващи стъпки за оглед, покупка или бартер.'
		)
		.replaceAll('Reach Out to Us', 'Свържете се със ELIT AUTO')
		.replaceAll('Reach Out To Us', 'Свържете се със ELIT AUTO')
		.replaceAll('Get In Touch', 'Свържете се със ELIT AUTO')
		.replaceAll('get in touch', 'Свържете се със ELIT AUTO')
		.replaceAll('Have any Question?', 'Имате въпрос?')
		...
		.replaceAll(
			...
			'Получавайте нови наличности и полезни съвети от ELIT AUTO.'
		)
		.replaceAll(
		...
		.replaceAll('Week-Day: 8:00 - 18:00', 'Работно време: по телефона')
		.replaceAll('Mon-Sat:8:00am - 18:00pm', 'Работно време: по телефона')
		.replaceAll('Sunday: Closed', 'Неделя: по уговорка')
		.replaceAll('Sunday: Close', 'Неделя: по уговорка')
		.replaceAll('Sun: Closed', 'Неделя: по уговорка')
		.replaceAll('Follow Us On social media:', 'Последвайте ELIT AUTO:')
		...
		.replaceAll(
			'To purchase a car from our dealership, start by exploring our inventory online or visiting us in person to find the vehicle that suits your needs. Schedule a test drive to ensure it’s the right fit, then review financing or leasing options with our team.',
			'Изберете автомобил от наличността, свържете се за оглед и уточнете документи, финансиране или бартер с екипа на ELIT AUTO.'
		)
		.replaceAll(
			'Provide the necessary documents, such as your ID, proof of insurance, and income verification. Once terms are agreed upon, finalize the paperwork, inspect the car, and drive away with your new vehicle!',
		...
		.replaceAll(
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer sed euismod justo, sit amet efficitur dui. Aliquam sodales vestibulum velit, eget sollicitudin quam. Donec non aliquam eros. Etiam sit amet lectus vel justo dignissim condimentum.',
			'Информацията в сайта има информативен характер. Актуални цени, наличност, условия за финансиране и конкретни документи се потвърждават директно със ELIT AUTO преди сделка.'
		)
		.replaceAll(
			'In malesuada neque quis libero laoreet posuere. In consequat vitae ligula quis rutrum. Morbi dolor orci, maximus a pulvinar sed, bibendum ac lacus. Suspendisse in consectetur lorem. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Aliquam elementum, est sed interdum cursus, felis ex pharetra nisi, ut elementum tortor urna eu nulla. Donec rhoncus in purus quis blandit.',
		...
		.replaceAll(
			'Etiam eleifend metus at nunc ultricies facilisis. Morbi finibus tristique interdum. Nullam vel eleifend est, eu posuere risus. Vestibulum ligula ex, ullamcorper sit amet molestie',
			'ELIT AUTO може да съдейства с документи, регистрация, финансиране, бартер и проверка според конкретния случай.'
		)
		.replaceAll('EXPERT REVIEW', 'Съвети')
		...
		.replaceAll('by Admin', 'ELIT AUTO')
		.replaceAll('Super Admin', 'Клиентски профил')
		.replaceAll('Or Purchase New Package', 'Заявка към ELIT AUTO')
		...
		.replaceAll('Apply for Dealer Listings', 'Заявка към ELIT AUTO')
		.replaceAll('Apply for 50 Dealer Listings', 'Разширена заявка към ELIT AUTO')
		.replaceAll('Apply for 150 Dealer Listings', 'Персонална заявка към ELIT AUTO')
		...
		.replaceAll('24/7 Live Chat Support', 'Отговор от екипа на ELIT AUTO')
		...
		.replaceAll('Verified Dealer', 'Проверена автокъща')
		.replaceAll('Emily Johnson', 'Клиент на ELIT AUTO')
		.replaceAll('CEO Avitex', 'Клиент на ELIT AUTO')
		.replaceAll('Benjamin Parker', 'Клиент от Варна')
		...
		.replaceAll('James Anderson', 'Клиент от Варна')
		.replaceAll('Project Manager', 'Клиент на ELIT AUTO')
		.replaceAll('Robert Fox', salesTeam?.name ?? 'ELIT AUTO Sales Team')
		.replaceAll('Bessie Cooper', tradeInTeam?.name ?? 'ELIT AUTO Trade-In Team')
		.replaceAll('Brooklyn Simmons', salesTeam?.name ?? 'ELIT AUTO Sales Team')
		.replaceAll('Kristin Watson', tradeInTeam?.name ?? 'ELIT AUTO Trade-In Team')
		.replaceAll('Guy Hawkins', salesTeam?.name ?? 'ELIT AUTO Sales Team')
		.replaceAll('Darrell Steward', salesTeam?.name ?? 'ELIT AUTO Sales Team')
		.replaceAll('Cody Fisher', tradeInTeam?.name ?? 'ELIT AUTO Trade-In Team')
		.replaceAll('Eleanor Pena', salesTeam?.name ?? 'ELIT AUTO Sales Team')
		.replaceAll('Los Angeles,California', 'Варна, България')
		.replaceAll('FROM:', 'Локация:')
		.replaceAll('two-sided marketplace business', 'дейността на ELIT AUTO')
		.replaceAll(
		...
		.replaceAll('How the adventure ended will...', 'Проверен автомобил от ELIT AUTO.')
		.replaceAll('How the adventure ended will be seen soon.', 'Проверен автомобил от ELIT AUTO.')
		.replaceAll('Vehicle Information', 'Данни за автомобила')
		.replaceAll('Car Details', 'Данни за автомобила')
		.replace(
		...
		.replace(
			...
			'<a href="/contact" class="h4 font-weight-600 mb-8 capitalize" title="Свържете се със ELIT AUTO за тази услуга">'
		)
		.replace(
			/<a href="#" class="h4 font-weight-600 mb-8">/g,
			'<a href="/sell-your-car/request" class="h4 font-weight-600 mb-8" title="Изпратете заявка към ELIT AUTO">'
		)
		.replace(
		...
		.replace(
			...
			'<a href="/contact" title="Мобилно приложение на ELIT AUTO"><img class="h-40" src="/assets/images/brand/app-store-primary.png" alt="app-store">'
		)
		.replace(
			/<a href="#">\s*<img class="h-40" src="\/assets\/images\/brand\/google-play-primary\.png" alt="google-play">/g,
			'<a href="/contact" title="Мобилно приложение на ELIT AUTO"><img class="h-40" src="/assets/images/brand/google-play-primary.png" alt="google-play">'
		)
		.replaceAll(
			'Darrell Steward is a dedicated automotive professional with over 15 years of experience in the car dealership industry. Known for his customer-first approach and in-depth knowledge of the market, Darrell has helped countless clients find their perfect vehicle while ensuring a seamless and enjoyable buying experience.',
			salesTeam?.bio ??
...
function replaceRemainingPresentationLinks(html: string, templateFile: string, routePath: string) {
	...
	if (templateFile === 'listing-details-3.html') {
		output = syncDeadCardAnchorsToCardRoutes(output)
			...
			.replace(
				/<a href="#" class="h4 mb-8 font-weight-600">ELIT AUTO<\/a>/g,
				'<a href="/about/daynight-auto-plovdiv" class="h4 mb-8 font-weight-600">ELIT AUTO</a>'
			)
			.replace(
				/<a href="#">\s*гр\. Варна, Варна, гр. Варна, м-т Пчелина, ул. Прилеп 74А\s*<\/a>/g,
	...
	if (templateFile === 'sale-agents-details.html') {
		...
		output = output
			...
			.replaceAll(
				member?.name ?? '',
				member?.name ?? daynightTeam[0]?.name ?? 'Екипът на ELIT AUTO'
			);
	}
	...
	if (templateFile === 'services-center.html') {
		output = output
			...
			.replace(
				/<a href="tel:0887 777 887" class="text-sm text-white">0887 777 887<\/a>\s*<a href="tel:0887 777 887" class="text-sm text-white">0887 777 887<\/a>/,
				'<a href="tel:0887 777 887" class="text-sm text-white">0887 777 887</a><a href="mailto:demo@getrich.local" class="text-sm text-white">demo@getrich.local</a>'
			)
	...
	if (templateFile === 'blog-details-1.html' || templateFile === 'blog-details-2.html') {
		output = output
			...
			.replace(
				/<a class="text-white" href="#">ELIT AUTO<\/a>/g,
				'<a class="text-white" href="/about/daynight-auto-plovdiv">ELIT AUTO</a>'
			)
			.replace(
...
export function applyDayNightTemplateContent(
	...
	return restoreTemplateAssetPaths(output)
		.replaceAll('Car Dealer, Rental & Listing HTML Template', 'ELIT AUTO Варна')
		.replaceAll('Aurexo |', 'ELIT AUTO |');
}
