<script lang="ts">
	...
	const contactCards = [
		...
		},
		{
			id: 'hours',
			title: 'Работно време',
			value: 'Работното време се уточнява по телефона.',
			href: null,
			icon: Clock
		}
	] as const;
	...
	<header class:mobile-contact-hero--import={isImportMode} class="mobile-contact-hero">
		...
		{#if isImportMode}<MobileHeroBar />{:else}<div class="mobile-contact-hero__bar">
				<a href={resolve('/')} aria-label="ELIT AUTO начало">
					<img src={resolve('/assets/elit/logo.png')} alt={daynightSite.shortName} />
				</a>
		...
		<div class="mobile-contact-hero__copy">
			...
			<h1>
				{isImportMode ? 'Намерете автомобила. Ние ще го внесем.' : 'Свържете се със ELIT AUTO'}
			</h1>
			<p>
				{isImportMode
