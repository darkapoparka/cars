...
describe('native Carwow locale adapter', () => {
	...
	it('keeps route identity and dealer facts independent', () => {
		...
		expect(dealerLabel('en', 'city')).toBe('Пазарджик');
		expect(dealerLabel('bg', 'city')).toBe('Пазарджик');
		expect(dealerLabel('en', 'address')).toBe('18 Atanas Manchev Street, Studentski Grad, Пазарджик');
