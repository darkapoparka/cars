...
const text = (locale: Locale) =>
	locale === 'bg'
		? {
				...
				emptyMap:
					'Няма автомобили по тези филтри. Изчисти филтрите или се свържи с Eliq Auto за входящи автомобили.',
				extras: 'Екстри',
				filterButton: 'Филтри',
		...
		: {
				...
				emptyMap:
					'No cars match these filters. Clear filters or contact Eliq Auto for incoming vehicles.',
				extras: 'Extras',
				...
				fuel: 'Fuel',
				gearbox: 'Gearbox',
				mapTitle: 'Eliq Auto viewing area',
				matchingVehicleLabel: (count: number) =>
					`${count} ${count === 1 ? 'matching vehicle' : 'matching vehicles'}`,
				mileage: 'Mileage',
				model: 'Model',
				price: 'Price',
				searchAria: 'Search Eliq Auto inventory',
				...
				showing: (count: number, hasFilters: boolean) =>
					count === 0
						? `Showing 0 of ${vehicles.length} Eliq Auto Listings`
						: hasFilters
							? `Showing 1 - ${count} of ${count} matching Eliq Auto Listings`
							: `Showing 1 - ${count} of ${count} Eliq Auto Listings`,
...
const makeFilter = ({
	...
	id: `eliqauto-inventory-filter-${name.replace(/[^a-z0-9-]/gi, '-')}`,
	label,
	mode,
	name,
	options,
...
const viewOptions = (state: InventoryState, locale: Locale): AuxeroInventoryViewOption[] =>
	(state.layout === 'dashboard'
		...
		const labels: Record<AuxeroInventoryViewOption['view'], string> =
			locale === 'bg'
				? {
						'3': 'Удобна мрежа с 3 колони',
						'4': 'Плътна мрежа с 4 колони',
						'5': 'Компактна мрежа с 5 колони',
						map: 'Изглед с карта'
					}
				: {
...
const layoutToggle = (state: InventoryState, locale: Locale): AuxeroInventoryLayoutToggle => {
	...
	const title =
		locale === 'bg'
			? active
				? 'Покажи класически изглед'
				: 'Покажи изглед със сайдбар'
			: active
				? 'Use classic inventory view'
				: 'Use sidebar dashboard view';
	...
	return {
		...
		label: locale === 'bg' ? (active ? 'Мрежа' : 'Сайдбар') : active ? 'Grid' : 'Sidebar',
		title
	};
};
...
const mapData = (
	...
	return {
		...
		appointmentNote:
			locale === 'bg'
				? `${eliqautoContact.appointmentNote}. Точната локация за оглед се потвърждава по телефона.`
				: `${eliqautoContact.appointmentNote}. The exact viewing location is confirmed by phone.`,
		ctaHref: eliqautoContact.primaryPhoneHref,
		ctaLabel: eliqautoContact.primaryPhoneLabel,
...
export const inventoryDesktopDataFromState = (
	...
	return {
		...
		ariaLabel: locale === 'bg' ? 'Каталог автомобили на Eliq Auto' : 'Eliq Auto vehicle inventory',
		filtersLabel: locale === 'bg' ? 'Филтри за автомобилите' : 'Vehicle filters',
		...
		pageTitle: locale === 'bg' ? 'Автомобили' : 'Vehicles',
		resultsLabel: locale === 'bg' ? 'Резултати с автомобили' : 'Vehicle results',
