<script lang="ts">
	...
	// Clean Svelte 5 + Tailwind v4 rebuild of the Eliqauto homepage hero, migrating
...
-->
<section
	class="relative isolate overflow-hidden bg-[#121214] pt-[64px] pb-12 font-bc-body text-white md:pt-[146px] md:pb-16"
	style="background-image:
		...
		linear-gradient(158deg, #121214 0%, #18181b 48%, #2a0c0c 100%);"
>
	<!--
				...
				>
					<div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-[repeat(4,1fr)_auto]">
						{#each hero.primaryFilters as filter (filter.id)}
							...
							>
								<span
									class="truncate text-bc-micro leading-[1.15] font-medium tracking-wide text-[#5f5f5f] uppercase"
								>
									{filter.title}
								</span>
								<select
			...
			{:else}
				...
				>
					<div class="flex flex-col gap-2 lg:flex-row lg:items-stretch">
						...
						>
							<span
								class="text-bc-micro leading-[1.15] font-bold tracking-wide text-[#5f5f5f] uppercase"
							>
