<script lang="ts">
	...
	// Clean Svelte 5 + Tailwind v4 rebuild of the "Съвети от Eliqauto" news band:
	//   • Dark-green band header (title left, green "Виж всички" pill right). The title
	//     reproduces the live composition — when copy.newsTitle contains "eliqauto",
	//     the brand word is swapped for the ELIQAUTO wordmark image (a <picture> that
	...
	// ("Eliqauto notes" → logo first) or trails it ("Съвети от Eliqauto" → logo last).
			...
			<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				{#each cards as post (post.slug)}
					...
					>
						...
						<span
							class="absolute top-[18px] left-[18px] z-[2] inline-flex rounded-bc-pill border border-white/25 bg-black/30 px-3 py-1.5 text-bc-micro font-semibold tracking-wider text-white uppercase backdrop-blur-sm"
						>
							{post.category}
						</span>
						...
						<span class="relative z-[2] mt-auto flex flex-col gap-[7px] px-[22px] pt-5 pb-5">
							...
							<span
								class="mt-1 inline-flex w-fit items-center gap-2 rounded-bc-pill bg-bc-accent-bright-soft py-[7px] pr-2 pl-3.5 text-sm font-bold text-bc-accent transition-colors group-hover:bg-white group-focus-visible:bg-white"
							>
								{copy.readMore}
								<!-- Inner ink disc keeps the arrow readable on the bright pill (raw hex) -->
								<span
									class="inline-flex h-[22px] w-[22px] items-center justify-center rounded-full bg-[#2a0c0c] text-white"
									aria-hidden="true"
								>
