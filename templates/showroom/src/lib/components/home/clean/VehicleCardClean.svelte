...
>
	...
	<div class="flex flex-1 flex-col p-4">
		...
		<a
			...
			class="text-bc-micro font-bold tracking-[0.14em] text-bc-muted uppercase transition-colors hover:text-bc-accent"
		>
			{vehicle.brand}
		</a>
		...
		<div class="mt-4 border-t border-bc-border pt-4">
			<a
				href={resolve(`/inventory/${encodeURIComponent(vehicle.slug)}`)}
				class="flex min-h-11 w-full items-center justify-center gap-2 rounded-bc-md border border-bc-border bg-white px-4 text-[15px] font-bold text-bc-accent transition-colors hover:border-bc-accent hover:bg-bc-accent hover:text-white"
			>
