<script lang="ts">
	...
	type AuxeroWindow = Window & {
		__ELIQAUTO_AUXERO_BODY_LOADER_EXECUTED__?: number;
		__ELIQAUTO_AUXERO_RUNTIME_MOUNTS__?: number;
		__ELIQAUTO_BODY_SCRIPTS_LOADED__?: boolean;
		__ELIQAUTO_RUNTIME__?: unknown;
	};

	let { html, pageKey = '' }: Props = $props();

	const replayAttribute = 'data-eliqauto-runtime-replay';
	...
	const hasUninitializedAuxeroWidgets = () =>
		Boolean(
			document.querySelector(
				'.swiper-container:not(.swiper-container-initialized):not(.eliqauto-native-scroll)'
			)
		);
	...
	const waitForServerRenderedScripts = async (
		...
		for (let attempt = 0; attempt < 30; attempt += 1) {
			...
			const bodyScriptsLoaded = Boolean(auxeroWindow.__ELIQAUTO_BODY_SCRIPTS_LOADED__);
			const runtimeReady = Boolean(auxeroWindow.__ELIQAUTO_RUNTIME__);
			const widgetsNeedReplay = hasUninitializedAuxeroWidgets();
			sawServerManagedScripts =
				sawServerManagedScripts ||
				runtimeReady ||
				Boolean(auxeroWindow.__ELIQAUTO_AUXERO_BODY_LOADER_EXECUTED__) ||
				bodyScriptsLoaded;
	...
	const runScriptsWhenNeeded = async (source: string, version: number) => {
		...
		const auxeroWindow = window as AuxeroWindow;
		const mountCount = auxeroWindow.__ELIQAUTO_AUXERO_RUNTIME_MOUNTS__ ?? 0;
		const isFirstAuxeroRuntimeMount = mountCount === 0;
		const sourceIncludesRuntime = source.includes('__ELIQAUTO_RUNTIME__');
		auxeroWindow.__ELIQAUTO_AUXERO_RUNTIME_MOUNTS__ = mountCount + 1;
