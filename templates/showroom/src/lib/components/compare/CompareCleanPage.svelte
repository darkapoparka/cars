...
<div class="min-h-screen bg-bc-bg font-bc-body text-bc-ink antialiased">
	...
	<main class="mx-auto w-full max-w-[1200px] px-4 pb-24">
		<!-- Hero -->
		<section
			class="relative mb-8 overflow-hidden rounded-bc-section bg-[linear-gradient(135deg,#2a0c0c_0%,#151518_58%,#17171a_100%)] px-9 py-8 ring-1 ring-white/10"
		>
			...
			<h1 class="m-0 mb-3 max-w-xl text-bc-h2 font-semibold text-white">
				{t('Сравни автомобили от Eliqauto', 'Compare Eliqauto vehicles')}
			</h1>
			...
			<a
				...
				class="mt-6 inline-flex min-h-[44px] items-center gap-2 rounded-bc-md border border-bc-accent bg-bc-accent px-5 text-sm font-bold text-bc-accent-contrast no-underline transition-colors hover:border-white hover:bg-white hover:text-bc-accent"
			>
		...
		>
			<div class="grid gap-0.5">
				...
				<strong class="text-lg leading-6 font-bold text-bc-accent">
					{selected.length}
			...
			<div class="flex items-center gap-2.5">
				...
				<button
					...
					onclick={clearAll}
					class="inline-flex min-h-[40px] items-center rounded-bc-md border border-bc-border bg-white px-3.5 text-[13px] font-bold text-bc-ink-soft transition-colors hover:border-bc-accent hover:bg-bc-surface-hover hover:text-bc-accent disabled:cursor-default disabled:opacity-45"
				>
					{t('Изчисти', 'Clear')}
				</button>
				<a
					href={resolve('/inventory')}
					class="inline-flex min-h-[40px] items-center gap-1.5 rounded-bc-md border border-bc-accent bg-bc-accent px-3.5 text-[13px] font-bold text-bc-accent-contrast no-underline transition-colors hover:border-bc-accent-hover hover:bg-bc-accent-hover"
				>
		...
		<div class="overflow-hidden rounded-bc-md border border-bc-border bg-white">
			{#if selected.length === 0}
				...
				>
					<span
						class="inline-flex h-14 w-14 items-center justify-center rounded-full bg-bc-accent-soft text-bc-accent"
					>
						<Plus size={26} strokeWidth={2.2} aria-hidden="true" />
					</span>
					<strong class="text-[19px] font-bold text-bc-ink">
					...
					<a
						href={resolve('/inventory')}
						class="mt-1.5 inline-flex min-h-[44px] items-center gap-2 rounded-bc-md border border-bc-accent bg-bc-accent px-[18px] text-sm font-bold text-bc-accent-contrast no-underline transition-colors hover:border-bc-accent-hover hover:bg-bc-accent-hover"
					>
						...
						<tr class="border-b border-bc-border">
							<td class="px-4 py-5 align-bottom">
								<div class="flex flex-col gap-1.5">
									<strong class="text-[15px] leading-5 font-bold text-bc-accent">
										{selected.length}
										{selected.length === 1
											? t('автомобил', 'вehicle')
								...
								<td class="border-l border-bc-border/60 px-4 pt-5 pb-4 align-bottom">
									<div class="relative flex flex-col items-center text-center">
										<button
											...
											class="absolute top-2 right-2 z-10 inline-flex h-[30px] w-[30px] items-center justify-center rounded-full border border-bc-border bg-white text-bc-ink transition-colors hover:border-bc-accent hover:bg-bc-accent hover:text-white"
										>
										...
										<span
											class="mt-3.5 w-fit rounded-full bg-bc-accent-soft px-2.5 py-0.5 text-bc-micro font-extrabold tracking-wider text-bc-accent uppercase"
										>
											{vehicle.brand}
										</span>
										<a
											href={resolve('/inventory/[slug]', { slug: vehicle.slug })}
											class="mt-0.5 line-clamp-2 min-h-12 text-[17px] leading-6 font-semibold text-bc-ink no-underline hover:text-bc-accent"
										>
											{vehicle.title}
										</a>
										<a
											href={resolve('/inventory')}
											class="mt-3 inline-flex w-fit items-center gap-1.5 rounded-full border border-bc-border bg-white px-3.5 py-1.5 text-xs font-bold text-bc-ink-soft no-underline transition-colors hover:border-bc-accent hover:bg-bc-surface-hover hover:text-bc-accent"
										>
								...
								{#each row.values as value, index (`${row.label}-${index}`)}
									...
									>
										<div class="flex flex-col items-center gap-1">
											...
											{#if isBest && row.badge}
												<span
													class="rounded-full bg-bc-accent px-2 py-0.5 text-bc-micro font-extrabold tracking-wide text-bc-accent-contrast uppercase"
												>
													{row.badge}
												</span>
											{/if}
						...
						{#if priceRow}
							<tr>
								<td class="bg-white px-4 py-4">
									<span class="text-[15px] font-bold text-bc-accent">{t('Цена', 'Price')}</span>
								</td>
								{#each selected as vehicle, index (`price-${vehicle.slug}`)}
									...
									>
										<div class="flex flex-col items-center gap-2">
											...
											<a
												href={resolve('/inventory/[slug]', { slug: vehicle.slug })}
												class="inline-flex min-h-[40px] items-center justify-center gap-1.5 rounded-bc-md border border-bc-accent bg-bc-accent px-[18px] text-sm font-bold text-bc-accent-contrast no-underline transition-colors hover:border-bc-accent-hover hover:bg-bc-accent-hover"
											>
		...
		<p class="mt-4 text-center text-xs font-semibold tracking-wide text-bc-muted uppercase">
			{t(
				'Чист Svelte 5 + Tailwind v4 — без Auxero тема, без',
				'Clean Svelte 5 + Tailwind v4 — no Auxero theme, zero'
			)}
		</p>
	</main>
