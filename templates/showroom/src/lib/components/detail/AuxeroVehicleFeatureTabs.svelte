<script lang="ts">
	...
	const tabId = (index: number) => `eliqauto-pdp-feature-tab-${index}`;
	const panelId = (index: number) => `eliqauto-pdp-feature-panel-${index}`;
...
<div class="flat-tabs mb-40">
			...
			{#each tabPanels as tab, index (tab.label)}
				<li class={[activeTabIndex === index && 'active']} role="presentation">
					<button
						...
						class="eliqauto-pdp-feature-tab"
						id={tabId(index)}
						onclick={() => {
	...
	<div class="content-tab">
		{#each tabPanels as tab, index (tab.label)}
			...
			>
				{#if tab.description}
					<p class="text-secondary eliqauto-pdp-tab-description">{tab.description}</p>
				{:else if tab.items}
...
<style>
	...
	.eliqauto-pdp-feature-tab:active,
	.eliqauto-pdp-feature-tab:focus:not(:focus-visible) {
		background: transparent;
		outline: none;
	}

	.eliqauto-pdp-feature-tab:focus-visible {
		border-radius: 6px 6px 0 0;
