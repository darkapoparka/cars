...
export const extractAuxeroBodyScriptsHtml = (bodyHtml: string) => {
	const scripts = bodyScriptTags(bodyHtml)
		.filter(
			(script) =>
				!script.content.includes('__ELIQAUTO_RUNTIME__') &&
	...
	return `<script>{
	window.__ELIQAUTO_AUXERO_BODY_LOADER_EXECUTED__ = (window.__ELIQAUTO_AUXERO_BODY_LOADER_EXECUTED__ || 0) + 1;
	try {
		window.localStorage.setItem('suggest_subscribe', window.localStorage.getItem('suggest_subscribe') || 'false');
	} catch (error) {}
	const eliqautoBodyScripts = ${serializedScriptPayload(scripts)};
	const eliqautoOptionalBodyScriptSources = ${JSON.stringify(optionalBodyScriptSources).replace(/</g, '\\u003c')};
	let eliqautoBodyScriptsStarted = false;

	const loadEliqautoBodyScript = (definition) =>
		new Promise((resolve, reject) => {
			...
			script.addEventListener(
				...
				() => {
					...
					if (eliqautoOptionalBodyScriptSources.some((optionalSource) => source.includes(optionalSource))) {
						resolve();
						return;
					}
	...
	const startEliqautoBodyScripts = async () => {
		if (eliqautoBodyScriptsStarted) return;
		eliqautoBodyScriptsStarted = true;

		for (const definition of eliqautoBodyScripts) {
			await loadEliqautoBodyScript(definition);
		}

		window.__ELIQAUTO_BODY_SCRIPTS_LOADED__ = true;
		window.dispatchEvent(new Event('eliqauto:scripts-loaded'));
	};

	const startEliqautoBodyScriptsAfterLoad = () => {
		...
		if (document.documentElement.hasAttribute('data-eliqauto-hydrated')) {
			requestAnimationFrame(() => requestAnimationFrame(startEliqautoBodyScripts));
			return;
		}

		window.addEventListener('eliqauto:hydrated', () => startEliqautoBodyScripts(), { once: true });
		window.setTimeout(startEliqautoBodyScripts, 2500);
	};
	window.addEventListener('eliqauto:svelte-mounted', startEliqautoBodyScripts, { once: true });
	if (document.readyState === 'complete') {
		startEliqautoBodyScriptsAfterLoad();
	} else {
		window.addEventListener('load', startEliqautoBodyScriptsAfterLoad, { once: true });
	}
}</script>`;
};
...
export const extractAuxeroRuntimeHtml = (
	...
	const runtimeScript = bodyScriptTags(bodyHtml)
		.filter(
			(script) =>
				!/\bsrc\s*=/i.test(script.attributes) && script.content.includes('__ELIQAUTO_RUNTIME__')
		)
		.at(-1);
	...
	const setEliqautoEarlyFormStatus = (form, message) => {
	...
	const eliqautoEarlyFormConfig = (form) => {
		if (form.matches('.eliqauto-contact-form')) {
			return {
				source: 'eliqauto-contact-form',
				status: 'Съобщението е изпратено. Eliqauto ще се свърже с вас скоро.',
				url: '/api/inquiries'
			};
		}

		if (form.matches('.eliqauto-blog-comment-form')) {
			return {
				source: 'eliqauto-blog-comment-form',
				status: 'Коментарът е запазен локално за преглед от Eliqauto',
				url: '/api/messages'
			};
		}

		if (form.matches('.eliqauto-sell-form')) {
			return {
				source: 'sell-your-car',
				status: 'Заявката е изпратена. Eliqauto ще се свърже с вас скоро.',
				url: '/api/inventory/submissions'
			};
		}

		if (form.matches('.eliqauto-service-form')) {
			return {
				source: 'eliqauto-service-form',
				status: 'Заявката за услуга е изпратена. Eliqauto ще подготви следващата стъпка.',
				url: '/api/inquiries'
			};
		}
	...
	document.addEventListener('submit', async (event) => {
		...
		const config = eliqautoEarlyFormConfig(form);
		if (!config) return;

		event.preventDefault();
		event.stopImmediatePropagation();
		try {
			setEliqautoEarlyFormStatus(form, 'Изпращаме заявката...');
			...
			form.reset();
			setEliqautoEarlyFormStatus(form, config.status);
		} catch (_error) {
			setEliqautoEarlyFormStatus(
				form,
				'Не успяхме да изпратим заявката. Моля опитайте отново или се обадете.'
			);
		}
	}, true);
	let eliqautoRuntimeStarted = false;
	const eliqautoRuntimeWaitsForBodyScripts = ${JSON.stringify(waitForBodyScripts)};
	const startEliqautoRuntime = () => {
		if (eliqautoRuntimeStarted) return;
		eliqautoRuntimeStarted = true;
${runtimeScript.content}
	};
	const startEliqautoRuntimeWhenScriptsLoad = () => {
		if (!eliqautoRuntimeWaitsForBodyScripts) {
			startEliqautoRuntime();
			return;
		}

		if (window.__ELIQAUTO_BODY_SCRIPTS_LOADED__) {
			startEliqautoRuntime();
			return;
		}

		window.addEventListener('eliqauto:scripts-loaded', startEliqautoRuntime, { once: true });
	};
	const startEliqautoRuntimeWhenSafe = () => {
		...
		if (document.documentElement.hasAttribute('data-eliqauto-hydrated')) {
			startEliqautoRuntimeWhenScriptsLoad();
			return;
		}

		window.addEventListener('eliqauto:hydrated', startEliqautoRuntimeWhenScriptsLoad, { once: true });
		window.setTimeout(startEliqautoRuntimeWhenScriptsLoad, 2500);
	};
	window.addEventListener('eliqauto:svelte-mounted', startEliqautoRuntimeWhenScriptsLoad, { once: true });
	if (document.readyState === 'complete') {
		startEliqautoRuntimeWhenSafe();
	} else {
		window.addEventListener('load', startEliqautoRuntimeWhenSafe, { once: true });
	}
}</script>`;
};
