...
type CmsCollections = {
	'inventory-listings': EliqautoInventoryListingRecord[];
	'vehicle-submissions': EliqautoVehicleSubmissionRecord[];
};
...
const dataRoot = () => {
	if (process.env.ELIQAUTO_CMS_DATA_DIR) return process.env.ELIQAUTO_CMS_DATA_DIR;
	if (process.env.VITEST) {
		return path.join(process.cwd(), '.tmp', 'eliqauto-cms-vitest', String(process.pid));
	}

	return path.join(process.cwd(), '.eliqauto-cms');
};
