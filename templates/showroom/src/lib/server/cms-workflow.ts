import type { EliqautoInventoryListingStatus } from '$lib/types/account';

export type CmsListingStatus = EliqautoInventoryListingStatus;
...
const emptyMarkers = new Set(['', 'on request', 'draft', 'eliqauto', 'eliqauto inventory draft']);
...
export const scoreInventoryCompleteness = (listing: CmsCompletenessInput): CmsCompleteness => {
	...
	score += addMissing({
		condition:
			...
			!listing.description?.includes('queued in the Eliqauto CMS'),
		missing,
		name: 'description',
		points: 8
	});
