import { eliqautoConsultants, eliqautoContact, eliqautoFetchedAt } from '$lib/data/eliqauto';
...
import type { EliqautoSession } from './auth';
...
const inventoryBrandPills = brands
	.map((brand) => ({
		...
		image: brandLogos[brand] ?? '/assets/eliqauto/brand/eliqauto-logo-dark.svg',
...
const viewToggle = (state: InventoryState) =>
	(['3', '4', '5', 'map'] as const)
		.map((view) => {
			...
			)}" data-eliqauto-view-toggle aria-label="${labels[view]}" title="${labels[view]}">${viewIcon(
				view
			)}</a>`;
		})
		.join('');
...
const filterDropdownOption = (
	...
	return `<label class="filter-checkbox eliqauto-inventory-filter-option">
...
const inventoryFilterDropdown = ({
	...
	const id = `eliqauto-inventory-filter-${name.replace(/[^a-z0-9-]/gi, '-')}`;
	const selectedValues = splitFilterValues(selected);
	...
	return `<div class="eliqauto-inventory-filter-field search-cars__select filter-select-dropdown bg-white" data-name="${escapeHtml(
		name
...
const inventoryActiveFilters = (state: InventoryState, modifierClass = '') => {
	...
	const className = ['eliqauto-inventory-active-filters', modifierClass].filter(Boolean).join(' ');
	const chipsClassName = 'eliqauto-inventory-active-filters__chips';
	...
	const chips = entries
		.flatMap(([key, value]) => {
			...
			return values.map((item) => {
				...
				return `<a class="eliqauto-active-filter" href="${href}" aria-label="Remove ${escapeHtml(
					filterLabels[key]
				)} filter">
	...
	return `<div class="${className}" aria-label="Active inventory filters">
		<p class="eliqauto-inventory-active-filters__summary">${state.selected.length} ${state.selected.length === 1 ? 'match' : 'matches'}</p>
		<div class="${chipsClassName}">
			${chips}
			<a class="eliqauto-active-filter eliqauto-active-filter--clear" href="${inventoryClearFiltersUrl(state)}">
				Clear filters
				<span aria-hidden="true">×</span>
			</a>
		</div>
...
const inventoryShowingText = (state: InventoryState) => {
	...
	if (selectedCount === 0) return `Showing 0 of ${vehicles.length} Eliqauto Listings`;

	if (hasFilters) {
		return `Showing 1 - ${selectedCount} of ${selectedCount} matching Eliqauto Listings`;
	}

	return `Showing 1 - ${selectedCount} of ${selectedCount} Eliqauto Listings`;
};
...
const inventoryLayoutToggle = (state: InventoryState) => {
	...
	return `<a class="eliqauto-inventory-layout-toggle ${dashboard ? 'active' : ''}" href="${href}" data-eliqauto-layout-toggle aria-pressed="${dashboard}" title="${dashboard ? 'Use classic inventory view' : 'Use sidebar dashboard view'}">
		<span>${dashboard ? 'Classic' : 'Sidebar'}</span>
	</a>`;
};

const inventoryViewSwitches = (state: InventoryState) =>
	`<div class="eliqauto-inventory-hero-switches">
		<span class="eliqauto-inventory-hero-switches__label">View</span>
		<div class="listing-tabs menu-tab eliqauto-view-toggle">${viewToggle(state)}${inventoryLayoutToggle(
			state
		)}</div>
	</div>`;
...
const inventoryUtilityToolbar = (state: InventoryState) => {
	...
	return `<div class="eliqauto-inventory-toolbar-row eliqauto-inventory-searchbar__utility">
		<div class="eliqauto-inventory-result-count">
			<button class="btn-filter eliqauto-inventory-searchbar__filter" id="filterSidebarToggle" type="button" aria-label="Open filters" aria-controls="filterSidebar">
		...
		<div class="eliqauto-inventory-sort">
			<span class="eliqauto-inventory-sort__label">Sort Vehicles by</span>
...
const inventoryHeroSearchSurface = (state: InventoryState) => {
	...
	return `<form class="eliqauto-inventory-searchbar" action="/inventory" method="get" role="search" aria-label="Search Eliqauto inventory" data-eliqauto-search-form="inventory">
		${inventorySearchHiddenInputs(state)}
		<div class="eliqauto-inventory-searchbar__row">
			<div class="eliqauto-inventory-searchbar__primary">
				<label class="eliqauto-inventory-searchbar__search">
				...
				<button class="eliqauto-inventory-searchbar__submit" type="submit">Search</button>
			</div>
		</div>
...
const inventorySearchSurface = (state: InventoryState) => {
	...
	return `<form class="eliqauto-inventory-searchbar" action="/inventory" method="get" role="search" aria-label="Search Eliqauto inventory" data-eliqauto-search-form="inventory">
		...
		${inventoryViewSwitches(state)}
		<div class="eliqauto-inventory-filter-grid">
...
const sidebarFilterGroup = ({
	...
	return `<fieldset class="eliqauto-inventory-sidebar-group">
		<legend>${escapeHtml(label)}</legend>
		<div class="eliqauto-inventory-sidebar-options">
...
const inventorySidebarRail = (state: InventoryState) => {
	...
	return `<aside class="eliqauto-inventory-dashboard-sidebar" aria-label="Inventory filters">
		<form class="eliqauto-filter-form eliqauto-inventory-sidebar-form" action="/inventory" method="get">
			...
			<div class="eliqauto-inventory-sidebar-actions">
				<a href="${clearHref}" class="eliqauto-active-filter eliqauto-active-filter--clear">Clear</a>
...
const inventoryBanner = (state: InventoryState) =>
	`<section class="eliqauto-inventory-banner" aria-label="Eliqauto inventory showcase">
	<div class="container">
		<div class="eliqauto-inventory-banner__copy">
			<h1 id="eliqauto-inventory-title" class="eliqauto-sr-only">Eliqauto Inventory</h1>
		</div>
		<div class="eliqauto-inventory-banner__media" aria-hidden="true">
			<img class="eliqauto-inventory-banner__media-image eliqauto-inventory-banner__media-image--generated" src="/assets/eliqauto/hero/home-hero-runway-cars-v1.webp" alt="" width="2172" height="724" loading="eager" decoding="async" fetchpriority="high">
		</div>
...
const compactCardMeta = (vehicle: Vehicle, tagClass = 'tag style2 mb-10 eliqauto-card-specs') =>
	`<ul class="${tagClass}">
	<li><img src="/assets/icons/calendar.svg" alt="year"><span>${vehicle.year}</span></li>
	<li><img src="/assets/icons/gaspump.svg" alt="fuel"><span>${escapeHtml(vehicle.fuel)}</span></li>
	<li><img src="/assets/icons/transmission.svg" alt="transmission"><span>${escapeHtml(vehicle.transmission)}</span></li>
...
const gridCard = (vehicle: Vehicle, index: number) => {
	...
	return `<div class="card-box card-box-style-1 eliqauto-no-image-zoom wow fadeIn" data-wow-delay="0.${(index % 4) + 1}s" data-eliqauto-slug="${escapeHtml(vehicle.slug)}">
		<div class="top">
			<p class="${highlightClass()} text-white highlight">${km(vehicle.mileage)}</p>
			<p class="heart eliqauto-favorite" role="button" tabindex="0" aria-label="Запази ${escapeHtml(vehicle.title)}">${heartIcon}</p>
		</div>
		<div class="image">
			<a href="${url}">
		...
		<div class="content border-light border-top-none">
			...
			<p class="h6 card-box__price eliqauto-card-price mb-15">
				<span class="eliqauto-card-price__amount">${escapeHtml(vehicle.priceLabel)}</span>
				<span class="eliqauto-card-price__finance">
					<span class="eliqauto-card-price__monthly text-sm">${monthly}</span>
					<a href="${url}" class="eliqauto-card-price__finance-link text-underline text-muted text-xs">See Finance</a>
				</span>
			</p>
			<div class="divider mb-15"></div>
			<div class="eliqauto-card-actions flex justify-between">
				<p class="compare-details btn btn-small open-modal" data-modal-id="#CompareModal" data-eliqauto-compare="${escapeHtml(vehicle.slug)}" role="button" tabindex="0">
					${compareIcon}
					Compare
				</p>
...
const listCard = (vehicle: Vehicle) => {
	...
	return `<div class="card-box card-box-style-9 eliqauto-no-image-zoom" data-eliqauto-slug="${escapeHtml(vehicle.slug)}">
		<div class="top">
		...
		<div class="content">
			...
			<p class="text-secondary clamp-1 clamp mb-8">${escapeHtml(vehicle.description || eliqautoContact.appointmentNote)}</p>
			...
			<div class="flex gap-32">
				<p class="compare-details btn btn-small open-modal" data-modal-id="#CompareModal" data-eliqauto-compare="${escapeHtml(vehicle.slug)}" role="button" tabindex="0">${compareIcon} Compare</p>
				<a href="${url}" class="view-details">View details <img class="ml-4" src="/assets/icons/CaretCircleRight.svg" alt="view details"></a>
			</div>
		</div>
	</div>`;
...
const inventoryContent = (state: InventoryState) => {
	...
	const cards = state.selected
		...
		.join('\n');
	const empty = `<div class="card-box card-box-style-1 eliqauto-empty-state">
		<div class="content border-light">
			<p class="h6 card-box__title mb-8">No Eliqauto vehicles match these filters</p>
			<p class="text-secondary mb-15">Clear filters or contact Eliqauto for an import request.</p>
			<a href="/inventory" class="view-details">Reset inventory <img class="ml-4" src="/assets/icons/CaretCircleRight.svg" alt="reset"></a>
		</div>
	</div>`;

	return `<div class="content-tab eliqauto-inventory-content">
...
const replaceInventoryContent = (html: string, state: InventoryState) => {
	...
	return replaceDivBlock(
		...
		`${inventoryActiveFilters(
			state,
			'eliqauto-inventory-active-filters--results'
...
const inventoryDashboard = (
	state: InventoryState
) => `<div class="eliqauto-inventory-dashboard eliqauto-inventory-dashboard--${state.view}">
	${inventorySidebarRail(state)}
	<div class="eliqauto-inventory-dashboard-results">
		${inventoryUtilityToolbar(state)}
		${inventoryActiveFilters(state, 'eliqauto-inventory-active-filters--results')}
		${inventoryContent(state)}
	</div>
	${
		state.view === 'map'
			? `<div class="eliqauto-inventory-dashboard-map">${inventoryMapFallback(state)}</div>`
			: ''
	}
</div>`;
...
const inventoryMainSection = (
	state: InventoryState
) => `<section class="eliqauto-inventory-main pb-100" aria-label="Eliqauto inventory results">
	<div class="container">
...
const compareTable = (
	selected: Vehicle[]
) => `<table class="card-details--table eliqauto-compare-table" data-eliqauto-compare-table>
	<tr>
		<td></td>
		${selected
			.map(
				(vehicle) => `<td data-eliqauto-compare-column="${escapeHtml(vehicle.slug)}">
			<div class="relative top">
				<button class="compare-item-remove-table" type="button" data-eliqauto-compare-remove="${escapeHtml(vehicle.slug)}" style="position: absolute; top: 0; right: 0; background: transparent; border: none; cursor: pointer; padding: 8px; z-index: 10;">
	...
	return replaceDemoVehicleCopy(
		html
			.replaceAll(
				...
				'<h1 class="h2">Сравни автомобили от Eliqauto</h1>'
			)
			.replaceAll(
				'<h2 class="text-center mb-12 capitalize">Compare Cars Side-by-Side</h2>',
				'<h1 class="h2 text-center mb-12 capitalize">Сравни автомобили от Eliqauto</h1>'
			)
			.replaceAll(
				'<h2 class="text-center mb-12 capitalize">Сравни автомобили от Eliqauto</h2>',
				'<h1 class="h2 text-center mb-12 capitalize">Сравни автомобили от Eliqauto</h1>'
			)
			.replaceAll('Compare Cars Side-by-Side', 'Сравни автомобили от Eliqauto')
			.replaceAll(
...
const agentSocials = () => `<ul class="sale-agent-social justify-center flex gap-12">
	${socialLink(eliqautoContact.facebookHref, 'input-facebook.svg', 'Facebook')}
	${socialLink(eliqautoContact.viberHref, 'ChatCircleDots.svg', 'Viber')}
	${socialLink(eliqautoContact.youtubeHref, 'input-youtube.svg', 'YouTube')}
	${socialLink(eliqautoContact.emailHref, 'input-telegram.svg', 'Email')}
</ul>`;
...
const agentCard = (agent: Agent | ManagedAgent, index: number, adminMode = false) => {
	...
	const managementMeta =
		adminMode && isManagedAgent(agent)
			? `<p class="text-secondary text-sm mt-4" data-eliqauto-agent-status="${agent.status}">${escapeHtml(agentStatusText(agent))}</p>
				<p class="text-secondary text-sm mt-4" data-eliqauto-agent-note="${escapeHtml(agent.slug)}">${escapeHtml(agent.note)}</p>`
			: '';
	...
	return `<div class="sale-agent-box ${index === 1 ? 'active' : ''}">
		...
		<div class="card-bottom flex items-center justify-between gap-16">
			...
			<ul class="contact">
				<li>
					<a href="${eliqautoContact.primaryPhoneHref}" aria-label="Call ${escapeHtml(agent.name)}">
						<img src="/assets/icons/PhoneCall.svg" alt="phone">
					</a>
				</li>
				<li>
					<a href="${eliqautoContact.emailHref}" aria-label="Email ${escapeHtml(agent.name)}">
...
const agentGrid = (
	adminMode = false
) => `<div class="grid grid-cols-4 sm-grid-cols-1 lg-grid-cols-2 gap-30 xl-gap-16 eliqauto-agent-grid" data-eliqauto-agent-management="${adminMode}">
...
export const applyAgentsData = (html: string, options: AuxeroRenderOptions = {}) => {
	...
	const next = html
		...
		.replace(
			'<h2>Eliqauto Consultants</h2>',
			adminMode
				? '<h1 class="h2">Управление на консултанти</h1>'
				: '<h1 class="h2">Консултанти на Eliqauto</h1>'
		);
...
const agentInventoryGrid = (agent: Agent) => {
	...
	return `<div class="grid grid-cols-1 gap-20 mb-40 eliqauto-agent-inventory">
		${inventory.vehicles.map((vehicle) => listCard(vehicle)).join('\n')}
	</div>`;
};

const eliqautoMapEmbedSrc =
	'https://maps.google.com/maps?q=Pazardzhik%20Ul.%20Svoboda%20Cemetery%20Park%20Bulgaria&t=&z=14&ie=UTF8&iwloc=&output=embed';
...
export const applyAgentDetailData = (html: string, options: AuxeroRenderOptions = {}) => {
	...
	let next = html
		...
		.replaceAll('Verified Dealer', 'Verified Eliqauto Consultant')
		...
		.replace(
			...
			`<p class="text-secondary mb-40">Eliqauto keeps every conversation practical: source details, inspection context, import or registration steps, and viewing appointments are confirmed before the next commitment.</p>`
		)
		.replace(/Dealer Inventory \(3\)/g, `Eliqauto Inventory (${inventoryState.count})`)
		.replace(
			/<iframe src="https:\/\/www\.google\.com\/maps\/embed\?pb=[^"]*"/g,
			`<iframe src="${eliqautoMapEmbedSrc}"`
		)
		.replaceAll(
			'6205 Peachtree Dunwoody Rd, Atlanta, GA 30328',
			escapeHtml(eliqautoContact.addressLabel)
		)
		.replaceAll('Get Directions', 'Plan Appointment')
		.replaceAll('1-555-678-8888', escapeHtml(eliqautoContact.primaryPhoneLabel))
		.replaceAll('1-555-678-9999', escapeHtml(eliqautoContact.marketplacePhoneLabel))
		...
		.replaceAll('Call To Dealer', 'Call Eliqauto')
		.replaceAll('Chat via WhatsApp', 'Chat on Viber')
		.replaceAll('value="Tony Nguyen"', 'value=""')
		.replaceAll(`value="${eliqautoContact.emailLabel}"`, 'value=""')
	...
	next = next.replace(
		/<a href="\/agents\/eliqauto-sales" class="btn btn-medium btn-primary-3/g,
		`<a href="${eliqautoContact.primaryPhoneHref}" class="btn btn-medium btn-primary-3`
	);
	next = next.replace(
		/<a href="#" class="btn btn-medium btn-primary-4/g,
		`<a href="${eliqautoContact.viberHref}" class="btn btn-medium btn-primary-4`
	);
...
export const applyContactData = (html: string) => {
	let next = html
		...
		.replace(
			...
			'<h1 class="h3 mb-12 capitalize">Свържете се с Eliqauto</h1>'
		)
		.replaceAll('Reach Out to Us', 'Свържете се с Eliqauto')
		.replaceAll(
			'We’re here to assist with any questions, concerns, or inquiries—contact us today!',
			'Пишете за наличен автомобил, внос и лизинг 100%, оглед, документи или продажба.'
		)
		.replaceAll('Address Business', 'Офис Eliqauto')
		.replaceAll('Contact Eliqauto', 'Контакт с Eliqauto')
		.replaceAll(
			'6205 Peachtree Dunwoody Rd, Atlanta, GA 30328',
			escapeHtml(eliqautoContact.addressLabel)
		)
		...
		.replaceAll('Follow Us On social media:', 'Последвайте Eliqauto:')
		.replaceAll(
			"We'd love to hear from you! If you have any questions",
			eliqautoContact.appointmentNote
		)
		.replaceAll('1-555-678-8888', escapeHtml(eliqautoContact.primaryPhoneLabel))
		.replaceAll('1-333-123-6666', escapeHtml(eliqautoContact.marketplacePhoneLabel))
		.replaceAll('tel:1-555-678-8888', eliqautoContact.primaryPhoneHref)
	...
	const contactInfoStart = next.indexOf('Офис Eliqauto');
	const contactInfoLabelStart = next.indexOf('Контакт с Eliqauto', contactInfoStart);

	if (contactInfoLabelStart >= 0) {
		next = `${next.slice(0, contactInfoLabelStart)}Телефон Eliqauto${next.slice(
			contactInfoLabelStart + 'Контакт с Eliqauto'.length
		)}`;
	}

	const contactMapOverlay = `<div class="eliqauto-contact-map__overlay">
		<p class="eliqauto-contact-map__eyebrow">Eliqauto Пазарджик</p>
		<h1>Огледи и консултации с уговорка</h1>
		<p>${escapeHtml(eliqautoContact.addressLabel)}</p>
		<div class="eliqauto-contact-map__actions">
			<a href="${eliqautoContact.primaryPhoneHref}" class="btn btn-white text-primary btn-large font-weight-600">${escapeHtml(eliqautoContact.primaryPhoneLabel)}</a>
			<a href="${eliqautoContact.viberHref}" class="btn btn-primary btn-large font-weight-600">Пиши във Viber</a>
		</div>
	</div>`;
	...
	next = next
		.replace(
			...
			'class="widget-gg-map eliqauto-contact-map flex radius-8 overflow-hidden"'
		)
		.replace(
			/(<div class="widget-gg-map eliqauto-contact-map[\s\S]*?<iframe[\s\S]*?<\/iframe>)(\s*<\/div>)/,
			`$1${contactMapOverlay}$2`
		);

	next = addClassToFirstFormAfter(next, 'contact-page-form', 'eliqauto-contact-form');
...
const replaceFilterSidebar = (html: string, state: InventoryState) => {
	...
	if (formStart >= 0) {
		next = `${next.slice(0, formStart)}<form action="/inventory" method="get" class="eliqauto-filter-form">
...
const mapLocationGroups = (source: Vehicle[]) => {
	...
	for (const vehicle of source) {
		const location = vehicle.location.trim() || eliqautoContact.addressLabel;
...
const mapLocationList = (groups: MapLocationGroup[]) => {
	if (!groups.length) {
		return `<p class="text-secondary mb-16" data-eliqauto-map-empty="true">Няма автомобили по тези филтри. Изчисти филтрите или се свържи с Eliqauto за входящи автомобили.</p>`;
	}

	return `<ul class="eliqauto-map-fallback__locations">
		${groups
			.map(
				(group) => `<li data-eliqauto-map-location="${escapeHtml(group.location)}">
...
const inventoryMapFallback = (state: InventoryState) => {
	...
	return `<div id="map" class="eliqauto-map-fallback" data-map-zoom="16" data-map-scroll="true" data-eliqauto-map-selected="${state.selected.length}">
				<div class="eliqauto-map-fallback__inner">
					<p class="h4 mb-12">Зона за огледи Eliqauto</p>
					<p class="text-secondary mb-8">${escapeHtml(eliqautoContact.addressLabel)}</p>
					<p class="text-secondary mb-12" data-eliqauto-map-summary="${escapeHtml(matchingVehicleLabel(state.selected.length))}">${escapeHtml(matchingVehicleLabel(state.selected.length))}, групирани по локация за оглед.</p>
					${mapLocationList(groups)}
					<p class="text-secondary mb-16">${escapeHtml(eliqautoContact.appointmentNote)}. Точната локация за оглед се потвърждава по телефона.</p>
					<a class="btn btn-medium btn-primary-3 font-weight-600" href="${eliqautoContact.primaryPhoneHref}">${escapeHtml(eliqautoContact.primaryPhoneLabel)}</a>
				</div>
			</div>`;
};
...
export const applyInventoryData = (
	...
	return replaceDemoVehicleCopy(next)
		.replaceAll('Listing Grid 4 Columns', 'Eliqauto Inventory')
		.replaceAll('Listing Grid 3 Columns', 'Eliqauto Inventory')
		.replaceAll('Listing Grid Half Map', 'Eliqauto Inventory Map')
	...
	const chunks = [
		...
		[
			...
			eliqautoFetchedAt
				...
				: 'Inventory refreshed from Eliqauto source data'
		]
	];
...
export const applyDetailData = (html: string, options: AuxeroRenderOptions = {}) => {
	...
	let next = html
		...
		.replace(
			...
			escapeHtml(
				vehicle.description ||
					`${vehicle.title} is available through Eliqauto with source review and viewing by appointment.`
			)
		)
		...
		.replaceAll(
			...
			'Estimated over 72 months. Final terms confirmed by Eliqauto.'
		)
		.replaceAll(
		...
		.replaceAll('Verified Dealer', 'Eliqauto Consultant')
		.replaceAll('1-555-678-8888', escapeHtml(eliqautoContact.primaryPhoneLabel))
		.replaceAll('1-555-678-9999', escapeHtml(eliqautoContact.marketplacePhoneLabel))
		.replaceAll('tel:1-555-678-8888', eliqautoContact.primaryPhoneHref)
		.replaceAll('tel:1-555-678-9999', eliqautoContact.marketplacePhoneHref)
	...
	next = next
		...
		.replaceAll('Randynox', 'Клиент от Пазарджик')
		.replaceAll('Mista Nyroom', 'Купувач от Пазарджик')
		...
		.replaceAll(
			...
			'Eliqauto ни помогна да изберем автомобил с ясна история и реален пробег. Огледът беше организиран спокойно, а документите бяха обяснени предварително.'
		)
		.replaceAll(
		...
		.replace(
			...
			`<iframe src="${escapeHtml(eliqautoContact.mapEmbedUrl)}"`
		);

	next = next
		.replace(
			'<div class="listing-details">',
			`<div class="listing-details" data-eliqauto-slug="${escapeHtml(vehicle.slug)}" data-eliqauto-detail="true">`
		)
		.replace(
			'<a href="#" class="btn btn-medium btn-line open-modal padding-button-medium gap-5 font-weight-600" data-modal-id="#CardModal">',
			`<a href="/compare" class="btn btn-medium btn-line open-modal padding-button-medium gap-5 font-weight-600" data-modal-id="#CompareModal" data-eliqauto-compare="${escapeHtml(vehicle.slug)}" role="button" aria-label="Compare ${escapeHtml(vehicle.title)}">`
		)
		.replace(
			'<a href="#" class="btn-icon-circle hover-stroke-white">',
			`<a href="/account" class="btn-icon-circle hover-stroke-white eliqauto-favorite" role="button" aria-label="Запази ${escapeHtml(vehicle.title)}">`
		)
		.replace(
	...
	return replaceDemoVehicleCopy(next)
		...
		.replace(/href="dealer-details\.html"/g, 'href="/agents/eliqauto-sales"')
