...
import { resolveEliqautoSession } from './auth';
...
import { getEliqautoGarageState } from './garage';
import {
	accountAvatarByRole,
	accountContext,
...
const menuItem = (
	...
	return `<li>
	<a href="${href}" class="dashboard-menu-item ${isActive ? 'active' : ''}" data-eliqauto-menu-item="${escapeHtml(id)}"${isActive ? ' aria-current="page"' : ''}>
		<img src="${icon}" alt="${escapeHtml(label)}">
		${escapeHtml(label)}
		${badge ? `<span class="number" data-eliqauto-menu-badge="${escapeHtml(id)}">${badge}</span>` : ''}
	</a>
</li>`;
};
...
const accountDropdownItem = ({ href, icon, id, label }: DashboardMenuItem) => `<li>
	<a href="${href}" data-eliqauto-user-menu-item="${escapeHtml(id)}">
...
const applyAccountShell = (
	...
	const listingHeading = context.isAdmin ? 'Eliqauto Inventory' : 'Моите автомобили';

	const next = html
		.replaceAll('Eliqauto Admin', escapeHtml(context.session.name))
		.replaceAll(
		...
		.replace('</head>', `<meta name="eliqauto-role" content="${context.session.role}">\n</head>`);
...
const statCards = (context: AccountContext) => {
	...
	${stats
		.map(
			(
				stat
			) => `<a href="${stat.href}" class="dashboard-cart" data-eliqauto-dashboard-stat="${escapeHtml(stat.id)}" data-eliqauto-stat-value="${escapeHtml(stat.value)}">
		<div>
...
const inventoryCartItem = (row: AuxeroAccountListingRow) => {
	...
	return `<div class="cart-item" data-eliqauto-slug="${escapeHtml(row.id)}">
...
) => `<div class="cart-wrapper eliqauto-account-listings" data-eliqauto-account-listings>
...
const submissionCartItem = (row: AuxeroAccountListingRow) => {
	...
	return `<div class="cart-item" data-eliqauto-submission-id="${escapeHtml(row.id)}">
...
const submissionCartWrapper = (
	data: AuxeroAccountListingsData
) => `<div class="cart-wrapper eliqauto-account-listings" data-eliqauto-account-listings data-eliqauto-submissions-table="true">
	<div class="cart-header">
		${data.headers.map((header) => `<div class="font-weight-600">${escapeHtml(header)}</div>`).join('\n')}
	</div>
	<div class="cart-items">
...
) => `<div class="cart-wrapper eliqauto-users-table" data-eliqauto-users-table>
	<div class="cart-header">
		${data.headers.map((header) => `<div class="font-weight-600">${escapeHtml(header)}</div>`).join('\n')}
	</div>
	<div class="cart-items">
		${data.rows
			.map(
				(
					user
				) => `<div class="cart-item" data-eliqauto-user-id="${escapeHtml(user.id)}" data-eliqauto-user-kind="${escapeHtml(user.kind)}" data-eliqauto-user-role="${escapeHtml(user.role.toLowerCase())}">
			<div class="cart-item__product">
...
) => `<div class="dashboard-box bg-white eliqauto-users-box">
...
const recentInquiriesBox = (context: AccountContext) => {
	...
	return `<div class="dashboard-box bg-white eliqauto-inquiries-box" data-eliqauto-dashboard-recent>
	...
	<div class="grid grid-cols-3 md-grid-cols-1 gap-1 mb-20" aria-label="Dashboard summary">
		${data.summary
			.map(
				(
					item
				) => `<div class="bg-white p-16" data-eliqauto-dashboard-summary="${escapeHtml(item.id)}">
	...
	<div class="grid grid-cols-4 xl-grid-cols-2 sm-grid-cols-1 gap-10 mb-24" aria-label="Quick dashboard actions">
		${data.actions
			.map(
				(
					action
				) => `<a href="${escapeHtml(action.href)}" class="car-box" data-eliqauto-dashboard-action="${escapeHtml(action.id)}">
...
const favoriteCard = (vehicle: Vehicle) => {
	...
	return `<div class="card-box card-box-style-1" data-eliqauto-slug="${escapeHtml(vehicle.slug)}">
	<div class="top">
		<p class="${tagClass} text-white highlight">${escapeHtml(vehicle.tag ?? 'Available')}</p>
		<p class="heart eliqauto-favorite is-active" role="button" tabindex="0" aria-label="Remove ${escapeHtml(vehicle.title)}">
			<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 14C8 14 1.5 10.5 1.5 6.375C1.5 5.47989 1.85558 5 4.875 3C6.28688 3 7.49625 3.76937 8 5C8.50375 3.76937 9.71312 3 11.125 3C14.1444 5 14.5 5.47989 14.5 6.375C14.5 10.5 8 14 8 14Z" stroke="white" fill="var(--bc-accent)"/></svg>
		</p>
	</div>
	...
	<div class="content border-light border-top-none">
		...
		<div class="flex justify-between">
			<p class="compare-details btn btn-small open-modal" data-modal-id="#CompareModal" data-eliqauto-compare="${escapeHtml(vehicle.slug)}" role="button" tabindex="0">Compare</p>
...
const favoriteGrid = (context: AccountContext) => {
	const savedSlugs = getEliqautoGarageState(context.session).favorites;
	...
	const content = savedVehicles.length
		...
		: `<div class="dashboard-box bg-white" data-eliqauto-favorites-empty="true">
			<p class="h4 mb-8">No saved Eliqauto vehicles yet</p>
			<p class="text-secondary">Use the heart action on inventory cards to build your saved list.</p>
		</div>`;

	return `<div class="grid grid-cols-3 lg-grid-cols-2 md-grid-cols-1 gap-x-23 gap-y-30 mb-30" data-eliqauto-favorites-grid data-eliqauto-favorites-count="${savedVehicles.length}">
	${content}
</div>`;
};
...
const messageContainer = (context: AccountContext) => {
	...
	return `<div class="message-container" data-eliqauto-message-container>
...
const applyDashboardData = (
	...
	next = replaceFirstDivAfter(
		next,
		context.isAdmin ? 'Eliqauto Inventory' : 'My Listings',
...
const applyProfileData = (
	...
	next = addClassToFirstFormAfter(
		next,
		'My profile',
		'eliqauto-profile-form',
		' method="POST" data-eliqauto-profile-form'
	);
	next = next.replace(
		'</form>',
...
const applyPasswordData = (
	...
	let next = applyAccountShell(html, templateFile, options)
		.replaceAll(`value="${eliqautoContact.emailLabel}|"`, `value="${password.email}"`)
		.replaceAll('value="eliqauto@2026"', 'value=""')
		.replaceAll('placeholder="Password"', 'placeholder="Password"');

	next = addClassToFirstFormAfter(
		next,
		'Change Password',
		'eliqauto-password-form',
		' method="POST" data-eliqauto-password-form'
	);
...
const applyAddListingData = (
	...
	let next = applyAccountShell(html, templateFile, options)
		...
		.replace(
			...
			'<a href="#" class="btn btn-line-1 px-24 btn-large font-weight-600 eliqauto-local-form-action" data-eliqauto-form-target=".eliqauto-add-listing-form" data-eliqauto-form-status="Listing draft saved locally for Eliqauto review" data-eliqauto-submit-form="true" data-eliqauto-listing-status="draft">'
		)
		.replace(
			'<a href="#" class="btn btn-primary px-24 btn-large font-weight-600">',
			`<a href="#" class="btn btn-primary px-24 btn-large font-weight-600 eliqauto-local-form-action" data-eliqauto-form-target=".eliqauto-add-listing-form" data-eliqauto-form-status="Listing submitted locally for Eliqauto" data-eliqauto-submit-form="true" data-eliqauto-listing-status="${primaryListingStatus}">`
		)
	...
	next = addClassToFirstFormAfter(
		next,
		pageHeading,
		'eliqauto-add-listing-form',
		` data-eliqauto-admin-listing-mode="${form.mode}" data-eliqauto-add-listing-form`
	);

	next = next.replace(
		`<form action="#" class="eliqauto-add-listing-form" novalidate data-eliqauto-admin-listing-mode="${form.mode}" data-eliqauto-add-listing-form>`,
		`<form action="" method="POST" enctype="multipart/form-data" class="eliqauto-add-listing-form" novalidate data-eliqauto-admin-listing-mode="${form.mode}" data-eliqauto-native-submit="true" data-eliqauto-add-listing-form>\n${hiddenFields}`
	);
...
const replaceDashboardDemoText = (html: string) =>
	demoVehicleTitles
		...
		.replaceAll('Tampa, FL', eliqautoContact.addressLabel)
		...
		.replaceAll('Randynox', 'Eliqauto Lead')
		.replaceAll('Mista Nyroom', 'Eliqauto Customer')
		.replaceAll('Marvin McKinney', 'Eliqauto Sales')
		.replaceAll('John Smith', 'Eliqauto Customer')
		.replaceAll('Brooklyn Simmons', 'Eliqauto Inspection')
		.replaceAll('Arlene McCoy', 'Eliqauto Import')
		.replaceAll('Darrell Steward', 'Eliqauto Admin')
		.replaceAll('Theresa Webb', 'Eliqauto Support')
		.replaceAll('grew-sra@gmail.com', eliqautoContact.emailLabel)
		.replaceAll('Hey! there I&#39;m available', 'Eliqauto follow-up is ready')
		.replaceAll("Hey! there I'm available", 'Eliqauto follow-up is ready');
...
export const getAuxeroAccountRuntimeData = (options: AuxeroRenderOptions = {}) => {
	const session = resolveEliqautoSession(options.routePath ?? '', options.searchParams);

	return {
		activeRole: session.role,
		session,
		sessions: Object.values({
			admin: resolveEliqautoSession('admin'),
			agent: resolveEliqautoSession('agent'),
			customer: resolveEliqautoSession('account')
		})
	};
};
