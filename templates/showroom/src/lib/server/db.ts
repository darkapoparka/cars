...
import type {
	EliqautoInquiryRecord,
	EliqautoInventoryListingRecord,
	EliqautoMessageRecord,
	EliqautoPasswordChangeRecord,
	EliqautoRole,
	EliqautoSessionRecord,
	EliqautoUser,
	EliqautoUserStatus,
	EliqautoVehicleSubmissionRecord
...
export type {
	EliqautoInquiryRecord,
	EliqautoInquiryStatus,
	EliqautoInventoryListingRecord,
	EliqautoInventoryListingStatus,
	EliqautoMessageRecord,
	EliqautoMessageStatus,
	EliqautoPasswordChangeRecord,
	EliqautoRole,
	EliqautoSessionRecord,
	EliqautoUser,
	EliqautoUserStatus,
	EliqautoVehicleSubmissionRecord,
	EliqautoVehicleSubmissionStatus
} from '$lib/types/account';
...
const prototypeUsers: EliqautoUser[] = [
	{
	...
	},
	{
		...
		phone: eliqautoContact.marketplacePhoneLabel,
		role: 'customer',
		status: 'active'
	}
];

const seededInquiries: EliqautoInquiryRecord[] = vehicles.slice(0, 3).map((vehicle, index) => ({
	assignedAgentSlug: vehicle.agentSlug,
	contactEmail: index === 0 ? 'customer@eliqauto.local' : eliqautoContact.emailLabel,
	contactName: ['Import & leasing lead', 'Sell-your-car submission', 'Document review'][index],
	contactPhone: eliqautoContact.primaryPhoneLabel,
...
const sessions: EliqautoSessionRecord[] = [];
const inquiries: EliqautoInquiryRecord[] = [...seededInquiries];
const messages: EliqautoMessageRecord[] = [
	{
		authorEmail: 'customer@eliqauto.local',
		authorName: 'Eliqauto Customer',
...
const seededVehicleSubmissions: EliqautoVehicleSubmissionRecord[] = [
	{
		contactEmail: 'seller@eliqauto.local',
	...
	{
		contactEmail: 'tradein@eliqauto.local',
		contactName: 'Trade-in customer',
		contactPhone: eliqautoContact.marketplacePhoneLabel,
		createdAt: '2026-05-24T14:30:00.000Z',
		expectedPrice: 'On request',
		id: 'submission-seed-2',
		message: 'Customer asked whether Eliqauto can prepare a direct offer or client listing.',
		mileage: '98 000 km',
...
const passwordChanges: EliqautoPasswordChangeRecord[] = [];

export const listEliqautoUsers = () => prototypeUsers.map((user) => ({ ...user }));

export const findEliqautoUserByEmail = (email: string) =>
	prototypeUsers.find((user) => user.email.toLowerCase() === email.trim().toLowerCase());

export const findEliqautoUserByRole = (role: EliqautoRole) =>
	prototypeUsers.find((user) => user.role === role);

export const createEliqautoUserRecord = ({
	email,
	name,
	phone,
	role = 'customer',
	...
	const existing = findEliqautoUserByEmail(email);

	if (existing) return { ...existing };

	const user: EliqautoUser = {
		email: email.trim().toLowerCase(),
		id: nextId('user'),
		name: name?.trim() || 'Eliqauto Customer',
		phone: phone?.trim() || eliqautoContact.marketplacePhoneLabel,
		role,
		status
	};
...
export const updateEliqautoUserProfile = ({
	email,
	name,
	phone,
	role
	...
	const user =
		(email ? findEliqautoUserByEmail(email) : undefined) ??
		(role ? findEliqautoUserByRole(role) : undefined);
...
export const updateEliqautoUserRecord = ({
	email,
	id,
	name,
	phone,
...
export const createEliqautoSessionRecord = (user: EliqautoUser): EliqautoSessionRecord => {
...
export const listEliqautoSessions = () => [...sessions];

export const findEliqautoSessionByToken = (token: string) =>
	sessions.find((session) => session.token === token);

export const deleteEliqautoSessionByToken = (token: string) => {
...
export const listEliqautoInquiries = () => [...inquiries];
...
export const createEliqautoInquiryRecord = (
	input: Partial<EliqautoInquiryRecord>
): EliqautoInquiryRecord => {
	...
	const fallbackAgent = agents[0]?.slug ?? 'eliqauto-sales';
	const record: EliqautoInquiryRecord = {
		assignedAgentSlug:
		...
		contactName: input.contactName?.trim() || 'Eliqauto website lead',
		contactPhone: input.contactPhone?.trim() || eliqautoContact.primaryPhoneLabel,
		createdAt: stamp(),
		id: nextId('inquiry'),
		message: input.message?.trim() || 'Website inquiry queued for Eliqauto follow-up.',
		routePath,
...
export const updateEliqautoInquiryRecord = (
	id: string,
	patch: Partial<Pick<EliqautoInquiryRecord, 'assignedAgentSlug' | 'message' | 'status'>>
) => {
...
export const listEliqautoMessages = () => [...messages];

export const createEliqautoMessageRecord = (
	input: Partial<EliqautoMessageRecord>
): EliqautoMessageRecord => {
	const record: EliqautoMessageRecord = {
		authorEmail: input.authorEmail?.trim() || eliqautoContact.emailLabel,
		authorName: input.authorName?.trim() || 'Eliqauto website visitor',
		createdAt: stamp(),
		id: nextId('message'),
		message: input.message?.trim() || 'Message queued for Eliqauto.',
		routePath: input.routePath ?? '/account/messages',
		status: input.status ?? 'open',
		threadId: input.threadId?.trim() || 'eliqauto-sales',
...
export const updateEliqautoMessageRecord = (
	id: string,
	patch: Partial<Pick<EliqautoMessageRecord, 'message' | 'status'>>
) => {
...
const writePersistedVehicleSubmissions = (records: EliqautoVehicleSubmissionRecord[]) =>
	writeCmsCollection('vehicle-submissions', records);

export const listEliqautoVehicleSubmissions = () => {
	...
	// promoted into the persisted collection (see updateEliqautoVehicleSubmissionRecord),
...
export const listEliqautoInventoryListings = ({
...
const normalizeListingRecord = (
	input: Partial<EliqautoInventoryListingRecord>,
	existing?: EliqautoInventoryListingRecord
): EliqautoInventoryListingRecord => {
	const createdAt = existing?.createdAt ?? input.createdAt ?? stamp();
	const id = existing?.id ?? (trimmedValue(input.id) || nextId('listing'));
	const title = trimmedValue(input.title) || existing?.title || 'Eliqauto inventory draft';
	...
	return {
		...
		brand: trimmedValue(input.brand) || existing?.brand || title.split(/\s+/)[0] || 'Eliqauto',
		color: trimmedValue(input.color) || existing?.color || 'On request',
		createdAt,
		description:
			trimmedValue(input.description) ||
			existing?.description ||
			`${title} is queued in the Eliqauto CMS for review.`,
		...
		id,
		location: trimmedValue(input.location) || existing?.location || eliqautoContact.addressLabel,
...
export const createEliqautoInventoryListingRecord = (
	input: Partial<EliqautoInventoryListingRecord>
): EliqautoInventoryListingRecord => {
...
export const updateEliqautoInventoryListingRecord = (
	id: string,
	patch: Partial<EliqautoInventoryListingRecord>
) => {
	const records = readCmsCollection('inventory-listings');
...
export const archiveEliqautoInventoryListingRecord = (id: string) =>
	updateEliqautoInventoryListingRecord(id, { status: 'archived' });

export const createEliqautoVehicleSubmissionRecord = (
	input: Partial<EliqautoVehicleSubmissionRecord>
): EliqautoVehicleSubmissionRecord => {
	const records = persistedVehicleSubmissions();
	const record: EliqautoVehicleSubmissionRecord = {
		contactEmail: input.contactEmail?.trim() || eliqautoContact.emailLabel,
		contactName: input.contactName?.trim() || 'Eliqauto customer',
		contactPhone: input.contactPhone?.trim() || eliqautoContact.primaryPhoneLabel,
		createdAt: stamp(),
...
export const updateEliqautoVehicleSubmissionRecord = (
	id: string,
	patch: Partial<
		Pick<
			EliqautoVehicleSubmissionRecord,
	...
	if (index >= 0) {
		...
		// change actually persists (listEliqautoVehicleSubmissions dedupes the seed away).
		records.unshift(record);
	}
	writePersistedVehicleSubmissions(records);
...
export const createEliqautoPasswordChangeRecord = (
	user: EliqautoUser
): EliqautoPasswordChangeRecord => {
	const record: EliqautoPasswordChangeRecord = {
		createdAt: stamp(),
		email: user.email,
		id: nextId('password'),
		role: user.role,
...
export const listEliqautoPasswordChanges = () => [...passwordChanges];
