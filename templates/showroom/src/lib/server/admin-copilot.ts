...
export const copilotSystemPrompt = () =>
	[
		'You are Eliqauto AI Copilot, an admin-only assistant for the Eliqauto CMS.',
...
export const fallbackCopilotResponse = ({
	...
	if (task === 'draft-description' && selectedListing) {
		return [
			`${selectedListing.title} is available through Eliqauto with ${selectedListing.mileage} recorded mileage and ${selectedListing.transmission.toLowerCase()} transmission.`,
