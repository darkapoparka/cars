...
import type { EliqautoCmsDocument, EliqautoInventoryListingRecord } from '$lib/types/account';
import {
	archiveEliqautoInventoryListingRecord,
	createEliqautoInventoryListingRecord,
	createEliqautoVehicleSubmissionRecord,
	listEliqautoInventoryListings,
	listEliqautoVehicleSubmissions,
	updateEliqautoInventoryListingRecord,
	updateEliqautoVehicleSubmissionRecord,
	type EliqautoInventoryListingStatus,
	type EliqautoVehicleSubmissionStatus
} from './db';
...
export const listInventoryForAdmin = ({
	...
	...listEliqautoInventoryListings({ includeArchived })
];

export const listVehicleSubmissions = () => listEliqautoVehicleSubmissions();
...
export const updateInventoryListing = (id: string, input: InventoryListingInput) =>
	updateEliqautoInventoryListingRecord(id, {
		bodyType: input.bodyType,
		brand: input.brand,
		color: input.color,
		description: input.description,
...
export const archiveInventoryListing = (id: string) => archiveEliqautoInventoryListingRecord(id);

export const updateInventoryListingStatus = (
	id: string,
	status: EliqautoInventoryListingStatus | string
) => updateEliqautoInventoryListingRecord(id, { status: normalizeCmsListingStatus(status) });

export const duplicateInventoryListing = (id: string) => {
	const source = listEliqautoInventoryListings({ includeArchived: true }).find(
		(candidate) => candidate.id === id || candidate.slug === id
	);

	if (!source) return undefined;

	return createEliqautoInventoryListingRecord({
		...source,
		createdAt: undefined,
		id: undefined,
		routePath: undefined,
...
export const createVehicleSubmission = (input: VehicleSubmissionInput) => {
	const submission = createEliqautoVehicleSubmissionRecord({
...
export const updateVehicleSubmission = (input: VehicleSubmissionUpdateInput) => {
	const submission = updateEliqautoVehicleSubmissionRecord(input.id, {
		documents: input.documents,
		expectedPrice: input.expectedPrice,
		galleryImages: input.galleryImages,
		message: input.message,
	...
	if (submission?.source === 'admin-listing') {
		updateEliqautoInventoryListingRecord(submission.id, {
			mileage: numberFromInput(submission.mileage),
			priceLabel: submission.expectedPrice,
			status: submission.status === 'published' ? 'published' : 'draft',
			title: submission.title,
