...
export const accountAvatarByRole: Record<EliqautoRole, string> = {
	admin: '/assets/eliqauto/team/avatar-sales.svg',
	agent: agents[1]?.image ?? '/assets/eliqauto/team/avatar-logistics.svg',
	customer: '/assets/eliqauto/team/avatar-inspection.svg'
};
...
const dashboardAgentLabel = (slug?: string) => {
	const labels: Record<string, string> = {
		'eliqauto-import': 'Import team',
		'eliqauto-inspection': 'Inspection team',
		'eliqauto-sales': 'Sales team'
	};

	return (slug && labels[slug]) || 'Eliqauto team';
};
...
export const accountContext = (
	...
	const session = options.session ?? resolveEliqautoSession(routePath, options.searchParams);
	...
	return {
		...
		isAdmin,
		roleLabel: eliqautoRoleLabel(session.role),
		session
	};
};
...
export const accountDashboardStatsData = (context: AccountContext): DashboardStat[] => {
	...
	const garage = context.isAdmin ? undefined : getEliqautoGarageState(context.session);
...
export const accountDashboardRecentData = (context: AccountContext): AuxeroDashboardRecentData => {
	...
	const items: RecentDashboardItem[] = messages.slice(0, 3).map((message) => ({
		avatarRole: 'agent' as EliqautoRole,
		body: message.message,
		date: message.createdAt,
		name: message.threadId === 'eliqauto-sales' ? 'Eliqauto Sales' : message.authorName,
		title: message.vehicleSlug
			? (vehicles.find((vehicle) => vehicle.slug === message.vehicleSlug)?.title ??
				'Автомобил на Eliqauto')
			: message.threadId
	}));
	...
	return {
		...
		intro: 'Продължи последния разговор с Eliqauto и движи запазените автомобили напред.',
		items: items.map((item, index) => {
			...
			return {
				...
				metaLabel: message?.threadId ?? 'Продажби Eliqauto',
				name: item.name,
