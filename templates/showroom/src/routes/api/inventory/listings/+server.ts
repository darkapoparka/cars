...
import type { EliqautoInventoryListingStatus } from '$lib/server/db';
import type { EliqautoCmsDocument } from '$lib/types/account';
...
): EliqautoInventoryListingStatus | undefined => {
...
const payloadDocuments = (payload: Record<string, unknown>) => {
	...
	return value
		.map((document) => {
			...
			const candidate = document as Partial<EliqautoCmsDocument>;
		...
		.filter((document): document is EliqautoCmsDocument => Boolean(document));
};
...
export async function PATCH({ request }: { request: Request }) {
	...
	if (!listing) {
		return errorJson('Eliqauto inventory listing not found', 404);
	}
