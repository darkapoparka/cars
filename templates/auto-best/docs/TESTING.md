...
| `npm run check:mobile` | Mobile accessibility and interaction checks |
| `npm run check:resilience` | Failure cases, state restoration and integration boundaries |
| `npm run check:docs` | Active Markdown links, commands and personalization paths |
| `npm run visual:baseline` | Capture the reference images for the visual comparison |
| `npm run visual:compare` | Compare against the existing reference images |
...
The current local `check` script includes `--fail-on-warnings`. Its `validate` chain also checks active documentation; `quality` adds mobile accessibility and resilience after smoke. These are configured behaviors, not substituted test results.
...
The local source also contains `home-hierarchy-smoke.mjs`, `mobile-quality.mjs`, `mobile-resilience.mjs` and `visual-regression.mjs`. Accessibility uses axe-core and needs review beyond automated results. Visual capture uses `visual:baseline`; comparison uses `visual:compare`. Both need the same intended server/browser setup. Preserve the actual before images when comparing a structural change.
