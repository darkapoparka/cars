(function($){$(document).on('click','#buy-car-online',function(e){e.preventDefault();var thisBtn=$(this);var carId=$(this).data('id');$.ajax({url:ajaxurl,type:"POST",dataType:'json',context:this,data:'car_id='+carId+'&action=stm_ajax_buy_car_online&security='+stm_security_nonce,beforeSend:function(){thisBtn.addClass('buy-online-load');},success:function(data){thisBtn.removeClass('buy-online-load');if(data.status=='success'){window.location=data.redirect_url;}}});});function sortByListingType(){$('body').on('change','.select-listing-type select',function(){let $select=$(this);let listingType=$select.val();let viewType=$('#stm_user_dealer_view_type').val();let listingsView=$('#stm-dealer-view-type').val();let userId=$select.data('user');let popular=listingsView==='popular'?'yes':'no';let userPublic=$select.data('user-public');let userPrivate=$select.data('user-private');let userFavourite=$select.data('user-favourite');let postsPerPage=parseInt($select.data('posts-per-page'),10);let offset=parseInt($select.data('offset'),10);if(!postsPerPage){postsPerPage=parseInt($('#sort_by_select').data('posts-per-page'),10)||-1;}
if(userPrivate){viewType='list';}else if(!viewType){viewType='grid';}
if(offset===0){offset=postsPerPage;}
$.ajax({url:ajaxurl,data:{action:'mvl_ajax_dealer_load_listings_by_type',listing_type:listingType,user_id:userId,popular:popular,view_type:viewType,user_public:userPublic,user_private:userPrivate,user_favourite:userFavourite,posts_per_page:postsPerPage,offset:offset,security:stm_security_nonce},method:'POST',dataType:'json',beforeSend:function(){$('.select-listing-type select').prop("disabled",true);},success:function(data){$('.select-listing-type select').prop("disabled",false);if(data){const $target=userPrivate||userFavourite?$('.archive-listing-page .car-listing-row'):userPublic?$('.archive-listing-page'):$('#'+listingsView).find('.car-listing-row');$target.html(data.html);}
...
$(document).on('change','#sort_by_select',function(){var sortBy=$(this).val()
var user_id=$(this).data('user')
var posts_per_page=$(this).data('posts-per-page')
var listing_type=$('.select-listing-type select').val()||'all'
var data={action:'stm_sort_listings',sort_by:sortBy,user_id:user_id,security:stm_security_nonce,page:1,posts_per_page:posts_per_page,listing_type:listing_type,}
...
var listing_type=$('.select-listing-type select').val()||'all'
var data={action:'stm_sort_listings',sort_by:sortBy,user_id:user_id,page:page,posts_per_page:posts_per_page,listing_type:listing_type,security:stm_security_nonce,}
$.ajax({url:ajaxurl,type:'POST',data:data,success:function(response){if(response.success){$('.car-listing-row').html(response.data.listings_html)
$('.pagination-container').html(response.data.pagination_html)
