			...
			<nav class="about-hero-contact" aria-label="Контакти и социални мрежи">
				...
				{#if daynightSite.socialLinks.facebook || daynightSite.socialLinks.instagram || youtubeChannelUrl}
					<div class="about-hero-socials">
						...
						{#if youtubeChannelUrl}<a href={youtubeChannelUrl} target="_blank" rel="noopener noreferrer" aria-label="YouTube"><img src={resolve('/variant-3/assets/icons/youtube-footer.svg')} alt="" width="22" height="22" /></a>{/if}
					</div>
				{/if}
			</nav>
		</div>
			...
			<div class="about-brands">
				{#each brands as brand (brand.brand)}
					<a href={resolve(`/inventory?brand=${encodeURIComponent(brand.brand)}`)}>
						<img
							src={resolve(`/variant-3/assets/images/brand/mobile/${brand.image}.svg`)}
							alt=""
							width="36"
							height="28"
							loading="lazy"
