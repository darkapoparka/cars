...
const leadImportFlagArtwork = {
  ...
  CN: (
    <svg aria-hidden="true" viewBox="0 0 32 20">
      ...
      <path
        d="m6.2 2.1.86 2.65h2.79L7.6 6.38l.86 2.65L6.2 7.4 3.95 9.03l.86-2.65-2.26-1.63h2.79z"
        fill="#ffde00"
      />
  ...
  KR: (
    <svg aria-hidden="true" viewBox="0 0 32 20">
      ...
      <path
        d="m6.8 5.1 4-2.3m-3.4 3.4 4-2.3M21.2 16l4-2.3m-4.6 1.2 4-2.3M21.3 3l4 2.3m-4.7-1.1 4 2.3M6.8 14.8l4 2.3m-4.6-1.1 4 2.3"
        stroke="#111"
        strokeWidth=".75"
      />
    </svg>
  ...
  US: (
    <svg aria-hidden="true" viewBox="0 0 32 20">
      <path d="M0 0h32v20H0z" fill="#fff" />
      <path
        d="M0 0h32v1.54H0zm0 3.08h32v1.54H0zm0 3.07h32v1.54H0zm0 3.08h32v1.54H0zm0 3.08h32v1.54H0zm0 3.07h32v1.54H0zm0 3.08h32V20H0z"
        fill="#b22234"
      />
    ...
    <label className="m-1.5 grid min-w-0 content-center rounded-xl bg-control px-4 py-1.5">
      ...
      >
        <option disabled value="">
          {localizeMarketplace(isBg, "Изберете марка", "Choose a make")}
        </option>
...
const DesktopImportSurface = ({
  ...
  const selectedOrigin = leadImportOrigins.find(
    (origin) => origin.code === originCode
  );
  const originValue = selectedOrigin
    ? selectedOrigin[isBg ? "labelBg" : "labelEn"]
  ...
  return (
    ...
    >
      ...
      {originCode ? (
        <input name="origin" type="hidden" value={originCode} />
      ) : null}
        ...
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              originOpen &&
                "bg-border/75 ring-2 ring-[var(--lead-site-accent-ring)] ring-inset"
            )}
            type="button"
            variant="ghost"
          >
      ...
      <Popover onOpenChange={setDestinationOpen} open={destinationOpen}>
        ...
        <PopoverContent align="start" className="w-80 p-4" sideOffset={8}>
          <p className="font-semibold text-sm">
            {localizeMarketplace(
              isBg,
              "Дестинацията е България",
              "The destination is Bulgaria"
            )}
          </p>
          <p className="mt-1 text-muted-foreground text-sm leading-5">
            {localizeMarketplace(
              isBg,
              "Заявката е за автомобил с доставка до България.",
              "Your request is for a vehicle delivered to Bulgaria."
            )}
          </p>
        </PopoverContent>
      </Popover>
    ...
    return (
      <nav
        aria-label={localizeMarketplace(
          isBg,
          "Бърз старт по вид автомобил",
          "Quick start by vehicle type"
        )}
        className="flex min-h-16 items-center justify-center gap-2"
        data-slot="desktop-service-shortcuts"
      >
        <span className="mr-1 font-semibold text-muted-foreground text-sm">
          {localizeMarketplace(isBg, "Продавате:", "Selling:")}
        </span>
        {leadSellCategories.map((category) => (
          <Button
            asChild
            className={serviceShortcutClassName}
            key={category.id}
            variant="ghost"
          >
            <Link
  ...
  if (mode === "imports") {
    return (
      <nav
        aria-label={localizeMarketplace(
          isBg,
          "Бързи маршрути за внос",
          "Quick import origins"
        )}
        className="flex min-h-16 items-center justify-center gap-2"
        data-slot="desktop-service-shortcuts"
      >
        <span className="mr-1 font-semibold text-muted-foreground text-sm">
          {localizeMarketplace(isBg, "Популярни държави:", "Popular origins:")}
        </span>
        {leadImportOrigins.map((origin) => (
          <Button
            asChild
        ...
        <Button asChild className={serviceShortcutClassName} variant="ghost">
          <Link
            href={getLocalizedPublicPath(
              locale,
              "/imports?start=1#import-request"
            )}
          >
