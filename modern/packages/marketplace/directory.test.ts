...
describe("organization directory search", () => {
  ...
  it("serializes only active filters and omits the first page", () => {
    expect(
      ...
      "type=importer&availability=local&brand=BMW&city=Sharjah&country=BG&origin=CN&deliverTo=BG&importService=transport&sort=inventory&official=true&verified=true&view=list"
    );
  });
  ...
  it("builds complete, de-duplicated facets from all directory entries", () => {
    expect(
      ...
      cities: ["Hamburg", "Plovdiv", "Ruse", "Sharjah", "Stara Zagora", "Varna"],
      importServices: [
        "vehicle_sourcing",
        "inspection",
        "export_documents",
