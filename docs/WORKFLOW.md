...
Cars owns canonical dealer source at `clients/<slug>/`. The four masters in templates/<key> own reusable improvements; immutable Cars template releases are selected by [templates.lock.json](../templates.lock.json). [Root instructions](../AGENTS.md) own task scope and publication authorization.
...
The everyday request is still: **clone the three approved templates, then personalize the dealer**. Standard output is `clients/<slug>/{auto-best,modern,carwow}`. The Import trio substitutes `import` for `modern`. These are three applications inside one dealer, not three new GitHub repositories or three Vercel projects. Admin remains one shared demo.
...
3. Use the existing synchronized Cars checkout on `main`. The clone command verifies the canonical Windows path, repository, current GitHub main and committed workflow/release inputs. It automatically compares each selected release with current template development on Cars main and checks immutable source integrity. A newer development commit is reported, not silently adopted: inspect it and complete [promotion](TEMPLATE-PROMOTION.md) for a finished verified update before claiming the newest release. Unfinished template drafts remain with their writer; they are not a reason to create another checkout.
...
5. Dry-run `node scripts/new-client.mjs --client <slug> --repository <owner/repo> --preset standard --dry-run` (or `--preset import`). On the authorized build, repeat without `--dry-run`. It copies the exact approved source with the same export policy used by releases and publishing, retains lockfiles, verifies the resulting digests, and excludes inherited secrets/instructions and reproducible build output. The three applications are prepared under ignored runtime and moved together into the one canonical dealer folder; a failed copy cannot install half a dealer. Generated guidance records exact template/source pins. The helper does not invent business facts or connect providers.
6. Apply identity, approved media, services, stock, prices and contact destinations through each template's existing boundaries. Preserve layout, typography, spacing, cards, hero composition/media behavior, routes and interactions. Do not create client-only replacement hero systems or workaround CSS/text/SVG logos. Complete the identity scan across retained routes and offered homepages. A styling defect that needs shared work goes to the Cars template master unless fixing it is necessary for this authorized demo.
