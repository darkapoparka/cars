...
2. Resolve the dealer's canonical published checkout. Use the existing recorded path when present. For a new standalone checkout, use `J:/cars-deployments/cars-<dealerkey>` and record it. Keep all selected variants in that one repository. Do not initialize or push the whole Cars library as the dealer repository.
3. Copy only required source, public assets, retained lockfiles, licensing/provenance and client metadata. Exclude `.git`, `.vercel`, credentials and `.env` secrets, dependencies, generated caches, local logs and CRM/deployment bindings. Recreate new deployment identity intentionally. Preserve dirty original client copies; do not sync over them. Record which checkout owns future published edits.
