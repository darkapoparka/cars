...
export const isLocale = (value) => value === 'en' || value === 'bg';
export const intlLocale = (locale) => locale === 'bg' ? 'bg-BG' : 'en-AE';
export const formatPrice = (value, locale) => new Intl.NumberFormat(intlLocale(locale), { style: 'currency', currency: 'AED', currencyDisplay: 'code', maximumFractionDigits: 0 }).format(value);
...
export function isResource(pathname) {
...
export function unsupportedLocale(pathname) {
...
export function localeHref(href, locale, defaultBase = '') {
...
export function safeReturnPath(value, origin) {
    if (typeof value !== 'string' || value.length > 2048 || !value.startsWith('/') || value.startsWith('//') || /[\\\u0000-\u001f]|%(?:0[0-9a-f]|1[0-9a-f]|5c)/i.test(value))
        return null;
    try {
        const url = new URL(value, origin);
        if (url.origin !== origin || isResource(url.pathname) || unsupportedLocale(url.pathname))
            return null;
...
export async function preferenceResponse(request) {
