# АВТОБОРСА МАНИЯ — source implementation and verification
...
Exact versions, files, diagnostics, hashes and limitations: `clients/avtoborsa-mania/VERIFICATION.json`, `ASSET-CHECKS.json`, `SOURCE-CHECKS.json`.
...
`clients/avtoborsa-mania/REVIEW.md` contains coordinator install/check commands and proposed local entries. No app server or deployment was started; no real enquiry or private record was sent. The shared Windows checkout and its source files were left unchanged.
