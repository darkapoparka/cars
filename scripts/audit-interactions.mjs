...
for(const [key,port,route,name] of entries) {
  ...
  try {
    ...
    if(name) {
      ...
      if(await trigger.count()) {
        ...
        await page.screenshot({path:path.join(out,'screenshots',`${key}-interaction-390.png`)});
  ...
  await fs.writeFile(path.join(out,process.argv[2]?'interactions-followup.json':'interactions.json'),JSON.stringify(results,null,2));
  await context.close();
}
await browser.close();
